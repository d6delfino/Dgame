...
function renderHostPanel(id) {
    ...
    panel.innerHTML = `
        ...
        <button class="action-btn" id="btn-start-online" onclick="hostStartGame()"
            ...
            AVVIA PARTITA (Normale)
        </button>
        <button class="action-btn" id="btn-start-campaign-online" onclick="hostStartOnlineCampaign()"
            style="width:100%; margin-top:6px; padding:14px; border:2px solid #555; color:#555; background:transparent; cursor:not-allowed; font-size:14px; font-weight:bold;" disabled>
            🗺️ AVVIA CAMPAGNA ONLINE
        </button>
    `;
...
function hostStartOnlineCampaign() {
    // Usiamo le variabili window per sicurezza
    ...
    // Notifica l'inizio ai client
    broadcastToClients({
        type: 'CAMPAIGN_ONLINE_START',
        numPlayers: window.onlineTotalPlayers,
    });

    // Attivazione stato campagna
    window.isCampaignOnline = true;
    window.campaignMyFaction = 1;
    
    if (typeof startCampaign === 'function') {
        startCampaign(window.onlineTotalPlayers);
    }
}
...
function showOnlineColorPicker() {
    ...
    overlay.style.cssText = `
        position:fixed; top:0; left:0; width:100%; height:100%;
        background:rgba(5,5,15,0.98); z-index:100000;
    ...
    let buttonsHtml = _FACTION_DEFS.map(def => {
        return `<button class="action-btn" onclick="requestColor(${def.slot})" 
                style="width:200px; margin:5px; border:2px solid ${def.color}; color:${def.color}; background:transparent;">
                ${def.name.toUpperCase()}
                </button>`;
    }).join('');

    overlay.innerHTML = `
        <h2 style="color:#fff; margin-bottom:20px;">SCEGLI LA TUA FAZIONE</h2>
