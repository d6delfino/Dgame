...
let clientConns = {};
window.getClientConns = () => clientConns;
...
// Set degli ID azione già processati (dedup messaggi ACTION duplicati).
// Viene svuotato a ogni FULL_STATE_SYNC per non crescere indefinitamente.
const _processedActionIds = new Set();
...
function showLocalMenu() {
    ...
    // FIX CRITICO: Resetta lo stato di rete quando si apre il menu locale.
    // Evita che una precedente visita al menu online blocchi i giocatori in locale.
    window.isOnline = false;
    window.isHost = false;
    window.myPlayerNumber = 0;
    window.isCampaignOnline = false;

    // Controllo Salvataggio: Mostra il tasto LOAD se esiste una campagna
    const loadBtn = document.getElementById('btn-local-load-camp');
    if (loadBtn) {
        const save = typeof getCampaignSave === 'function' ? getCampaignSave() : null;
        // Se c'è un salvataggio, togliamo il display:none per farlo inserire nel Flexbox
        loadBtn.style.display = save ? '' : 'none'; 
    }
}
...
function showOnlineMenu() {
    ...
    const campMenu = document.getElementById('campaign-num-players');
    if (campMenu) campMenu.remove();

    const box = document.getElementById('online-options');
    box.innerHTML = `
    <div style="display:flex; gap:15px; justify-content:center; margin-bottom:25px; flex-wrap:nowrap;">
        <button class="action-btn" id="btn-be-host" onclick="initAsHost()"
            style="padding:12px 22px; border:2px solid #00ff88; color:#00ff88; cursor:pointer; display:inline-flex; align-items:center; background:rgba(0,255,136,0.08); border-radius:12px; box-shadow:0 0 10px #00ff88 inset, 0 0 20px #00ff88, 0 0 45px #00ff8899;">
            <span style="font-size:2em; line-height:0; margin-right:10px; position:relative; top:2px;">💻</span> CREA (Host)
            </button>
            <button class="action-btn" id="btn-be-client" onclick="showClientPanel()"
            style="padding:12px 22px; border:2px solid #cc00ff; color:#cc00ff; cursor:pointer; display:inline-flex; align-items:center; background:rgba(204,0,255,0.08); border-radius:12px; box-shadow:0 0 10px #cc00ff inset, 0 0 20px #cc00ff, 0 0 45px #cc00ff99;">
            <span style="font-size:2em; line-height:0; margin-right:10px; position:relative; top:2px;">📡</span> UNISCITI (client)
        </button>
    </div>
...
function initAsHost() {
    ...
    peer = new Peer(generateShortId(4), { config: buildIceConfig(), debug: 1 });
    ...
    // Host appena connesso: nessuna fazione è ancora presa, sceglie a caso tra tutte
    const _hostRandomSlot = Math.floor(Math.random() * 8) + 1;
    players[1]._cosmeticFaction = _hostRandomSlot;
    players[1].color = _FACTION_DEFS[_hostRandomSlot - 1].color;
    players[1].name  = _FACTION_DEFS[_hostRandomSlot - 1].name;
    ...
    peer.on('connection', (c) => {
        ...
        if (c.metadata && c.metadata.playerNumber) {
            ...
            // 2. Assegnazione iniziale (Fix Limbo COOP)
            // In COOP onlineTotalPlayers è 9. Usiamo _coopHumanPlayers per limitare la ricerca 
            // solo agli slot umani reali, evitando di assegnare slot fantasma.
            const searchLimit = window._coopHumanPlayers ? window._coopHumanPlayers : onlineTotalPlayers;
            for (let n = 2; n <= searchLimit; n++) {
                if (!clientConns[n]) { 
                    assignedPlayer = n; 
                    break; 
                }
        ...
        // --- FIX CHIRURGICO: Espansione Automatica Lobby ---
        // Se un amico con il codice cerca di entrare ma la lobby risulta piena,
        // allarghiamo automaticamente la stanza per fargli spazio (fino a 8 max), 
        // a patto che l'Host sia ancora nel Menu principale in attesa.
        const isLobbyOpen = document.getElementById('network-menu') && document.getElementById('network-menu').style.display !== 'none';
        
        if (!assignedPlayer && isLobbyOpen) {
            if (window._coopHumanPlayers && window._coopHumanPlayers < 8) {
                window._coopHumanPlayers++;
                assignedPlayer = window._coopHumanPlayers;
            } else if (!window._coopHumanPlayers && onlineTotalPlayers < 8) {
                onlineTotalPlayers++;
                totalPlayers = onlineTotalPlayers;
                assignedPlayer = onlineTotalPlayers;
            }
            
            // Aggiorna visivamente i tasti dell'Host (evidenzierà automaticamente il tasto 3P, 4P ecc.)
            if (assignedPlayer && typeof renderHostPanel === 'function') {
                renderHostPanel(currentPeerId);
                // Avvisa i client già connessi che si è aggiunto un posto al tavolo
        ...
        // Il rifiuto ora avviene SOLO se si superano gli 8 giocatori o se la partita è già avviata
        if (!assignedPlayer) { 
            c.on('open', () => {
                c.send(JSON.stringify({ type: 'CONNECTION_REJECTED', reason: 'Partita già avviata o limite di 8 giocatori raggiunto.' }));
...
function renderHostPanel(id) {
    ...
    panel.innerHTML = `
        ...
        <div style="display:flex; gap:8px; margin-top:0;">
          <!-- Aggiunto overflow:visible allo stile del bottone -->
          <button class="action-btn" id="btn-start-online" onclick="hostStartGame()"
        style="flex:1; padding:14px 6px; border:2px solid #555; color:#555; background:transparent; cursor:not-allowed; font-size:13px; font-weight:bold; overflow:visible;" disabled>
        <span style="display:inline-block; width:0; height:0; overflow:visible; position:relative; z-index:0;">
            <img src="img/battle.png" style="height:4.8em; position:absolute; top:-2.9em; left:-1.5em; pointer-events:none; opacity:1;">
        </span>
        <span style="margin-left:2.2em; position:relative; z-index:1; text-shadow:-1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 0 #000;">AVVIA BATTAGLIA</span>
         </button>
        ...
        style="flex:1; padding:14px 6px; border:2px solid #555; color:#555; background:transparent; cursor:not-allowed; font-size:13px; font-weight:bold; overflow:visible;" disabled>
        <span style="display:inline-block; width:0; height:0; overflow:visible; position:relative; z-index:0;">
            <img src="img/campagna.png" style="height:4.8em; position:absolute; top:-2.9em; left:-1.5em; pointer-events:none; opacity:1;">
        </span>
        <span style="margin-left:2.2em; position:relative; z-index:1; text-shadow:-1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 0 #000;">AVVIA CAMPAGNA</span>
          </button>
        </div>
        <button class="action-btn" id="btn-start-coop-online" onclick="hostStartOnlineCoop()"
            style="width:100%; margin-top:8px; padding:14px 6px; border:2px solid #555; color:#555; background:transparent; cursor:not-allowed; font-size:13px; font-weight:bold;" disabled>
            <span style="display:inline-block; width:0; height:0; overflow:visible; position:relative;">
            <!-- Altezza portata a 4.8em e alzata con top: -1.4em -->
            <img src="img/coop.png" style="height:4.8em; position:absolute; top:-1.4em; left:-2.2em; pointer-events:none;">
            </span>
            <span style="margin-left:3.4em; text-shadow:-1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 0 #000;"> AVVIA COOP</span>
        </button>
...
function setOnlinePlayers(n) {
    ...
    for (let i = n + 1; i <= 8; i++) onlineAIFactions.delete(i);
    renderHostPanel(currentPeerId);
}
...
function updateHostLobby() {
    ...
    // --- NUOVA VARIABILE CONDIVISA ---
    // onlineAIFactions è l'elenco dei Bot aggiunti dall'Host
    const hasBots = onlineAIFactions.size > 0;

    // Gestione visiva bottone Campagna
    const campBtn = document.getElementById('btn-start-campaign-online');
    if (campBtn) {
        // La Campagna è pronta solo se: Umani Pronti + Min 2 Max 4 giocatori + NESSUN BOT
        const campReady = allReady && onlineTotalPlayers >= 2 && onlineTotalPlayers <= 4 && !hasBots;
        ...
        // Cambia il testo dinamicamente se ci sono bot
        if (hasBots) {
            // Usa innerHTML per mantenere l'immagine ma rimpicciolita/trasparente per indicare il "disabilitato"
            campBtn.innerHTML = `
                <span style="display:inline-block; width:0; height:0; overflow:visible; position:relative; z-index:0;">
                    <img src="img/campagna.png" style="height:3.5em; position:absolute; top:-1.5em; left:-1em; pointer-events:none; opacity:0.4;">
                </span>
                <span style="margin-left:1.8em; position:relative; z-index:1;">NO BOT IN CAMPAGNA</span>`;
            campBtn.style.fontSize = '10px';
        } else {
            campBtn.innerHTML = `
                <span style="display:inline-block; width:0; height:0; overflow:visible; position:relative; z-index:0;">
                    <img src="img/campagna.png" style="height:4.8em; position:absolute; top:-2.9em; left:-1.5em; pointer-events:none; opacity:1;">
                ...
                <span style="margin-left:2.2em; position:relative; z-index:1; text-shadow:-1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 0 #000;">AVVIA CAMPAGNA</span>`;
            campBtn.style.fontSize = '13px';
        }
    }

    // Gestione visiva bottone COOP
    const coopBtn = document.getElementById('btn-start-coop-online');
    if (coopBtn) {
        // La Coop è pronta solo se: Umani pronti + NESSUN BOT
        const coopReady = allReady && !hasBots;

        coopBtn.disabled = !coopReady;
        coopBtn.style.borderColor = coopReady ? '#00aaff' : '#555';
        coopBtn.style.color       = coopReady ? '#00aaff' : '#555';
        coopBtn.style.cursor      = coopReady ? 'pointer' : 'not-allowed';
        coopBtn.style.background  = coopReady ? 'rgba(0,170,255,0.08)' : 'transparent';
        
        if (hasBots) {
            coopBtn.innerHTML = `
                <span style="display:inline-block; width:0; height:0; overflow:visible; position:relative; z-index:0;">
                    <img src="img/coop.png" style="height:3.6em; position:absolute; top:-1.8em; left:-1em; pointer-events:none; opacity:0.4;">
                </span>
                <span style="margin-left:2.8em; position:relative; z-index:1;"> NO BOT IN COOP</span>`;
            coopBtn.style.fontSize = '11px'; 
        } else {
            coopBtn.innerHTML = `
                <span style="display:inline-block; width:0; height:0; overflow:visible; position:relative; z-index:0;">
                    <img src="img/coop.png" style="height:4.8em; position:absolute; top:-2.5em; left:-2em; pointer-events:none; opacity:1;">
                </span>
                <span style="margin-left:3.4em; position:relative; z-index:1; text-shadow:-1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 0 #000;"> AVVIA COOP</span>`;
            coopBtn.style.fontSize = '13px';
        }
    }
}
...
function setupHostConnection(c, playerNum) {
    ...
    c.on('data', (raw) => { 
        ...
        window.handleHostReceivedData(data, playerNum); 
    });
...
function handleClientDisconnection(playerNum) {
    ...
    // SE SIAMO IN CAMPAGNA: Forza il rinfresco della mappa e informa gli altri
    if (window.isCampaignOnline) {
        if (typeof _doHostBroadcast === 'function') _doHostBroadcast(); 
...
function showClientPanel() {
    ...
    panel.innerHTML = `
        <p style="color:#888; font-size:11px; margin-bottom:6px; text-transform:uppercase; text-align:left;">ID dell'Host</p>
        
        <!-- NUOVO CONTENITORE FLEXBOX PER ALLINEARLI -->
        <div style="display: flex; gap: 8px; width: 100%; margin-bottom: 12px; align-items: stretch;">
            
            <input type="text" id="peer-id-input" placeholder="Incolla ID..." autocomplete="off" autocorrect="off" autocapitalize="characters" spellcheck="false"
                style="flex: 1; min-width: 0; box-sizing:border-box; padding:12px 10px; background:#111; border:2px solid #cc00ff; color:#fff; font-size:18px; letter-spacing:2px; text-align:center; border-radius:4px; font-family:'Courier New',monospace;">
            
            <button class="action-btn" onclick="connectToHost()" id="btn-connect"
                style="padding:0 20px; background:#cc00ff; color:#fff; border:none; cursor:pointer; font-size:16px; font-weight:bold; border-radius:4px; white-space:nowrap; display:flex; align-items:center; justify-content:center;">
                CONNETTI
            </button>
...
function setupClientConnection(c) {
    ...
    c.on('data', (raw) => { 
        const data = typeof raw === 'string' ? JSON.parse(raw) : raw;
        window.handleClientReceivedData(data); 
    });
...
function _tryClientReconnect() {
    if (_storedHostId) {
        ...
        // ID host non disponibile: impossibile riconnettersi automaticamente.
        // Riporta il client al menu online con un messaggio esplicativo.
        console.warn("[Network] Nessun ID host memorizzato: riconnessione automatica impossibile.");
        const subtitle = document.getElementById('overlay-subtitle');
        if (subtitle) subtitle.innerText = "Riconnessione non disponibile. Ritorno al menu...";
        setTimeout(() => {
            resetClientToPreConnectionState();
            showTemporaryMessage("⚠️ Connessione persa. Reinserisci l'ID dell'Host per riconnetterti.", 5000);
        }, 2000);
    }
}
...
// Variabile globale per ricordare quale fazione stiamo guardando in anteprima
window._selectedPreviewSlot = null;

function showOnlineColorPicker() {
    const existing = document.getElementById('online-color-picker');
    if (existing) existing.remove();

    let currentFaction = players[myPlayerNumber]?._cosmeticFaction;
    if (!currentFaction) {
        // Nessuna fazione ancora scelta: anteprima casuale tra quelle libere
        const taken     = typeof _getTakenFactions === 'function' ? _getTakenFactions() : new Set();
        const freeSlots = _FACTION_DEFS.map(d => d.slot).filter(s => !taken.has(s));
        currentFaction  = freeSlots.length > 0
            ? freeSlots[Math.floor(Math.random() * freeSlots.length)]
            : myPlayerNumber;
    }
    window._selectedPreviewSlot = null; // Resetta l'anteprima all'apertura
    ...
    overlay.style.cssText = `
        position:fixed; top:0; left:0; width:100%; height:100%;
        background-image: url('img/faction${currentFaction}.png');
    ...
    // NOTA: Tolto l'onmouseenter e cambiato l'onclick per fare SOLO l'anteprima
    let buttonsHtml = _FACTION_DEFS.map(def => {
        return `<button id="fac-btn-${def.slot}" class="faction-btn action-btn" 
                onclick="previewFactionColor(${def.slot}, '${def.color}')" 
                style="width:100%; margin:0; border:2px solid ${def.color}; color:${def.color}; background:rgba(0,0,0,0.75); padding: 15px 5px; font-size: 14px; font-weight:bold; transition: all 0.2s; cursor:pointer;">
                ${def.name.toUpperCase()}
                </button>`;
    }).join('');

    overlay.innerHTML = `
        <div style="width: 100%; max-width: 450px; padding: 25px; box-sizing: border-box; text-align: center; background: rgba(10, 10, 20, 0.6); backdrop-filter: blur(2px); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; box-shadow: 0 0 40px rgba(0,0,0,0.8);">
            <h2 style="color:#fff; margin-top:0; margin-bottom:20px; font-size: clamp(18px, 5vw, 24px); text-transform: uppercase; text-shadow: 0 0 10px rgba(0,0,0,1); letter-spacing: 2px;">SCEGLI LA TUA FAZIONE</h2>
            
            <div id="faction-buttons-container" style="display:grid; grid-template-columns: 1fr 1fr; gap:10px; width: 100%; margin-bottom: 20px;">
                ${buttonsHtml}
            </div>
            
            <!-- NUOVO BOTTONE DI CONFERMA -->
            <button id="btn-confirm-faction" class="action-btn" onclick="confirmFactionColor()" disabled
                style="width:100%; padding: 15px; font-size: 18px; font-weight: bold; border: 2px solid #555; color: #555; background: rgba(0,0,0,0.8); cursor: not-allowed; opacity: 0.5; transition: all 0.3s;">
                SELEZIONA UN COLORE
            </button>

            <p id="color-picker-instruction" style="color:#ddd; margin-bottom:0; margin-top:15px; font-size:12px; line-height:1.4; text-transform: uppercase;">
                L'host dovrà poi avviare la missione
            </p>
        </div>
    `;
...
// Funzione chiamata quando si tocca/clicca un colore
window.previewFactionColor = function(slot, color) {
    window._selectedPreviewSlot = slot;
    playSFX('click'); // Suono opzionale se lo hai definito
    playFactionMusic(slot);
    
    // 1. Cambia lo sfondo
    document.getElementById('online-color-picker').style.backgroundImage = `url('img/faction${slot}.png')`;
    
    // 2. Resetta l'aspetto di tutti i bottoni
    ...
    // 3. Evidenzia il bottone selezionato
    const activeBtn = document.getElementById(`fac-btn-${slot}`);
    if (activeBtn) {
        activeBtn.style.boxShadow = `0 0 15px ${color}`;
        activeBtn.style.transform = 'scale(1.05)';
        activeBtn.style.background = `rgba(0,0,0,0.95)`;
    }

    // 4. Abilita il bottone di conferma colorandolo con la fazione scelta
    ...
    if (confirmBtn) {
        ...
        confirmBtn.style.boxShadow = `0 0 20px ${color}44`;
        confirmBtn.innerText = "✔ CONFERMA FAZIONE";
    }
};

// Funzione chiamata quando si preme "CONFERMA FAZIONE"
window.confirmFactionColor = function() {
    if (window._selectedPreviewSlot) {
        // Disabilita il bottone per evitare doppi click
        document.getElementById('btn-confirm-faction').innerText = "IN ATTESA...";
        document.getElementById('btn-confirm-faction').disabled = true;
        
        // Invia la scelta alla rete (la funzione requestColor è già gestita dal tuo gioco)
        requestColor(window._selectedPreviewSlot);
    }
};
...
function showOnlineColorPickerHost() {
    ...
    if (_ab) { _ab.style.display = 'block'; _ab.style.zIndex = '200001'; } 
    ...
    window.requestColor = function(slot) {
        ...
        if(taken.has(slot)) {
            showTemporaryMessage("⚠️ COLORE GIÀ PRENOTATO DA UN CLIENT!", 3000);
        } else {
