    ...
    panel.innerHTML = `
        ...
        <div style="display:flex; align-items:center; justify-content:center; gap:8px; margin-bottom:10px;">
            ...
            <div style="display:flex; gap:5px;">
                ${[2,3,4,5,6,7,8].map(n => `<button class="action-btn" onclick="setOnlinePlayers(${n})" id="online-p-btn-${n}"
...
function _tryClientReconnect() {
    if (_isReconnecting) return;
    _isReconnecting = true; // Lucchetto attivato
    ...
    setTimeout(() => {
        ...
        // PULIZIA: Distruggi attivamente la connessione zombie se esiste ancora
        if (hostConn) {
            hostConn.close();
            hostConn = null;
        }
        ...
        // Sblocca il lucchetto dopo poco, nel caso questo tentativo fallisca
