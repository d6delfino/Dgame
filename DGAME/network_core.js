...
function renderHostPanel(id) {
    ...
    panel.innerHTML = `
        ...
        <button class="action-btn" id="btn-start-campaign-online" onclick="hostStartOnlineCampaign()"
            ...
            \U0001F5FA\uFE0F AVVIA CAMPAGNA ONLINE
        </button>
    `;
...
function hostStartOnlineCampaign() {
    ...
    if (onlineTotalPlayers < 2 || onlineTotalPlayers > 4) {
        showTemporaryMessage('La campagna online supporta 2-4 giocatori.');
        return;
    }
    ...
    broadcastToClients({
        ...
        numPlayers: onlineTotalPlayers,
    });

    isCampaignOnline = true;
    campaignMyFaction = 1;
    if (typeof startCampaign === 'function') {
        startCampaign(onlineTotalPlayers);
    }
}
