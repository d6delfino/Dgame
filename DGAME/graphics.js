...
function drawGame() {
    ...
    grid.forEach(cell => {
        ...
        if (cell.terrain && cell.type === 'empty' && !cell.entity) {
            const p = hexToPixel(cell.q, cell.r);
            ctx.font      = `${Math.round(HEX_SIZE * 0.4)}px Arial`;
            ctx.textAlign = 'center';
            ctx.globalAlpha = 1; // 0.85 Leggermente trasparente
            ctx.fillText(TERRAINS[cell.terrain].icon, p.x, p.y + HEX_SIZE * 0.15);
            ctx.globalAlpha = 1.0;
        }
    ...
    controlPoints.forEach(cp => {
        ...
        // Bordo esagonale pulsante (più spesso se conquistato)
        const lineW = cp.faction > 0 ? 3 : 1.5;
        ctx.save();
        ctx.shadowBlur  = cp.faction > 0 ? 12 : 4;
        ctx.shadowColor = cpColor;
        drawHex(cp.q, cp.r, cpColor, cpColor + '22', lineW);
        ctx.restore();

        // Icona: rombo colorato con fazione (canvas-drawn, rispetta il colore)
        const ds = HEX_SIZE * 0.28;
        ctx.save();
        ...
        ctx.textAlign = 'center';
        ctx.shadowBlur = 0;
        ctx.fillText('+1', p.x, p.y + HEX_SIZE * 0.62);
        ctx.restore();
    });
