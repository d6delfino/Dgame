...
function handleHostReceivedData(data, fromPlayer) {
    ...
    if (data.type === 'SETUP_DONE') {
        // FIX CRITICO: Deep copy degli agenti ricevuti dal client per evitare che vengano 
        // accidentalmente resettati a 1 dalle logiche di riferimento di Javascript.
        clientSetupBuffer[fromPlayer] = JSON.parse(JSON.stringify(data.agents));
        ...
        players[fromPlayer]._colorConfirmed = true; // Protegge il colore del client dal loop AI
...
function handleClientReceivedData(data) {
    ...
    if (data.type === 'FULL_STATE_SYNC') {
        // Il sync completo reimposta tutto lo stato: gli actionId
        // precedenti non sono più rilevanti e il Set va svuotato
        // per non crescere indefinitamente nel corso della partita.
    ...
    if (data.type === 'ASSIGN_PLAYER') {
        ...
        // Scarta duplicati: se questo actionId è già stato processato, ignora.
        if (data.actionId && _processedActionIds.has(data.actionId)) {
            console.warn(`[Client] ACTION duplicata scartata: ${data.actionId}`);
            return;
        }
        if (data.actionId) _processedActionIds.add(data.actionId);
        executeRemoteAction(data);
...
function hostStartGame() {
    ...
    // Ri-applica i metadati cosmetici dei client (azzerati da resetPlayers)
    for (const [p, meta] of Object.entries(clientCosmeticBuffer)) {
        const pNum = parseInt(p);
        if (meta.cosmeticFaction) players[pNum]._cosmeticFaction = meta.cosmeticFaction;
        if (meta.color)           players[pNum].color            = meta.color;
        if (meta.name)            players[pNum].name             = meta.name;
        players[pNum]._colorConfirmed = true; // Ripristina il flag: i bot non possono sovrascrivere questo colore
    }
    ...
    onlineAIFactions.forEach(faction => {
        ...
        // FIX CHIRURGICO: Segna il colore del Bot come "confermato", in modo che
        // le prossime intelligenze artificiali lo leggano come "già occupato"
        // e non appaiano più bot con colori (ed HQ) sdoppiati.
        players[faction]._colorConfirmed = true;
...
function tryHostStart() {
    ...
    // Applica gli agenti dal buffer prima di generare la mappa (CON DEEP COPY)
    for (const [p, agents] of Object.entries(clientSetupBuffer)) {
        players[parseInt(p)].agents = JSON.parse(JSON.stringify(agents));
    }
...
function _hostSendFullSync(targetPlayerNum = null) {
    ...
    const msg = {
        ...
        state: {
            ...
            currentPlayer,
            turnCount,
            firstPlayerOfGame: _firstPlayerOfGame,
            onlineAIFactions:  Array.from(onlineAIFactions),
            controlPoints:     Array.from(controlPoints.values()),
        },
    };
    ...
    if (targetPlayerNum !== null) {
        ...
        safeSend(c, msg, `FullSync → P${targetPlayerNum}`);
    } else {
...
function _applyFullStateSync(st) {
    ...
    if (st.firstPlayerOfGame !== undefined) _firstPlayerOfGame = st.firstPlayerOfGame;
    ...
    if (st.players) {
        for (let p = 1; p <= totalPlayers; p++) {
            ...
            players[p].agents.forEach(a => {
                // FIX: _moveTime/_lastQ/_lastR sono solo metadati locali per
                // l'animazione del movimento in drawGame(); se arrivano dalla
                // rete portano il timestamp dell'orologio dell'Host, che
                // confrontato con Date.now() del client può causare un
                // "teletrasporto" visivo casuale. Li azzeriamo sempre
                // quando l'agente viene (ri)creato da uno stato di rete.
                a._moveTime = null;
    ...
    // Applicato PRIMA di startTimer così il setInterval parte dal
    // valore corretto dell'host e non da GAME.TURN_TIMER_SEC.
    if (st.timeLeft !== undefined) {
        timeLeft = st.timeLeft;
    }

    updateUI();
    // Unico punto di avvio del timer nel sync: dopo che tutto lo stato
    // è ricostruito e timeLeft è già al valore corretto.
    if (state === 'PLAYING' && !window._isSpectating) startTimer();
    invalidateStaticLayer();
    
    // --- Centra la base una sola volta, al primo sync ricevuto ---
    if (!window._clientCameraFocused && typeof focusCameraOnLocalPlayer === 'function' && state === 'PLAYING') {
        window._clientCameraFocused = true;
...
// ============================================================
// FIX CHIRURGICO: Gestione Rifiuto Connessione (Lobby Piena)
// ============================================================
registerClientMessageHandler('CONNECTION_REJECTED', function(data) {
    // 1. Fondamentale: impostando isOnline a false, impediamo alla funzione 
    // _onClientSideDisconnect di far scattare la schermata di "Connessione Interrotta"
    window.isOnline = false; 
    
    // 2. Blocchiamo il loop di riconnessione automatica cancellando la memoria
    sessionStorage.removeItem('RICONNETTITI');
    window._storedHostId = '';

    // 3. Resettiamo l'interfaccia utente (rimettiamo a posto il bottone di connessione)
    const status = document.getElementById('connection-status');
    if (status) {
        status.innerText = `❌ Rifiutata: ${data.reason}`;
    ...
    // 4. Mostriamo un banner a schermo per l'utente
    if (typeof showTemporaryMessage === 'function') {
        showTemporaryMessage(`Accesso negato: ${data.reason}`, 4000);
    }
});
