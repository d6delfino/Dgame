...
function broadcastToClients(data, exceptPlayer = null) {
    ...
    Object.entries(clientConns).forEach(([num, c]) => {
        if (parseInt(num) !== exceptPlayer && c && c.open) {
            // Invia il pacchetto con un leggero ritardo crescente per ogni client.
            // Questo previene l'overflow del buffer WebRTC (causa di disconnessioni).
            setTimeout(() => {
                try {
                    c.send(data);
                } catch(e) {
            ...
            delayMs += 40; // Aggiunge 40ms di respiro tra l'invio a un giocatore e l'altro
        }
    });
}
...
function tryHostStart() {
    ...
    grid.forEach(cell => {
        if (cell.type === 'wall' || cell.type === 'barricade') {
            walls.push({
                q: cell.q, r: cell.r, type: cell.type,
