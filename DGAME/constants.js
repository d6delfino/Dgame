...
const GAME = {
    SETUP_POINTS:     20,    // punti disponibili nella fase di setup
    AGENT_COST:        1,    // costo per reclutare un agente 4 di default
    AP_PER_TURN:       3,    // action points per agente per turno
    HQ_HP:            30,    // HP iniziali di ogni quartier generale
    HQ_DMG:            2,    // Danno base quartier generale
    HQ_RNG:            2,    // Gittata base quartier generale
    HQ_MOV:            0,    // Movimento quartier generale (non può muoversi)
    ...
    CREDIT_AGENT_BASE:   2,    // costo base per reclutare un agente 4 di default (+ stat extra)
...
const AGENT_STAT_MAX = { hp: 5, mov: 3, rng: 6, dmg: 4 };   // massimali delle abilità degli agenti
...
const CAMPAIGN = {
    ...
    HQ_INCOME:                5,
    STARTING_CREDITS:         9,   // crediti a inizio campagna
    UNIT_UPKEEP:              1,   // crediti/turno per mantenere ogni unità persistente in vita
    UNIT_AGENT_MIN_VALUE:     4,    // investimento minimo per creare un'unità Agente sulla mappa campagna
    ...
    UPGRADE_MINEFIELD_COST:  15,   // costo Campo Minato (ferma l'attaccante)
    ...
    UPGRADE_REVOLT_COST:     20,    // costo upgrade Rivolta
...
// Tipologie di terreno assegnate casualmente ai settori della mappa campagna.
// 'bonus' è un placeholder per il futuro effetto legato al terreno (per ora nessun effetto).
const CAMPAIGN_TERRAIN_DEFS = [
    { key: 'mountain', icon: '⛰️', name: 'Montagna', bonus: null },
    { key: 'sea',       icon: '🌊', name: 'Mare',      bonus: null },
    { key: 'forest',    icon: '🌲', name: 'Foresta',   bonus: null },
    { key: 'desert',    icon: '☀️', name: 'Deserto',   bonus: null },
];
...
// --- AUDIO ---
const SFX = {
    intro:   new Audio('sfx/sfx-intro.mp3'),
    ...
    airdrop: new Audio('sfx/sfx-airdrop.mp3'),
    shield:  new Audio('sfx/sfx-shield.mp3'),
    cards4:  new Audio('sfx/sfx-cards4.mp3'),
    emp:     new Audio('sfx/sfx-emp.mp3'),
...
/** Chiave intera univoca per Map e Set (zero allocazioni in memoria RAM) */
function getKey(q, r) { 
    // Offset di 100 per le coordinate negative (gestisce mappe fino a raggio 90)
    // Moltiplicatore 1000 sfalsa le 'q' dalle 'r' evitando qualsiasi collisione
    return (q + 100) * 1000 + (r + 100); 
}
...
function playSFX(effect) {
    ...
    // 1. VARIEGAZIONE BASE: Diamo a TUTTI i suoni una leggerissima 
    // variazione di velocità/pitch (tra 0.95 e 1.05). 
    // Questo rende spari e passi meno noiosi e meccanici.
    ...
    // Suoni a cui vogliamo applicare l'effetto "due finestre" (spari, passi, esplosioni)
    const canBeThickened = ['laser', 'melee', 'move', 'uzi', 'boom'].includes(effect);

    // 50% di probabilità di avere il suono "corposo" (puoi alzare a 1 se lo vuoi sempre)
    if (canBeThickened && Math.random() < 0.5) {
        
        // Simula la latenza di rete tra due finestre: un ritardo tra i 25 e i 60 ms.
        const lagDelayMs = 25 + Math.floor(Math.random() * 35);

        setTimeout(() => {
            const clone = audio.cloneNode();
            
            // Il clone ha un pitch ancora leggermente diverso dall'originale.
            // Questo crea l'effetto Chorus/Corposità che hai notato.
            clone.playbackRate = baseRate + (Math.random() * 0.08 - 0.04); 
            
            // Il volume del clone è un po' più basso (60%) per "ingrossare" il 
            // suono originale senza saturare le casse o creare rimbombi fastidiosi.
            clone.volume = 0.6; 
            ...
            // Pulizia memoria quando finisce
...
function toggleLegend() {
    ...
    // Mostra l'ID sessione se si è in una partita online
    const sessionDiv = document.getElementById('legend-session-id');
    const sessionVal = document.getElementById('legend-session-id-value');
    if (sessionDiv && sessionVal && typeof isOnline !== 'undefined' && isOnline) {
        const id = (typeof isHost !== 'undefined' && isHost)
            ? (typeof currentPeerId !== 'undefined' ? currentPeerId : '')
            : (typeof _storedHostId !== 'undefined' ? _storedHostId : '');
        if (id) {
            sessionVal.textContent = id;
            sessionVal.onclick = () => {
                navigator.clipboard?.writeText(id);
                sessionVal.textContent = '✅ copiato!';
                setTimeout(() => { sessionVal.textContent = id; }, 1500);
            };
