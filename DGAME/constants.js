...
function toggleMusic() {
    ...
    // Sincronizza il colore su ENTRAMBI i bottoni (Battaglia e Campagna)
    const color = musicPlaying ? '#00ff88' : '#ff4444';
    
    const battleBtn = document.getElementById('audio-toggle');
    if (battleBtn) battleBtn.style.color = color;
    
    const campBtn = document.getElementById('camp-music-btn');
    if (campBtn) campBtn.style.color = color;
}
...
function playSFX(effect) {
    ...
    if (audio) {
        ...
        // --- NUOVO: DISTORSIONE ACUSTICA CASUALE ---
        // Applichiamo la variazione solo ai suoni di combattimento/azione
        // per evitare che i suoni dell'interfaccia (click) sembrino "stonati"
        if (['laser', 'melee', 'explosion'].includes(effect)) {
            // Genera un valore tra 0.9 (più grave/lento) e 1.1 (più acuto/veloce)
            const randomPitch = 0.75 + Math.random() * 0.6;
            audio.playbackRate = randomPitch;
        } else {
            // Reset per suoni normali (click, menu, etc.)
            audio.playbackRate = 1.0;
        }
        
        // Firefox/Chrome richiedono la gestione della Promise
        const playPromise = audio.play();
        
        if (playPromise !== undefined) {
            playPromise.catch(error => {
                // Silenzia errori se l'utente non ha ancora interagito
                console.warn(`[Audio] Riproduzione in attesa: ${effect}`);
            });
        }
    }
}
