...
function processConflicts() {
    ...
    campaignState.sectors.forEach(sector => {
        ...
        // CAMPO MINATO: se attivo, il settore viene rimosso dalla battleQueue
        // e gli attaccanti vengono fermati sul settore senza conquistarlo.
        // Il campo minato si consuma dopo il primo utilizzo.
        if (sector.mineField && sector.owner > 0 && attackers.length > 0) {
            sector.mineField = false;           // consumato
            sector._mineFieldJustTriggered = true;  // usato da _showConflictSummary
            return;   // forEach callback — skippa questo settore
        }
...
function _showConflictSummary() {
    ...
    const peaceful = [];
    const minefieldsTriggered = [];  // settori dove il campo minato è scattato
    ...
    campaignState.sectors.forEach(s => {
        ...
        // Rileva campi minati scattati: settori che avevano mineField=true ma ora è false
        // e avevano almeno un attaccante (ordine pendente su di essi da un nemico)
    ...
    if (minefieldsTriggered.length > 0) {
        html += `<p style="color:#ff8800;font-size:22px;margin-top:30px;margin-bottom:12px;font-weight:bold;text-transform:uppercase;">💣 CAMPI MINATI SCATTATI:</p>`;
        minefieldsTriggered.forEach(sid => {
            html += `<div style="font-size:22px;margin-bottom:10px;background:rgba(255,136,0,0.1);padding:10px;border-radius:8px;border-left:4px solid #ff8800;">
                💥 Settore ${sid}: gli attaccanti sono stati bloccati dalle mine!</div>`;
        });
    }
    if (battles.length > 0) {
    ...
    if (!peaceful.length && !battles.length && !minefieldsTriggered.length)
...
function showBattleResults(winnerFaction) {
    ...
    // Gli upgrade NON-economici (Campo Minato, Fortezza) non si trasferiscono al conquista-tore:
    // sono infrastrutture del precedente difensore e vengono distrutte in battaglia.
    // La Miniera invece si trasferisce perché è già codificata in s.income (valore intrinseco del settore).
    if (winnerFaction !== previousOwner) {
        sector.mineField      = false;
        sector.fortressUpgrade = false;
        // s.mineUpgrade e s.income rimangono: la miniera appartiene al territorio, non al giocatore
    }
...
function _showCampaignWinUI(winner) {
    ...
    ['controls-panel', 'setup-overlay', 'battle-results-overlay', 'cn-result-overlay'].forEach(id => {
