...
function processConflicts() {
    ...
    (campaignState._roundLog || []).forEach(entry => {
        if (entry.type === 'ARTILLERY') {
            _applyArtilleryStrikeLocale(entry.sid, entry.p);
        }
    });
    
    // --- FASE 0b: RIVOLTA ---
    // Se qualcuno ha acquistato la Rivolta questo turno, la eseguiamo ora.
    // Tutti i settori (alleati e nemici) con crediti totali < soglia tornano neutrali.
    ...
    if (hasRevolt) {
        campaignState.sectors.forEach(s => {
            ...
            // Somma i crediti allocati su questo settore da tutti i giocatori
        ...
        // Auto-rimozione: cancella il flag su tutti i settori
    ...
    campaignState.sectors.forEach(sector => {
        ...
        if (sector.mineField && sector.owner > 0 && attackers.length > 0) {
            ...
            // NUOVO: Annientamento logico degli attaccanti
            attackers.length = 0; 
            
            return; // Fine della valutazione per questo settore
        }
