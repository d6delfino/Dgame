...
function _showConflictSummary() {
    ...
    ? `document.querySelectorAll('.campaign-summary-overlay').forEach(el=>el.remove()); if(typeof _net_clientSend==='function') _net_clientSend('SYNC_REQUEST',{}); if(typeof _prepareMapDOM==='function') _prepareMapDOM(); renderCampaignMap();`
    : `document.querySelectorAll('.campaign-summary-overlay').forEach(el=>el.remove()); runNextBattle();`;
...
function _agentSurvivorValue(faction) {
    ...
    survivors.forEach(a => {
        // 2. Legge le stat "nude" filtrando sia i bonus laboratorio sia i bonus
        //    temporanei delle carte (sniper, demo, medikit) usando i valori
        //    originali salvati sull'agente prima di ogni buff.
        const baseMaxHp = a.medikitTempBuff ? (a._medikitOrigMaxHp ?? a.maxHp) : a.maxHp;
        const baseMov   = a.medikitTempBuff ? (a._medikitOrigMov   ?? a.mov)   : a.mov;
        const baseRng   = a.sniperBuff      ? (a.originalRng       ?? a.rng)   :
                          a.medikitTempBuff  ? (a._medikitOrigRng   ?? a.rng)   : a.rng;
        const baseDmg   = a.demoBuff        ? (a.originalDmg       ?? a.dmg)   :
                          a.medikitTempBuff  ? (a._medikitOrigDmg   ?? a.dmg)   : a.dmg;
        ...
        //totalValue += Math.floor(agentRealCost * 0.5);  // metà del costo agente
    });
    return totalValue;
}
...
function showCampaignMenu() {
    ...
    if (save) {
        ...
        div.innerHTML = `
            <p style="color:#aaa;margin-bottom:12px;text-transform:uppercase;">campagna : numero giocatori</p>
