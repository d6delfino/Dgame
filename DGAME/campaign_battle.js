/* ============================================================
   campaign_battle.js  —  CAMPAGNA: CONFLITTI, BATTAGLIE E RISULTATI
   ============================================================
   RESPONSABILITÀ:
   - Risoluzione conflitti al termine di ogni round di pianificazione
   - Avvio battaglie tattiche (startCampaignBattle, runNextBattle)
   - Override delle funzioni di gioco per il contesto campagna
     (freshSetupData, resetTurnState, confirmPlayerSetup, showGameOverlay)
   - Visualizzazione risultati battaglia e aggiornamento mappa
   - Controllo vittoria campagna (checkCampaignWin)
   - Stub funzioni network campagna

   ESPONE: processConflicts, runNextBattle, startCampaignBattle,
           showBattleResults, checkCampaignWin

   DIPENDE DA: constants.js (CAMPAIGN, GAME), state.js,
               campaign_map.js (campaignState, renderCampaignMap)
   CARICATO DOPO: campaign_map.js
   ============================================================ */

// ============================================================
// RISOLUZIONE CONFLITTI
// ============================================================

/**
 * Scansiona tutte le unità persistenti raggruppandole per settore.
 * - Se in un settore ci sono unità di una sola fazione: conquista pacifica
 *   (nessuna battaglia, come oggi per gli attacchi a credito senza opposizione).
 * - Se ci sono unità di 2+ fazioni: le somma per proprietario e accoda una
 *   battaglia multi-fazione nella stessa battleQueue usata dal sistema esistente.
 */


/** Restituisce il settore in cui un'unità si troverà A FINE TURNO, cioè dopo l'eventuale
 *  movimento in sospeso di questo round (pendingUnitMoves) — non quello di inizio turno.
 *  Serve a far colpire Nuke e Artiglieria in base alla posizione reale al momento
 *  dell'impatto: un'unità che fugge dal settore bersaglio si salva, una che vi entra
 *  proprio in questo turno viene colpita. Va chiamato PRIMA che _applyPendingUnitMoves()
 *  svuoti campaignState.pendingUnitMoves. */
function _getUnitEndOfRoundSector(unit) {
    const myMoves = (campaignState.pendingUnitMoves && campaignState.pendingUnitMoves[unit.owner]) || [];
    const move = myMoves.find(m => m.unitId === unit.id);
    return move ? move.destSectorId : unit.sectorId;
}
window._getUnitEndOfRoundSector = _getUnitEndOfRoundSector;

/** Applica simultaneamente tutti gli spostamenti di unità accumulati nel round,
 *  per tutti i giocatori insieme, prima che scatti la risoluzione dei conflitti. */
function _applyPendingUnitMoves() {
    if (!campaignState.pendingUnitMoves) return;
    const mineVictims = campaignState._mineVictimUnitIds || [];

    Object.values(campaignState.pendingUnitMoves).flat().forEach(order => {
        // FIX: se l'unità stava entrando in un settore con Droni-Mina attivi, viene
        // eliminata invece di essere spostata, e il campo minato si consuma (monouso) —
        // esattamente come per il vecchio sistema di attacco a crediti.
        if (mineVictims.includes(order.unitId)) {
            const destSector = campaignState.sectors[order.destSectorId];
            if (destSector) {
                destSector.mineField = false;
                destSector._mineFieldJustTriggered = true;
            }
            removeCampaignUnit(order.unitId);
            return;
        }

        const unit = campaignState.units.find(u => u.id === order.unitId);
        if (unit) unit.sectorId = order.destSectorId;
    });

    campaignState.pendingUnitMoves = {};
    campaignState._mineVictimUnitIds = [];
}

function _resolveUnitConflicts() {
    const bySector = {};
    campaignState.units.forEach(u => {
        (bySector[u.sectorId] = bySector[u.sectorId] || []).push(u);
    });

    Object.entries(bySector).forEach(([sectorIdStr, unitsHere]) => {
        const sectorId = parseInt(sectorIdStr);
        const sector   = campaignState.sectors[sectorId];
        if (!sector || sector.blocked) return;

        const factions = [...new Set(unitsHere.map(u => u.owner))];

        if (factions.length === 1) {
            if (sector.owner !== factions[0]) {
                sector.owner = factions[0]; // conquista pacifica
                if (!campaignState._roundLog) campaignState._roundLog = [];
                campaignState._roundLog.push({ type: 'UNIT_CONQUEST', p: factions[0], sid: sectorId });
            }
            return;
        }

        // 2+ fazioni nello stesso settore: battaglia multi-fazione.
        // L'Artiglieria NON contribuisce al budget di battaglia (vale 0): viene solo
        // tracciata a parte per essere catturata da chi vince il settore, non distrutta.
        const battleCredits  = {};
        const unitIdsByOwner = {};
        const artilleryIds   = [];
        unitsHere.forEach(u => {
            if (u.type === 'artillery') { artilleryIds.push(u.id); return; }
            battleCredits[u.owner]  = (battleCredits[u.owner] || 0) + u.value;
            (unitIdsByOwner[u.owner] = unitIdsByOwner[u.owner] || []).push(u.id);
        });

        // FIX: applica il bonus Fortezza (+5 crediti in difesa) anche negli scontri tra
        // unità persistenti, non solo negli attacchi a crediti "classici" della FASE 2/3.
        // Il bonus va al proprietario ATTUALE del settore (il difensore), solo se ha
        // davvero unità in combattimento in quel settore.
        if (sector.fortressUpgrade && battleCredits[sector.owner] > 0) {
            battleCredits[sector.owner] += CAMPAIGN.UPGRADE_FORTRESS_COST / 2;
        }

        // Solo le fazioni con una vera forza di combattimento (agenti, quindi credito > 0,
        // dato che il minimo per creare un agente è già 4) partecipano davvero alla battaglia.
        const fightingFactions = Object.keys(battleCredits).map(Number).filter(f => battleCredits[f] > 0);

        if (fightingFactions.length >= 2) {
            campaignState.battleQueue.push({ sectorId, factions: fightingFactions, battleCredits, unitIdsByOwner, artilleryIds });
        } else if (fightingFactions.length === 1) {
            // Un solo combattente reale presente (le altre fazioni avevano solo artiglieria):
            // conquista senza battaglia, e l'artiglieria altrui passa a lui.
            const winner = fightingFactions[0];
            if (sector.owner !== winner) {
                sector.owner = winner;
                if (!campaignState._roundLog) campaignState._roundLog = [];
                campaignState._roundLog.push({ type: 'UNIT_CONQUEST', p: winner, sid: sectorId });
            }
            artilleryIds.forEach(uid => {
                const u = campaignState.units.find(x => x.id === uid);
                if (u) { u.owner = winner; u.hasActedThisTurn = true; }
            });
        }
        // Se fightingFactions.length === 0 (solo artiglierie di fazioni diverse, nessun agente):
        // nessuno ha una forza per conquistare, il settore resta invariato (stallo).
    });
}

async function processConflicts() {
    // --- FIX CRITICO: BLOCCO IMMEDIATO DELLA FASE ---
    // Passiamo subito a 'RESOLVING' prima di qualsiasi await.
    // Questo disattiva immediatamente il pulsante "Conferma" per Host e Locale.
    campaignState.phase = 'RESOLVING';
    if (typeof renderCampaignMap === 'function') renderCampaignMap();

    if (window.isCampaignOnline && !window.isHost) {
        console.log("[Campaign] Client in attesa di risoluzione dall'Host...");
        return; 
    }

    (campaignState._roundLog || []).forEach(entry => {
        if (entry.type === 'ARTILLERY') _applyArtilleryStrikeLocale(entry.sid, entry.p);
    });
    
    // --- FASE 0b: RIVOLTA ---
    const hasRevolt = campaignState.sectors.some(s => s.revoltUpgrade === true);
    if (hasRevolt) {
        // Raccoglie i settori colpiti PRIMA di modificare owner
        const revoltedSectors = [];
        campaignState.sectors.forEach(s => {
            if (s.owner <= 0 || s.blocked) return;
            const totalCredits = Object.values(campaignState.sectorCredits[s.id] || {})
                .reduce((sum, v) => sum + (v || 0), 0);
            if (totalCredits < CAMPAIGN.UPGRADE_REVOLT_THRESHOLD) {
                revoltedSectors.push({ x: s.x, y: s.y });
                s.owner = 0;
                campaignState.sectorCredits[s.id] = {};
            }
        });
        if (!campaignState._roundLog) campaignState._roundLog = [];
        campaignState._roundLog.push({ type: 'REVOLT' });
        campaignState.sectors.forEach(s => { delete s.revoltUpgrade; });

        // Anima i settori colpiti
        if (revoltedSectors.length > 0) {
            if (window.isCampaignOnline && window.isHost && typeof _net_broadcast === 'function') {
                _net_broadcast({ type: 'CAMPAIGN_REVOLT_ANIMATION', sectors: revoltedSectors });
            }
            _queueCampaignAnim(() => _playRevoltAnimation(revoltedSectors));
            await _waitCampaignAnimations();
        }
    }

    // --- FASE 1: ESECUZIONE SABOTAGGI ---
    // Raccoglie gli strikes, anima prima, poi applica la logica
    const nukeStrikes = [];
    for (let p = 1; p <= campaignState.numPlayers; p++) {
        for (const order of (campaignState.pendingOrders[p] || [])) {
            if (!order.isSabotage) continue;
            const sector = campaignState.sectors[order.sectorId];
            const src = campaignState.sectors.find(s => s.owner === p && s.icbmUpgrade)
                     || campaignState.sectors.find(s => s.owner === p && s.nukeUnlockUpgrade)
                     || campaignState.sectors.find(s => s.owner === p);
            nukeStrikes.push({
                p,
                sectorId: order.sectorId,
                fromX: src ? src.x : 500, fromY: src ? src.y : 375,
                toX: sector.x, toY: sector.y,
            });
        }
    }

    // --- FASE 1a-bis: ANIMAZIONE INFILTRATI ---
    const infiltratiAnims = [];
    for (let p = 1; p <= campaignState.numPlayers; p++) {
        for (const order of (campaignState.pendingOrders[p] || [])) {
            if (!order.isInfiltrati || order.fromX == null) continue;
            const target = campaignState.sectors[order.sectorId];
            if (!target) continue;
            infiltratiAnims.push({
                fromX: order.fromX, fromY: order.fromY,
                toX: target.x,      toY: target.y,
            });
        }
    }
    if (infiltratiAnims.length > 0) {
        if (window.isCampaignOnline && window.isHost && typeof _net_broadcast === 'function') {
            _net_broadcast({ type: 'CAMPAIGN_INFILTRATI_ANIMATION', strikes: infiltratiAnims });
        }
        for (const a of infiltratiAnims) {
            _queueCampaignAnim(() => _playInfiltratiAnimation(a.fromX, a.fromY, a.toX, a.toY));
        }
        await _waitCampaignAnimations();
    }

    // --- FASE 1a: ANIMAZIONE MINE ---
    // Legge pendingOrders PRIMA che la Fase 2 consumi mineField,
    // stesso pattern del nuke che legge isSabotage prima di _applyNuclearize.
    const mineAnims = [];
    campaignState._mineVictimUnitIds = []; // FIX: unità da eliminare per le mine, usato da _applyPendingUnitMoves

    for (let p = 1; p <= campaignState.numPlayers; p++) {
        for (const order of (campaignState.pendingOrders[p] || [])) {
            if (order.isSabotage) continue;
            const sector = campaignState.sectors[order.sectorId];
            if (!sector || !sector.mineField || sector.owner === 0) continue;
            if (sector.owner === p) continue; // non attacca se è già suo
            // Trova il settore dell'attaccante più vicino al bersaglio
            let srcSector = null;
            let minDist = Infinity;
            campaignState.sectors.forEach(s => {
                if (s.owner !== p) return;
                const d = Math.hypot(s.x - sector.x, s.y - sector.y);
                if (d < minDist) { minDist = d; srcSector = s; }
            });
            // Evita duplicati se più ordini puntano allo stesso settore minato
            if (!mineAnims.some(a => a.toX === sector.x && a.toY === sector.y)) {
                mineAnims.push({
                    fromX: srcSector ? srcSector.x : sector.x - 80,
                    fromY: srcSector ? srcSector.y : sector.y,
                    toX: sector.x,
                    toY: sector.y,
                });
            }
        }

        // FIX: gli attacchi ora avvengono quasi sempre spostando unità persistenti
        // (moveCampaignUnit / infiltrateCampaignUnit), che non passano più da
        // pendingOrders: senza questo, i Droni-Mina non "vedono" mai l'attacco.
        for (const move of (campaignState.pendingUnitMoves[p] || [])) {
            const sector = campaignState.sectors[move.destSectorId];
            if (!sector || !sector.mineField || sector.owner === 0) continue;
            if (sector.owner === p) continue; // non attacca se è già suo

            const movingUnit = campaignState.units.find(u => u.id === move.unitId);
            const srcSector  = movingUnit ? campaignState.sectors[movingUnit.sectorId] : null;

            if (!mineAnims.some(a => a.toX === sector.x && a.toY === sector.y)) {
                mineAnims.push({
                    fromX: srcSector ? srcSector.x : sector.x - 80,
                    fromY: srcSector ? srcSector.y : sector.y,
                    toX: sector.x,
                    toY: sector.y,
                });
            }
            campaignState._mineVictimUnitIds.push(move.unitId);
        }
    }

    if (mineAnims.length > 0) {
        if (window.isCampaignOnline && window.isHost && typeof _net_broadcast === 'function') {
            _net_broadcast({ type: 'CAMPAIGN_MINEFIELD_ANIMATION', strikes: mineAnims });
        }
        for (const a of mineAnims) {
            _queueCampaignAnim(() => _playMinefieldAnimation(a.fromX, a.fromY, a.toX, a.toY));
        }
        await _waitCampaignAnimations();
    }

    // --- FASE 1b: ANIMAZIONE ARTIGLIERIA ---
    const artilleryStrikes = (campaignState._roundLog || []).filter(e => e.type === 'ARTILLERY');
    if (artilleryStrikes.length > 0) {
        const artAnims = artilleryStrikes.map(entry => {
            const target = campaignState.sectors[entry.sid];
            const src = campaignState.sectors[entry.fromSectorId]
                     || campaignState.sectors.find(s => s.owner === entry.p);
            return {
                fromX: src ? src.x : 500, fromY: src ? src.y : 375,
                toX: target.x, toY: target.y,
            };
        });

        if (window.isCampaignOnline && window.isHost && typeof _net_broadcast === 'function') {
            _net_broadcast({ type: 'CAMPAIGN_ARTILLERY_ANIMATION', strikes: artAnims });
        }

        for (const a of artAnims) {
            _queueCampaignAnim(() => _playArtilleryAnimation(a.fromX, a.fromY, a.toX, a.toY));
        }
        await _waitCampaignAnimations();
    }

    if (nukeStrikes.length > 0) {
        // Manda l'animazione ai client prima di animare sull'host
        if (window.isCampaignOnline && window.isHost && typeof _net_broadcast === 'function') {
            _net_broadcast({ type: 'CAMPAIGN_NUKE_ANIMATION', strikes: nukeStrikes });
        }
        // Accoda e aspetta le animazioni sull'host
        for (const strike of nukeStrikes) {
            _queueCampaignAnim(() => _playNukeAnimation(strike.fromX, strike.fromY, strike.toX, strike.toY));
        }
        await _waitCampaignAnimations();
        // Applica la logica di stato solo dopo l'animazione
        for (const strike of nukeStrikes) {
            _applyNuclearize(strike.sectorId, strike.p, 0);
        }
    }

    // --- FASE 2 e 3: PREPARAZIONE E RISOLUZIONE (invariate) ---
    campaignState.battleQueue = [];
    const orders = campaignState.pendingOrders || {};
    const sectorMap = {};
    campaignState.sectors.forEach(s => { sectorMap[s.id] = { attackers: [], defender: s.owner }; });

    for (let p = 1; p <= campaignState.numPlayers; p++) {
        (orders[p] || []).forEach(o => {
            if (o.isSabotage) return; 
            // FIX: l'ordine isInfiltrati è fittizio (serve solo per l'animazione in
            // processConflicts) e non deve essere trattato come un attacco a crediti,
            // altrimenti genera una battaglia duplicata con crediti undefined (mostra "?"
            // nel riepilogo) e senza unitIdsByOwner, per cui l'unità sconfitta non viene
            // mai rimossa dalla mappa. L'infiltrazione è già gestita da
            // pendingUnitMoves/_resolveUnitConflicts più sotto.
            if (o.isInfiltrati) return;
            if (sectorMap[o.sectorId]) sectorMap[o.sectorId].attackers.push({ p, credits: o.credits });
        });
    }

    campaignState.sectors.forEach(sector => {
        if (sector.blocked) {
            const { attackers } = sectorMap[sector.id];
            if (attackers.length > 0) {
                console.log(`Attacchi annullati su settore nuclearizzato (${sector.id}). Crediti bruciati.`);
            }
            return; 
        }

        const { attackers } = sectorMap[sector.id];
        if (attackers.length === 0) return;
        
        const participants = new Set(attackers.map(a => a.p));
        if (sector.owner > 0 && !participants.has(sector.owner)) participants.add(sector.owner);

        const bCredits = {};
        attackers.forEach(a => { bCredits[a.p] = a.credits; });
        
        if (sector.owner > 0) {
            let defCr = campaignState.sectorCredits[sector.id]?.[sector.owner] || 0;
            if (sector.fortressUpgrade) defCr += CAMPAIGN.UPGRADE_FORTRESS_COST / 2;
            bCredits[sector.owner] = defCr;
        }

        if (sector.mineField && sector.owner > 0 && attackers.length > 0) {
            sector.mineField = false;
            sector._mineFieldJustTriggered = true;
            attackers.length = 0; 
            return;
        }

        const defenderCanFight = sector.owner === 0 ||
            (bCredits[sector.owner] || 0) >= GAME.AGENT_COST;

        if (participants.size > 1 && defenderCanFight) {
            campaignState.battleQueue.push({ sectorId: sector.id, factions: Array.from(participants), battleCredits: bCredits });
        } else {
            const winner = attackers[0].p;
            sector.owner = winner;
            if (!campaignState.sectorCredits[sector.id]) campaignState.sectorCredits[sector.id] = {};
            campaignState.sectorCredits[sector.id][winner] = (campaignState.sectorCredits[sector.id][winner] || 0) + (attackers[0].credits || 0);
        }
    });

    // --- UNITÀ PERSISTENTI: applica tutti gli spostamenti in sospeso, poi risolve i conflitti ---
    _applyPendingUnitMoves();
    _resolveUnitConflicts();

    // --- RIEPILOGO: appare solo dopo tutte le animazioni ---
    if (window.isCampaignOnline && window.isHost) {
        if (typeof _net_broadcast === 'function') {
            _net_broadcast({ 
                type: 'CAMPAIGN_CONFLICT_SUMMARY', 
                campaignSnap: typeof _net_buildSnapshot === 'function' ? _net_buildSnapshot() : null 
            });
        }
        _showConflictSummary();
    } else {
        _showConflictSummary();
    }
}
window.processConflicts = processConflicts;

function _showConflictSummary() {
    const n = campaignState.numPlayers;
    const battles = campaignState.battleQueue;
    const peaceful = [];
    const minefieldsTriggered = [];
    const nuclearStrikes = []; // Registro sabotaggi
    const bonificheRound = [];
    const artilleryStrikes = (campaignState._roundLog || []).filter(e => e.type === 'ARTILLERY');
    const revoltOccurred = campaignState._roundLog?.some(e => e.type === 'REVOLT');

    campaignState.sectors.forEach(s => {
        for (let p = 1; p <= n; p++) {
            (campaignState.pendingOrders[p] || []).forEach(o => {
                // Se è un attacco normale ed è finito in conquista pacifica
                if (o.sectorId === s.id && s.owner === p && !o.isSabotage && !battles.some(b => b.sectorId === s.id))
                    peaceful.push({ p, sid: s.id });
                
                // Se è un sabotaggio nucleare (viene risolto in Fase 1 di processConflicts)
                if (o.sectorId === s.id && o.isSabotage)
                    nuclearStrikes.push({ p, sid: s.id });
            });
        }
        if (!s.mineField && s._mineFieldJustTriggered) {
            minefieldsTriggered.push(s.id);
            delete s._mineFieldJustTriggered;
        }
    });

    // Conquiste pacifiche ottenute muovendo unità (senza scontro): stessa sezione del riepilogo
    (campaignState._roundLog || [])
        .filter(e => e.type === 'UNIT_CONQUEST')
        .forEach(e => peaceful.push({ p: e.p, sid: e.sid }));

    // Raccoglie le bonifiche e pulisce il segreto
    campaignState.sectors.forEach(s => {
        if (s._bonifiedThisRoundBy) {
            bonificheRound.push({ p: s._bonifiedThisRoundBy, sid: s.id });
            delete s._bonifiedThisRoundBy; // Tolto il segreto, ora è libero per tutti
        }
    });

    let html = `<div style="font-family:Courier New;color:#fff;padding:40px;background:rgba(10,15,25,0.95);
                    border:2px solid #555;border-radius:12px;max-width:900px;text-align:left;width:90%;box-shadow:0 0 40px rgba(0,0,0,0.8);
                    margin:auto;">
        <h1 style="color:#fff;text-align:center;margin-top:0;font-size:36px;letter-spacing:2px;border-bottom:1px solid #444;padding-bottom:15px;">RIEPILOGO ORDINI</h1>`;

    // AGGIUNTA: Sezione Sabotaggi e Artiglieria
    if (nuclearStrikes.length > 0 || artilleryStrikes.length > 0) {
        html += `<p style="color:#ff4444;font-size:22px;margin-top:20px;margin-bottom:12px;font-weight:bold;text-transform:uppercase;">☢️ ATTACCHI SPECIALI:</p>`;
        
        nuclearStrikes.forEach(hit => {
            const c = players[hit.p]?.color || COLORS['p' + hit.p];
            html += `<div style="color:${c};font-size:22px;margin-bottom:8px;padding-left:15px;border-left:4px solid #ff4444;">
                ☢️ ${players[hit.p]?.name || 'P'+hit.p} ha NUCLEARIZZATO il Settore ${hit.sid}</div>`;
        });

        artilleryStrikes.forEach(hit => {
            const c = players[hit.p]?.color || COLORS['p' + hit.p];
            const blocked = campaignState.sectors[hit.sid]?.fortressUpgrade;
            html += `<div style="color:${c};font-size:22px;margin-bottom:8px;padding-left:15px;border-left:4px solid ${blocked ? '#ff8800' : '#00ff88'};">
                🎯 ${players[hit.p]?.name || 'P'+hit.p} ha colpito con l'ARTIGLIERIA il Settore ${hit.sid}${blocked ? ' — <span style="color:#ff8800">🏰 BLOCCATO dalla Fortezza!</span>' : ''}</div>`;
        });
    }

    if (revoltOccurred) {
        html += `<p style="color:#ff8800;font-size:22px;margin-top:20px;margin-bottom:12px;font-weight:bold;text-transform:uppercase;">✊ RIVOLTA POPOLARE:</p>
        <div style="font-size:20px;margin-bottom:8px;padding:10px;border-left:4px solid #ff8800;background:rgba(255,136,0,0.08);border-radius:0 6px 6px 0;">
            Tutti i settori con meno di ${CAMPAIGN.UPGRADE_REVOLT_THRESHOLD} crediti sono tornati neutrali.</div>`;
    }

    if (peaceful.length > 0) {
        html += `<p style="color:#aaa;font-size:22px;margin-top:20px;margin-bottom:12px;font-weight:bold;text-transform:uppercase;">CONQUISTE PACIFICHE:</p>`;
        peaceful.forEach(({ p, sid }) => {
            const c = players[p]?.color || COLORS['p' + p];
            html += `<div style="color:${c};font-size:24px;margin-bottom:8px;padding-left:15px;border-left:4px solid ${c};">→ ${players[p]?.name || 'P'+p} conquista il Settore ${sid}</div>`;
        });
    }

    if (minefieldsTriggered.length > 0) {
        html += `<p style="color:#ff8800;font-size:22px;margin-top:30px;margin-bottom:12px;font-weight:bold;text-transform:uppercase;">🛸 DRONI-MINA ATTIVATI:</p>`;
        minefieldsTriggered.forEach(sid => {
            html += `<div style="font-size:22px;margin-bottom:10px;background:rgba(255,136,0,0.1);padding:10px;border-radius:8px;border-left:4px solid #ff8800;">
                💥 Settore ${sid}: gli attaccanti sono stati eliminati dalle mine!</div>`;
        });
    }

    if (battles.length > 0) {
        html += `<p style="color:#ff4444;font-size:22px;margin-top:30px;margin-bottom:12px;font-weight:bold;text-transform:uppercase;">⚔️ BATTAGLIE IMMINENTI:</p>`;
        battles.forEach(b => {
            const names = b.factions.map(pid => {
                const c = players[pid]?.color || COLORS['p' + pid];
                const cr = b.battleCredits[pid] ?? '?';
                return `<span style="color:${c}">${players[pid]?.name || 'P'+pid} (💰${cr})</span>`;
            }).join(' vs ');
            html += `<div style="font-size:24px;margin-bottom:12px;background:rgba(255,50,50,0.1);padding:10px;border-radius:8px;">
                <strong>Settore ${b.sectorId}:</strong><br>${names}</div>`;
        });
    }
    
    if (!peaceful.length && !battles.length && !minefieldsTriggered.length && !nuclearStrikes.length && !artilleryStrikes.length)
        html += `<p style="color:#888;text-align:center;font-size:24px;margin:30px 0;">Nessun movimento questo turno.</p>`;

    const btnAction = (window.isCampaignOnline && !window.isHost)
    ? `if(typeof playSFX==='function') playSFX('click'); document.querySelectorAll('.campaign-summary-overlay').forEach(el=>el.remove()); if(typeof _net_clientSend==='function') _net_clientSend('SYNC_REQUEST',{}); if(typeof _prepareMapDOM==='function') _prepareMapDOM(); renderCampaignMap();`
    : `if(typeof playSFX==='function') playSFX('click'); document.querySelectorAll('.campaign-summary-overlay').forEach(el=>el.remove()); runNextBattle();`;

    html += `<button class="action-btn"
        style="width:100%;margin-top:40px;border-color:#00ff88;color:#00ff88;font-size:28px;padding:15px;font-weight:bold;"
        onclick="${btnAction}">
        AVANTI ▶
    </button></div>`;

    const overlay = document.createElement('div');
    overlay.className = 'campaign-summary-overlay';
    overlay.style.cssText = `position:fixed;top:0;left:0;width:100%;height:100%;
        background:rgba(0,0,0,0.92);z-index:999999;display:flex;align-items:flex-start;justify-content:center;
        overflow-y:auto;padding:20px 0;box-sizing:border-box;`;
    overlay.innerHTML = html;
    document.body.appendChild(overlay);
}

// ============================================================
// RENDITA
// ============================================================

function _collectIncome() {
    const n = campaignState.numPlayers;
    const earned = {};
    for (let p = 1; p <= n; p++) earned[p] = 0;
    campaignState.sectors.forEach(s => {
    if (s.owner > 0 && s.owner <= n) {
        const inc = s.income ?? 1;   // fallback a 1 se undefined/0
        campaignState.credits[s.owner] = (campaignState.credits[s.owner] || 0) + inc;
        earned[s.owner] += inc;
    }
});
    return earned;
}

/**
 * Applica il mantenimento delle unità persistenti a inizio round:
 * ogni unità viva costa CAMPAIGN.UNIT_UPKEEP crediti. Se i crediti in cassa
 * non bastano, la parte scoperta erode il valore delle unità più deboli
 * per prime; un'unità erosa a 0 si disgrega e viene rimossa dalla mappa.
 */
function _applyUnitUpkeep() {
    for (let p = 1; p <= campaignState.numPlayers; p++) {
        const myUnits = campaignState.units.filter(u => u.owner === p);
        if (myUnits.length === 0) continue;

        const upkeepCost = myUnits.length * CAMPAIGN.UNIT_UPKEEP;
        const available   = campaignState.credits[p] || 0;

        if (available >= upkeepCost) {
            campaignState.credits[p] = available - upkeepCost;
            continue;
        }

        campaignState.credits[p] = 0;
        let shortfall = upkeepCost - available;

        myUnits.sort((a, b) => a.value - b.value); // erode prima le unità più deboli
        for (const u of myUnits) {
            if (shortfall <= 0) break;
            const erosion = Math.min(u.value, shortfall);
            u.value  -= erosion;
            shortfall -= erosion;
        }
    }

    // Rimuove le unità disgregate (valore azzerato dall'erosione)
    campaignState.units = campaignState.units.filter(u => u.value > 0);
}
window._applyUnitUpkeep = _applyUnitUpkeep;

// ============================================================
// PROSSIMO ROUND DI PIANIFICAZIONE
// ============================================================

function startNextPlanningRound() {
    // Il client non guida mai questo flusso: aspetta lo snapshot dall'host
    if (window.isCampaignOnline && !window.isHost) {
        _net_clientSend('SYNC_REQUEST', {});
        return;
    }

    campaignState.pendingOrders = {};
    _collectIncome();
    _applyUnitUpkeep();

    // Procede direttamente al prossimo round (locale e online).
    // In multiplayer l'host chiama _doStartNextPlanningRound che fa broadcast
    // dello snapshot aggiornato; i client si sincronizzano via CAMPAIGN_STATE_SYNC
    // senza schermata di rendita intermedia, eliminando la race condition.
    _doStartNextPlanningRound();
}
window.startNextPlanningRound = startNextPlanningRound;

function _doStartNextPlanningRound() {
    // Ripristina i settori nuclearizzati usando il cooldown
    campaignState.sectors.forEach(s => {
        if (s._nuclearCooldown !== undefined && s._nuclearCooldown > 0) {
            s._nuclearCooldown--;
            // Si sblocca solo quando arriva a 0
            if (s._nuclearCooldown === 0) {
                s.blocked      = false;
                s.owner        = 0;
                s.income       = 1;
                if (!campaignState.sectorCredits[s.id]) campaignState.sectorCredits[s.id] = {};
            }
        }
    });
    
    campaignState.turnCount = (campaignState.turnCount || 1) + 1;
    campaignState.phase        = 'PLANNING';
    campaignState.currentPlayer = 1;
    campaignState._roundLog = [];
    campaignState.pendingMoves  = {};
    campaignState._allOrderedSectors = {};
    campaignState.units.forEach(u => { u.hasActedThisTurn = false; }); // Nuovo round: tutte le unità possono agire di nuovo

    // Ripristina cariche artiglieria calcolando quanti edifici possiede ciascuno
    campaignState.artilleryCharges = {};
    for(let p = 1; p <= campaignState.numPlayers; p++) {
        const bonuses = getPlayerCampaignBonuses(p);
        campaignState.artilleryCharges[p] = bonuses.artilleryCount;
    }

    // --- NUOVO: SNAPSHOT INIZIO ROUND ---
    campaignState._creditsAtRoundStart = {};
    for (let p = 1; p <= campaignState.numPlayers; p++) {
        campaignState._creditsAtRoundStart[p] = campaignState.credits[p] || 0;
    }
    campaignState._sectorCreditsAtRoundStart = JSON.parse(JSON.stringify(campaignState.sectorCredits || {}));
    campaignState._sectorsAtRoundStart = {};
    campaignState.sectors.forEach(s => {
        let snapObj = {};
        snapObj.income = s.income;
        if (window.CAMPAIGN_UPGRADE_KEYS) {
            // Se esiste il dizionario dinamico, copiali tutti in automatico
            window.CAMPAIGN_UPGRADE_KEYS.forEach(k => {
                snapObj[k] = s[k] || false;
            });
        } else {
            // Fallback (non dovrebbe mai accadere, ma salva dai crash)
            snapObj.mineUpgrade     = s.mineUpgrade;
            snapObj.mineField       = s.mineField;
            snapObj.fortressUpgrade = s.fortressUpgrade;
        }
        campaignState._sectorsAtRoundStart[s.id] = snapObj;
    });
    // ------------------------------------

    while (campaignState.currentPlayer <= campaignState.numPlayers &&
           !campaignState.sectors.some(s => s.owner === campaignState.currentPlayer)) {
        campaignState.currentPlayer++;
    }
    if (checkCampaignWin()) return;

    saveCampaignSnapshot();
    
    if (window.isCampaignOnline && window.isHost) {
        if (typeof _net_hostBroadcast === 'function') _net_hostBroadcast();
    }
    
    renderCampaignMap();
}
window._doStartNextPlanningRound = _doStartNextPlanningRound;

// ============================================================
// GESTIONE BATTAGLIE
// ============================================================

function runNextBattle() {
    // Se un client preme "Avanti" sul riepilogo, chiede l'aggiornamento all'host e torna in attesa sulla mappa
    if (window.isCampaignOnline && !window.isHost) {
        if (typeof _net_clientSend === 'function') _net_clientSend('SYNC_REQUEST', {});
        if (typeof _prepareMapDOM === 'function') _prepareMapDOM();
        renderCampaignMap();
        return;
    }

    if (campaignState.battleQueue.length === 0) {
        startNextPlanningRound(); 
        return;
    }

    // Se siamo online e siamo l'host, usiamo la funzione di rete dedicata
    if (window.isCampaignOnline && window.isHost) {
        if (typeof _hostRunNextBattle === 'function') {
            _hostRunNextBattle(); 
            return;
        }
    }

    // Logica Locale (Hotseat o Fallback)
    const battle = campaignState.battleQueue.shift();
    campaignState._currentBattle = battle;
    startCampaignBattle(battle.factions, battle.sectorId);
}
window.runNextBattle = runNextBattle;

function startCampaignBattle(factions, sectorId) {
    // Segna chi riceve la rendita omaggio iniziale (deve essere bloccata al primo turno)
    campaignState._hasReceivedFirstIncome = {};
    factions.forEach(f => { campaignState._hasReceivedFirstIncome[f] = false; });

    // Azzera sectorCredits difensore (sono entrati in battaglia)
    const battle = campaignState._currentBattle;
    if (battle?.battleCredits && sectorId != null) {
        if (!campaignState.sectorCredits[sectorId]) campaignState.sectorCredits[sectorId] = {};
        factions.forEach(p => {
            if (campaignState.sectors[sectorId]?.owner === p)
                campaignState.sectorCredits[sectorId][p] = 0;
            (battle.fortressAdjacentZeroed || []).forEach(adjId => {
                if (campaignState.sectorCredits[adjId]) campaignState.sectorCredits[adjId][p] = 0;
            });
        });
    }

    campaignState.targetSector              = sectorId;
    campaignState.currentBattleParticipants = factions.slice().sort((a, b) => a - b);

    totalPlayers = campaignState.numPlayers; // FIX: Usa il numero reale di giocatori della campagna
    fullResetForBattle();
    turnCount = 0;

    for (let p = 1; p <= totalPlayers; p++) {
        if (!players[p]) continue;
        if (factions.includes(p)) {
            const bc = battle?.battleCredits?.[p];
            // Ora usa la costante se i crediti del giocatore non sono definiti
            players[p].credits = bc !== undefined ? bc : (campaignState.credits[p] ?? CAMPAIGN.STARTING_CREDITS);
        }
    }

    // Allinea credits dal battle
    if (battle?.battleCredits) {
        factions.forEach(p => {
            if (battle.battleCredits[p] !== undefined) players[p].credits = battle.battleCredits[p];
        });
    }

    if (timerUI) timerUI.style.display = 'none';
    document.getElementById('campaign-overlay').style.display = 'none';
    state = 'SETUP_P1';
    document.getElementById('setup-overlay').style.display = 'flex';
    currentPlayer = campaignState.currentBattleParticipants[0];
    setupData = freshSetupData();
    if (battle?.battleCredits?.[currentPlayer] !== undefined)
        setupData.points = battle.battleCredits[currentPlayer];
    updateSetupUI();
}
window.startCampaignBattle = startCampaignBattle;

// ============================================================
// OVERRIDE freshSetupData — usa battleCredits specifici
// ============================================================

const _core_origFreshSetupData = window.freshSetupData;
window.freshSetupData = function() {
    const data = _core_origFreshSetupData ? _core_origFreshSetupData() : { points: GAME.SETUP_POINTS, agents: [] };
    if (!campaignState.isActive) return data;
    const battle = campaignState._currentBattle;
    if (battle?.battleCredits?.[currentPlayer] !== undefined) {
        data.points = battle.battleCredits[currentPlayer];
        players[currentPlayer].credits = battle.battleCredits[currentPlayer];
    } else {
        data.points = campaignState.credits[currentPlayer] ?? 10;
    }
    return data;
};

// ============================================================
// OVERRIDE resetTurnState — blocca rendita al primo turno battaglia
// ============================================================

const _core_origResetTurnState = window.resetTurnState;
window.resetTurnState = function() {
    if (campaignState.isActive) {
        const saved = players[currentPlayer]?.credits ?? 0;
        _core_origResetTurnState();
        if (campaignState._hasReceivedFirstIncome && !campaignState._hasReceivedFirstIncome[currentPlayer]) {
            players[currentPlayer].credits = saved;
            campaignState._hasReceivedFirstIncome[currentPlayer] = true;
            if (typeof updateUI === 'function') updateUI();
        }
    } else {
        _core_origResetTurnState();
    }
};

// ============================================================
// OVERRIDE confirmPlayerSetup — flusso campagna
// ============================================================

const _core_origConfirmPlayerSetup = window.confirmPlayerSetup;
window.confirmPlayerSetup = function() {
    if (!campaignState.isActive) {
        if (_core_origConfirmPlayerSetup) _core_origConfirmPlayerSetup();
        return;
    }

    if (!setupData.agents || setupData.agents.length === 0) {
        showTemporaryMessage('ERRORE: Devi reclutare almeno un agente per scendere in campo!');
        playSFX('click'); return;
    }

    players[currentPlayer].agents    = JSON.parse(JSON.stringify(setupData.agents));
    players[currentPlayer].credits   = setupData.points;
    players[currentPlayer].cards     = typeof getFinalCardSelection === 'function' ? getFinalCardSelection() : [];
    players[currentPlayer].usedCards = {};

    // Bonus specializzazioni settore e UPGRADES LABS
    const bonuses = getPlayerCampaignBonuses(currentPlayer);
    
    players[currentPlayer].agents.forEach(agent => {
        // Applica Lab Upgrades
        if (bonuses.hp > 0) { agent.maxHp += bonuses.hp; agent.hp += bonuses.hp; }
        if (bonuses.mov > 0) agent.mov += bonuses.mov;
        if (bonuses.rng > 0) agent.rng += bonuses.rng;
        if (bonuses.dmg > 0) agent.dmg += bonuses.dmg;
    });


    // In campagna online: usa il flusso di rete (gestito in campaign_multiplayer.js)
    if (window.isCampaignOnline) {
        _net_handleConfirmSetup();
        return;
    }

    // Locale: avanza al prossimo partecipante o avvia la partita
    const participants = campaignState.currentBattleParticipants;
    const idx = participants.indexOf(currentPlayer);
    if (idx + 1 < participants.length) {
        currentPlayer = participants[idx + 1];
        setupData = freshSetupData();
        if (typeof cardSelectionData !== 'undefined') cardSelectionData.selected = [];
        updateSetupUI();
    } else {
        startActiveGameLocal();
        drawGame();
    }
};

// ============================================================
// OVERRIDE showGameOverlay — reindirizza a showBattleResults
// ============================================================

const _core_origShowGameOverlay = window.showGameOverlay;
window.showGameOverlay = function(title, message, color) {
    if (!campaignState.isActive) {
        if (_core_origShowGameOverlay) _core_origShowGameOverlay(title, message, color);
        return;
    }
    let winnerFaction = campaignState.currentBattleParticipants[0];
    for (let p = 1; p <= 4; p++) {
        if (players[p] && players[p].color === color) { winnerFaction = p; break; }
    }
    setTimeout(() => showBattleResults(winnerFaction), 3000);
};

// ============================================================
// RISULTATI BATTAGLIA
// ============================================================

function showBattleResults(winnerFaction) {
    const participants = campaignState.currentBattleParticipants;
    const sectorId     = campaignState.targetSector;

    const results = {};
    participants.forEach(faction => {
        // Recupera quanto è rimasto nel negozio tattico
        const shopResidual  = Math.max(0, players[faction]?.credits || 0);
        // Calcola il rimborso per gli agenti sopravvissuti (4 cr l'uno)
        const survivorValue = _agentSurvivorValue(faction);
        
        results[faction] = { 
            shopResidual, 
            survivorValue, 
            total: shopResidual + survivorValue 
        };
    });

    const sector = campaignState.sectors.find(s => s.id === sectorId);
    sector.owner = winnerFaction;

    // --- SCONTRO TRA UNITÀ PERSISTENTI: sostituisce i vecchi token Agente con quello risultante ---
    const unitIdsByOwner = campaignState._currentBattle?.unitIdsByOwner;
    if (unitIdsByOwner) {
        participants.forEach(faction => {
            (unitIdsByOwner[faction] || []).forEach(uid => removeCampaignUnit(uid));
            if (faction === winnerFaction) {
                createCampaignUnit(sectorId, faction, results[faction].total, 'agent');
            }
        });
    }

    // --- ARTIGLIERIA: non conta nel combattimento, viene catturata da chi vince il settore ---
    const artilleryIds = campaignState._currentBattle?.artilleryIds;
    if (artilleryIds && artilleryIds.length > 0) {
        artilleryIds.forEach(uid => {
            const u = campaignState.units.find(x => x.id === uid);
            if (u) {
                u.owner = winnerFaction;      // se era già sua, nessun cambiamento
                u.hasActedThisTurn = true;    // evita azioni immediate dopo la cattura
            }
        });
    }

    // --- APPLICAZIONE ECONOMICA POST-CONFLITTO ---
    participants.forEach(faction => {
        const { total } = results[faction];

        if (faction === winnerFaction) {
            // IL VINCITORE: Il suo tesoro residuo diventa il NUOVO capitale del settore.
            // Puliamo il settore da vecchi residui e impostiamo il valore assoluto.
            campaignState.sectorCredits[sectorId] = { 
                [winnerFaction]: total 
            };
            console.log(`[Campaign] Settore ${sectorId} stabilizzato. Nuovo tesoro: ${total}`);
        } else {
            // I PERDENTI: I loro superstiti e i soldi rimasti nel negozio tornano all'HQ.
            // Usiamo il Proxy: questo aggiornerà automaticamente il settore base del giocatore.
            campaignState.credits[faction] = (campaignState.credits[faction] || 0) + total;
            
            // Rimuoviamo la loro presenza economica da questo settore specifico
            if (campaignState.sectorCredits[sectorId]) {
                delete campaignState.sectorCredits[sectorId][faction];
            }
            console.log(`[Campaign] P${faction} sconfitto. ${total} cr riportati alla base.`);
        }
    });

    // Sincronizzazione Multiplayer (solo Host)
    if (window.isCampaignOnline) {
        if (window.isHost) {
            _net_broadcast({
                type:         'CAMPAIGN_BATTLE_RESULT',
                winnerFaction,
                sectorId,
                results,
                campaignSnap: _net_buildSnapshot(),
            });
            _showBattleResultsUI(winnerFaction, sectorId, results);
        } else {
            const go = document.getElementById('gameover-overlay');
            if (go) go.style.display = 'none';
        }
        return;
    }

    _showBattleResultsUI(winnerFaction, sectorId, results);
}
window.showBattleResults = showBattleResults;

function _showBattleResultsUI(winnerFaction, sectorId, results) {
    if (typeof playSFX === 'function') playSFX('intro');
    const participants = campaignState.currentBattleParticipants;
    const n            = campaignState.numPlayers;
    const winnerColor  = players[winnerFaction]?.color || COLORS['p' + winnerFaction];
    const winnerName   = players[winnerFaction]?.name || 'P' + winnerFaction;

    const creditsHtml = participants.map(faction => {
        const c    = players[faction]?.color || COLORS['p' + faction];
        const name = players[faction]?.name || 'P' + faction;
        const r    = results[faction];
        const isW  = faction === winnerFaction;
        const dest = isW
            ? `→ 📦 Nel Settore: <b>${campaignState.sectorCredits[sectorId]?.[faction] || 0}</b>`
            : `→ 🏦 Alla Banca: <b>+${r.total}</b>`;
        return `<div style="color:${c}; font-size:15px; margin:10px 0; border-left:3px solid ${c}; padding:5px 12px; background:rgba(255,255,255,0.03); border-radius:0 5px 5px 0;">
            <div style="font-weight:bold; font-size:18px;">${name} ${isW ? '🏆' : '💀'}</div>
            <div style="color:#aaa; font-size:13px; margin:4px 0;">Negozio: ${r.shopResidual} + Agenti vivi: ${r.survivorValue} = <b>${r.total}</b></div>
            <div style="font-size:14px; border-top:1px solid rgba(255,255,255,0.05); padding-top:4px;">${dest}</div>
        </div>`;
    }).join('');

    const ownedHtml = Array.from({ length: n }, (_, i) => i + 1).map(p => {
        const cnt = campaignState.sectors.filter(s => s.owner === p).length;
        const c   = players[p]?.color || COLORS['p' + p];
        return `<span style="color:${c}; margin:4px 10px; font-weight:bold; font-size:14px; white-space:nowrap;">${players[p]?.name || 'P'+p}: 🏴${cnt}</span>`;
    }).join('');

    const overlay = document.createElement('div');
    overlay.id = 'battle-results-overlay';
    overlay.style.cssText = `position:fixed; top:0; left:0; width:100%; height:100%;
        background:rgba(0,0,0,0.94); z-index:2000000;
        display:flex; align-items:center; justify-content:center;
        font-family:Courier New,monospace; padding:10px; box-sizing:border-box;`;

    overlay.innerHTML = `
        <div style="background:rgba(10,15,30,0.98); border:3px solid ${winnerColor}; border-radius:15px;
                    padding:20px; width:100%; max-width:580px; max-height:95vh; overflow-y:auto; 
                    box-shadow:0 0 50px rgba(0,0,0,1); box-sizing:border-box; text-align:center;">
            
            <h1 style="color:${winnerColor}; text-shadow:0 0 15px ${winnerColor}; margin:0 0 10px; font-size:clamp(22px, 5vw, 32px); text-transform:uppercase;">⚔️ BATTAGLIA CONCLUSA</h1>
            <h2 style="color:#fff; margin:0 0 20px; font-size:clamp(16px, 4vw, 22px);">VINCITORE: <span style="color:${winnerColor}">${winnerName.toUpperCase()}</span></h2>
            
            <div style="background:rgba(0,0,0,0.3); border:1px solid #333; border-radius:10px; padding:10px; margin-bottom:20px; text-align:left;">
                <p style="color:#888; font-size:11px; margin:0 0 10px; text-transform:uppercase; text-align:center; letter-spacing:1px;">Resoconto Crediti</p>
                ${creditsHtml}
            </div>

            <div style="background:rgba(255,255,255,0.03); border:1px solid #222; border-radius:8px; padding:10px; margin-bottom:25px; display:flex; flex-wrap:wrap; justify-content:center;">
                ${ownedHtml}
            </div>

            <button class="action-btn" id="results-continue-btn"
                style="padding:15px; border:3px solid ${winnerColor}; color:${winnerColor}; background:rgba(0,0,0,0.5); 
                       cursor:pointer; font-size:22px; font-weight:bold; width:100%; border-radius:10px; text-transform:uppercase;">
                AVANTI ▶
            </button>
        </div>`;
    document.body.appendChild(overlay);

    overlay.querySelector('#results-continue-btn').onclick = () => {
        if (typeof playSFX === 'function') playSFX('click');
        overlay.remove();
        if (typeof grid !== 'undefined') grid.clear();
        if (typeof controlPoints !== 'undefined') controlPoints.clear();
        state = 'SETUP_P1';
        document.getElementById('controls-panel').style.display = 'none';
        document.getElementById('setup-overlay').style.display  = 'none';
        if (typeof ctx !== 'undefined') ctx.clearRect(0, 0, canvas.width, canvas.height);
        if (checkCampaignWin()) return;
        runNextBattle();
    };
}
window._showBattleResultsUI = _showBattleResultsUI;

function _agentSurvivorValue(faction) {
    const survivors = (players[faction]?.agents || []).filter(a => a && a.hp > 0);
    const bonuses = getPlayerCampaignBonuses(faction);
    let totalValue = 0;

    survivors.forEach(a => {
        // 2. Legge le stat "nude" sottraendo l'esatto valore aggiunto dai buff temporanei
        //    in modo da non perdere eventuali upgrade permanenti ottenuti in battaglia.
        let baseMaxHp = a.maxHp;
        let baseMov   = a.mov;
        let baseRng   = a.rng;
        let baseDmg   = a.dmg;

        if (a.medikitTempBuff) {
            baseMaxHp -= 1;
            baseMov   -= 1;
            baseRng   -= 1;
            baseDmg   -= 1;
        }
        if (a.sniperBuff) {
            baseRng -= (a.originalRng * (a.pierceCount || 1));
        }
        if (a.demoBuff) {
            baseDmg -= a.originalDmg;
        }

        const paidMaxHp = Math.max(1, baseMaxHp - (bonuses.hp  || 0));
        const paidMov   = Math.max(1, baseMov   - (bonuses.mov || 0));
        const paidRng   = Math.max(1, baseRng   - (bonuses.rng || 0));
        const paidDmg   = Math.max(1, baseDmg   - (bonuses.dmg || 0));

        // 3. Calcoliamo il valore basandoci solo sulla parte "pagata"
        const statsValue = (paidMaxHp - 1) + (paidMov - 1) + (paidRng - 1) + (paidDmg - 1);
        const agentRealCost = GAME.CREDIT_AGENT_BASE + statsValue;
        
        totalValue += agentRealCost;     // intero costo agente
    });
    return totalValue;
}
window._bat_agentSurvivorValue = _agentSurvivorValue;

// ============================================================
// VITTORIA CAMPAGNA
// ============================================================

function checkCampaignWin() {
    let winner = null;
    for (let p = 1; p <= campaignState.numPlayers; p++) {
        if (campaignState.sectors.filter(s => s.owner === p).length >= campaignState.victoryThreshold) {
            winner = p; break;
        }
    }
    if (!winner) return false;

    clearCampaignSave();

    // Online: solo l'host notifica tutti i client
    if (window.isCampaignOnline && window.isHost) {
        if (typeof broadcastToClients === 'function') {
            broadcastToClients({ type: 'CAMPAIGN_WIN', winner });
        }
    }

    _showCampaignWinUI(winner);
    return true;
}
window.checkCampaignWin = checkCampaignWin;

function _showCampaignWinUI(winner) {
    if (typeof playSFX === 'function') playSFX('intro');
    const color = players[winner]?.color || COLORS['p' + winner] || '#ffffff';
    const name  = players[winner]?.name || 'P' + winner;
    const cnt   = campaignState.sectors.filter(s => s.owner === winner).length;

    // DISTRUGGIAMO (non solo nascondiamo) tutte le interfacce popup della campagna
    // per assicurarci che non interferiscano e non coprano la vittoria
    ['controls-panel', 'setup-overlay', 'battle-results-overlay', 'cn-result-overlay', 'eco-credit-modal', 'cn-order-sent-overlay', 'campaign-summary-overlay'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.remove(); 
    });

    // Creiamo un overlay di vittoria dedicato ad altissimo z-index, 
    // slegato dal campaign-overlay in modo che non venga sovrascritto dalla mappa
    let winOverlay = document.getElementById('campaign-win-overlay-final');
    if (!winOverlay) {
        winOverlay = document.createElement('div');
        winOverlay.id = 'campaign-win-overlay-final';
        document.body.appendChild(winOverlay);
    }

    winOverlay.style.cssText = `
        position:fixed; top:0; left:0; width:100%; height:100%;
        background:rgba(0,0,0,0.98); z-index:9999999;
        display:flex; flex-direction:column; align-items:center; justify-content:center;
        font-family:Courier New, monospace; padding: 15px; box-sizing: border-box;
    `;

    // Sostituita la 🏆 statica con un contenitore vuoto id="campaign-win-icon"
    winOverlay.innerHTML = `
        <div style="text-align:center;color:${color};padding:40px 20px;border:4px solid ${color};
                    background:rgba(10,15,25,0.95);border-radius:15px;max-width:550px;width:100%;
                    box-shadow:0 0 60px ${color}88; box-sizing: border-box;">
            
            <div id="campaign-win-icon" style="margin-bottom:15px; display:flex; justify-content:center; align-items:center;"></div>
            
            <h1 style="color:${color};text-shadow:0 0 20px ${color};margin:0 0 10px;font-size:clamp(24px, 6vw, 36px);">DOMINIO GLOBALE</h1>
            <h2 style="color:#fff;margin:0 0 20px;font-size:clamp(20px, 5vw, 28px);">${name.toUpperCase()}</h2>
            <p style="color:#aaa;margin-bottom:35px;font-size:18px;">Ha conquistato ${cnt}/${campaignState.sectors.length} settori — Vittoria Totale!</p>
            <button class="action-btn"
                style="border:3px solid ${color};color:${color};background:rgba(0,0,0,0.5);padding:15px;cursor:pointer;font-size:22px;font-weight:bold;width:100%;border-radius:8px;"
                onclick="location.reload()">TORNA AL MENU</button>
        </div>`;

    // --- INIZIO GESTIONE ICONA (Identica a gamelogic.js) ---
    const iconContainer = winOverlay.querySelector('#campaign-win-icon');
    const hqId = players[winner]?._cosmeticFaction || winner;

    if (hqId) {
        const img = document.createElement('img');
        img.src = `img/HQ${hqId}.png`;
        
        img.style.cssText = `
            height: 16.5em; 
            max-height: 40vh; 
            width: auto; 
            object-fit: contain; 
            filter: drop-shadow(0 0 30px ${color});
            animation: pulse 2s infinite;
        `;
        
        img.onerror = () => {
            iconContainer.innerHTML = '<span style="font-size:8em; animation: pulse 2s infinite;">🏆</span>';
        };
        
        iconContainer.appendChild(img);
    } else {
        iconContainer.innerHTML = '<span style="font-size:8em; animation: pulse 2s infinite;">🏆</span>';
    }
    // --- FINE GESTIONE ICONA ---
}
window._showCampaignWinUI = _showCampaignWinUI;

// ============================================================
// MENU CAMPAGNA E INFO
// ============================================================

// ============================================================
// MENU CAMPAGNA LOCALE
// ============================================================

function attemptStartCampaign(numPlayers) {
    if (typeof playSFX === 'function') playSFX('click');
    const save = typeof getCampaignSave === 'function' ? getCampaignSave() : null;

    // Se esiste un salvataggio, chiede conferma prima di sovrascriverlo
    if (save) {
        showCustomConfirm(
            "⚠️ SOVRASCRIVI SALVATAGGIO", 
            `Una campagna salvata esiste già (Round ${save.turnCount}).<br>Avviare una nuova campagna <b>cancellà definitivamente</b> i progressi. Procedere?`, 
            "NUOVA PARTITA", 
            () => {
                clearCampaignSave();
                startCampaign(numPlayers);
            }, 
            '#ff4444'
        );
    } else {
        // Altrimenti avvia dritto
        startCampaign(numPlayers);
    }
}
window.attemptStartCampaign = attemptStartCampaign;

function showCampaignMenu() {
    if (typeof playSFX === 'function') playSFX('click');
    const menu = document.getElementById('network-menu');
    if (!menu) return;

    // Se il menu è già aperto, evito di crearlo due volte
    const existing = document.getElementById('campaign-num-players');
    if (existing) { existing.remove(); return; }

    const save = typeof getCampaignSave === 'function' ? getCampaignSave() : null;
    if (!save) return; // Sicurezza: se non c'è il save, il tasto non doveva esserci

    const d = new Date(save.savedAt);
    const dateStr = d.toLocaleDateString('it-IT') + ' ' + d.toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' });

    // Creiamo il div che si innesterà nel menu principale
    const div = document.createElement('div');
    div.id = 'campaign-num-players';
    div.style.cssText = `margin-top:20px; text-align:center; border-top:1px solid #333; padding-top:16px; font-family:Courier New; width: 100%; max-width: 450px; margin-left: auto; margin-right: auto; box-sizing: border-box;`;

    // Ripristiniamo l'esatto pannello con il Floppy Disk d'oro
    div.innerHTML = `
        <div style="border:2px solid #FFD700; border-radius:8px; padding:16px; margin-bottom:16px; background:rgba(255,215,0,0.05);">
            <p style="color:#FFD700; font-weight:bold; margin:0 0 6px;">💾 CAMPAGNA IN CORSO</p>
            <p style="color:#aaa; font-size:12px; margin:0 0 12px;">Round ${save.turnCount} — Salvata il ${dateStr}</p>
            <button class="action-btn" id="btn-load-campaign"
                style="border:2px solid #FFD700; color:#FFD700; background:transparent; padding:12px 24px; font-size:15px; cursor:pointer; width:100%;">
                ▶ RIPRENDI CAMPAGNA
            </button>
        </div>
        <button class="action-btn" onclick="document.getElementById('campaign-num-players').remove()"
            style="border:2px solid #555; color:#aaa; background:transparent; padding:10px 20px; font-size:12px; cursor:pointer;">
            ANNULLA
        </button>
    `;

    menu.appendChild(div);

    // Gestione del caricamento
    document.getElementById('btn-load-campaign').onclick = () => {
        if (typeof playSFX === 'function') playSFX('click');
        div.remove();
        loadCampaignSnapshot(save);
    };
}
window.showCampaignMenu = showCampaignMenu;

function showCampaignInfoModal() {
    if (typeof playSFX === 'function') playSFX('click');

    const modal = document.createElement('div');
    modal.id = 'campaign-info-modal';
    modal.style.cssText = `position:fixed;top:0;left:0;width:100%;height:100%;
        background:rgba(0,0,0,0.95);z-index:1000000;display:flex;align-items:center;justify-content:center;font-family:'Courier New',monospace;`;
    
    modal.innerHTML = `
        <div style="background:#050a14;border:2px solid #00ff88;padding:30px;max-width:550px;width:90%;border-radius:12px;box-shadow:0 0 40px rgba(0,255,136,0.3);">
            <h2 style="color:#00ff88;text-align:center;margin-top:0;">INFORMAZIONI</h2>
            ${(typeof isOnline !== 'undefined' && isOnline) ? `
            <div style="text-align:center;margin-bottom:16px;padding:10px;border:1px solid #FFD700;border-radius:6px;background:rgba(255,215,0,0.05);">
                <div style="font-size:11px;color:#888;margin-bottom:4px;">🔗 ID PARTITA</div>
                <div id="camp-info-session-id" style="font-size:18px;color:#FFD700;font-weight:bold;letter-spacing:2px;cursor:pointer;"
                    title="Clicca per copiare"
                    onclick="navigator.clipboard?.writeText(this.dataset.id);this.textContent='✅ copiato!';setTimeout(()=>{this.textContent=this.dataset.id;},1500);"
                    data-id="${(typeof isHost !== 'undefined' && isHost) ? (typeof currentPeerId !== 'undefined' ? currentPeerId : '') : (typeof _storedHostId !== 'undefined' ? _storedHostId : '')}">
                    ${(typeof isHost !== 'undefined' && isHost) ? (typeof currentPeerId !== 'undefined' ? currentPeerId : '') : (typeof _storedHostId !== 'undefined' ? _storedHostId : '')}
                </div>
            </div>` : ''}
            <div style="margin:20px 0;max-height:60vh;overflow-y:auto;padding-right:10px;">
                
                <div style="margin-bottom:20px;border-left:4px solid #FFD700;padding-left:15px;background:rgba(255,215,0,0.05);">
                    <div style="font-size:20px;color:#FFD700;font-weight:bold;">🏆 OBIETTIVO FINALE</div>
                    <div style="font-size:14px;color:#fff;">Conquista <b>${campaignState.victoryThreshold} settori</b> per ottenere il dominio totale, o vince chi possiede più settori all'inizio del turno concordato.</div>
                </div>

                <div style="margin-bottom:20px;border-left:4px solid #ff4444;padding-left:15px;background:rgba(255,68,68,0.05);">
                    <div style="font-size:20px;color:#ff4444;font-weight:bold;">❌ Zone Contaminate</div>
                    <div style="font-size:14px;color:#aaa;">Settori <b>completamente inagibili</b>. Possono essere bonificati spendendo 30 crediti cliccando sul settore.</div>
                </div>

                <div style="margin-bottom:10px;border-left:4px solid #00aaff;padding-left:15px;background:rgba(0,170,255,0.05);">
                    <div style="font-size:20px;color:#00aaff;font-weight:bold;">🛠️ Costruzioni e Ricerca</div>
                    <div style="font-size:14px;color:#aaa;">Clicca su un tuo settore per accedere al menu di costruzione, dove potrai erigere strutture difensive, creare nuovi agenti, armi globali o laboratori per potenziare i tuoi agenti.</div>
                </div>

                <div style="margin-bottom:10px;border-left:4px solid #ff8800;padding-left:15px;background:rgba(255,136,0,0.05);">
                    <div style="font-size:20px;color:#ff8800;font-weight:bold;">👤 Mantenimento Unità</div>
                    <div style="font-size:14px;color:#aaa;">Ogni unità (Agente o Artiglieria) costa <b>💰${CAMPAIGN.UNIT_UPKEEP} crediti a turno</b> di mantenimento. Se non hai crediti sufficienti in cassa, il debito erode il valore delle tue unità più deboli, fino a farle disgregare del tutto.</div>
                </div>
                
            </div>
            <!-- BLOCCO PULSANTI MANUALE + CHIUDI -->
            <div style="display: flex; gap: 10px; width: 100%;">
                <button class="action-btn" style="flex: 1; padding: 15px; border-color: #00aaff; color: #00aaff; background: rgba(0, 170, 255, 0.1); font-size: 14px;"
                    onclick="window.open('manuale.html', '_blank')"><span style="font-size:2em; line-height:1; display:inline-block; margin-top:-0.25em; margin-bottom:-0.25em; position:relative; top:4px;">📖</span> MANUALE</button>
                <button class="action-btn" style="flex: 1; padding: 15px; border-color: #00ff88; color: #00ff88; font-size: 14px;"
                    onclick="document.getElementById('campaign-info-modal').remove()">CHIUDI</button>
            </div>
        </div>`;
    document.body.appendChild(modal);
}
window.showCampaignInfoModal = showCampaignInfoModal;

// ============================================================
// OVERRIDE isPlayerEliminated — salta non-partecipanti in battaglia
// ============================================================

const _core_origIsEliminated = window.isPlayerEliminated;
window.isPlayerEliminated = function(p) {
    if (campaignState.isActive) {
        const parts = campaignState.currentBattleParticipants || [];
        if (parts.length > 0 && !parts.includes(p)) return true;
    }
    return _core_origIsEliminated ? _core_origIsEliminated(p) : false;
};

// ============================================================
// OVERRIDE resetTurnState — pulizia HQ fantasma in campagna
// ============================================================

const _core_origResetForHQ = window.resetTurnState;
window.resetTurnState = (function() {
    // Catturiamo l'ultimo override (quello che blocca la rendita)
    // e ci aggiungiamo la pulizia HQ
    const prev = window.resetTurnState;
    return function() {
        if (campaignState.isActive) {
            const parts = campaignState.currentBattleParticipants || [];
            if (parts.length > 0) {
                grid.forEach(cell => {
                    if (cell.entity?.type === 'hq' && !parts.includes(cell.entity.faction))
                        cell.entity = null;
                });
                for (let p = 1; p <= 4; p++) {
                    if (!parts.includes(p) && players[p]) {
                        players[p].hq = null; players[p].agents = [];
                    }
                }
            }
        }
        prev();
    };
})();

// ============================================================
// STUB _net_* — queste funzioni vengono implementate da
// campaign_multiplayer.js quando la campagna è online.
// In modalità locale rimangono no-op.
// ============================================================

window._net_hostBroadcast    = window._net_hostBroadcast    || function() {};
window._net_broadcast        = window._net_broadcast        || function() {};
window._net_clientSend       = window._net_clientSend       || function() {};
window._net_buildSnapshot    = window._net_buildSnapshot    || function() { return {}; };
window._net_applyTurnState   = window._net_applyTurnState   || function() {};
window._net_showOrderSentOverlay = window._net_showOrderSentOverlay || function() {};
window._net_handleConfirmSetup   = window._net_handleConfirmSetup   || function() {};

console.log('[campaign_battle.js] Caricato.');


markScriptAsLoaded('campaign_battle.js');