/* ============================================================
   ai.js — Intelligenza Artificiale Strategica (V21 - Focus Kill)
   ============================================================
...
async function executeAITurn() {
    ...
    while (totalApToSpend > 0 && iterations < MAX_ITER) {
        ...
        for (const va of vAgents) {
            ...
            // Registra il bersaglio come reclamato (solo per tiri su agenti nemici)
...
function _aiTryBlitzPostPlan(aiFaction, vAgents, masterPlan, virtualOccupied, virtualHP, claimedTargets) {
    ...
    for (const va of vAgents) {
        ...
        const extraScore = _scoreAction(vaExtra, extraAct, aiFaction, virtualOccupied, virtualHP);
 ...
 */
function _scoreAction(va, action, faction, virtualOccupied, virtualHP) {
    if (action.type === 'shoot') {
        ...
        if (action.subType === 'hq') {
            ...
            const avgFactionDmg = liveFaction.length > 0
                ...
                : va.ref.dmg;
            if (adjHp <= va.ref.dmg)                                    return 200000;
            if (adjHp <= Math.ceil(avgFactionDmg) + va.ref.dmg)        return 160000;
            return 120000 + Math.floor((va.ref.dmg / adjHp) * 20000);
        } else {
            // ostacolo
            return action.subType === 'barricade' ? 60000 : 45000;
        }
    }
    ...
    if (action.type === 'move') {
        ...
        const newDist     = hexDistance({ q: action.q, r: action.r }, navTarget);
        if (newDist < currentDist) return 90000 + (currentDist - newDist) * 4000;
        if (newDist === currentDist) return 20000;
        return 5000;
    }
...
function _aiEvaluateAndPlayCards(aiFaction) {
    ...
    pData.cards.forEach((cardId, slotIndex) => {
        ...
        switch (cardId) {
            ...
            case 'C02': { // Airdrop — teletrasporto a costo AP
                // Attiva solo se la destinazione e significativamente migliore
                // della posizione attuale: kill immediato o accerchiamento.
                let bestAction = null;
                let bestScore  = 0;
                myAgents.forEach(a => {
                    if (a.ap < 3) return;
...
function _planBestAction(va, faction, virtualOccupied, virtualHP, claimedTargets) {
    ...
    for (const t of combatTargets) {
        ...
        // HP reali del bersaglio (prima del piano virtuale), per calcolare il danno
        // gia subito da altri agenti in questo turno
        const realHp    = t.hp; // t.hp = hp reali al momento della costruzione targets
        const vhpRaw    = virtualHP.get(getKey(t.q, t.r));
        const alreadyDamaged = (vhpRaw !== undefined && vhpRaw < realHp);
    ...
    for (const m of moves) {
        ...
        for (const t of tft) {
            const ehp = virtualHP.get(getKey(t.q, t.r)) ?? t.hp;
            if (t.type !== 'hq') {
                if (ehp <= va.ref.dmg)                                  lookaheadScore = Math.max(lookaheadScore, 85000);
                else if (ehp <= Math.ceil(avgFactionDmg) + va.ref.dmg) lookaheadScore = Math.max(lookaheadScore, 72000);
                else                                                     lookaheadScore = Math.max(lookaheadScore, 55000);
            } else {
                lookaheadScore = Math.max(lookaheadScore, 40000);
            }
        }

        // Lookahead livello 2: se dalla mossa m non sparo, posso muovermi
        // ancora e sparare al prossimo AP? (combo muovi+muovi+spara)
        if (lookaheadScore === 0) {
            const movesFromM = getVirtualMoves(sVA, virtualOccupied);
            for (const m2 of movesFromM) {
                if (va.visited.has(getKey(m2.q, m2.r)) || getKey(m2.q, m2.r) === getKey(m.q, m.r)) continue;
                ...
                for (const t2 of tft2) {
                    ...
                    if (t2.type !== 'hq') {
                        // Valore ridotto: e a 2 AP di distanza
                        if (ehp2 <= va.ref.dmg)                                  lookaheadScore = Math.max(lookaheadScore, 60000);
                        else if (ehp2 <= Math.ceil(avgFactionDmg) + va.ref.dmg) lookaheadScore = Math.max(lookaheadScore, 48000);
                        else                                                      lookaheadScore = Math.max(lookaheadScore, 35000);
                    } else {
    ...
    // D. Movimento laterale (gia coperto dal base score = 20000 in B, qui solo fallback)
...
function getHuntingTarget(va, faction, virtualHP, claimedTargets) {
    ...
    for (let p = 1; p <= totalPlayers; p++) {
        ...
        players[p].agents.forEach(enemy => {
            ...
            let killBonus = 0;
            if (effectiveHp <= va.ref.dmg)              killBonus = 10; // killabile da me: -10 distanza
            else if (effectiveHp <= avgFactionDmg)      killBonus = 7;  // killabile da un alleato
            else if (effectiveHp < enemy.maxHp)         killBonus = Math.min(4, Math.floor((enemy.maxHp - effectiveHp) / enemy.maxHp * 6));

            // Penalita leggera se gia reclamato da un altro agente virtuale
            const claimPen = claimedTargets.has(getKey(enemy.q, enemy.r)) ? 2 : 0;
