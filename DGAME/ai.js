...
async function executeAITurn() {
    ...
    // Se il valutatore Spettro ha identificato una cella di atterraggio
    // ottimale, la passiamo al pianificatore come suggerimento di prima mossa.
    ghostTarget: a._aiSpettroMoveTarget ?? null,
    }));

    // Pulizia: rimuove il flag temporaneo dagli agenti reali
...
function _aiEvaluateAndPlayCards(aiFaction) {
    ...
    pData.cards.forEach((cardId, slotIndex) => {
        ...
        switch (cardId) {
            ...
            case 'C08': // Fortino (4 barricate gratis)
            ...
            case 'C04': // EMP (Paralisi d'area - Raggio 5)
                let bestEMPScore = 0;
            ...
            case 'C09': // Medikit (buffer HP automatico)
            ...
            case 'C06': // Spettro (Attraversa muri / Infiltrazione permanente)
    {
        let bestSpettroScore = 0;

        myAgents.forEach(a => {
            if (a.spectreBuff || a.ap < 1) return; // già fantasma o fermo

            // ── Valutazione tattica concreta ──────────────────────
            // Trova la cella di atterraggio migliore raggiungibile
            // attraverso i muri e misura i bersagli sparabili da lì.
            const ghostSpot = _aiGetGhostShootingSpots(a, aiFaction);
            if (!ghostSpot) return;

            let agentScore = ghostSpot.score;

            // Bonus kill garantito: lo Spettro sblocca un'eliminazione
            if (ghostSpot.killable) agentScore += 10000;

            // Bonus permanenza: lo Spettro dura tutta la partita.
            // Più AP ha l'agente ora, più il buff vale (potrà muoversi
            // E sparare nello stesso turno usando la nuova libertà).
            agentScore += a.ap * 1500;

            // Bonus agente prezioso: più fa danno, più vale tenerlo mobile
            agentScore += a.dmg * 800;

            // Malus se quasi morto: potrebbe non sopravvivere abbastanza
            // per sfruttare il buff nei turni successivi
            if (a.hp <= 1) agentScore -= 8000;

            // Memorizza la cella di atterraggio per usarla nel masterPlan
            a._aiSpettroMoveTarget = ghostSpot.cell;
        ...
        // Soglia bassa: qualsiasi vantaggio tattico concreto basta
        if (bestSpettroScore >= 5000) shouldPlay = true;
    }
    break;

            

            case 'C02': // Airdrop (Teletrasporto Globale - Costo 3 AP)
    ...
    myAgents.forEach(a => {
        ...
        if (dropSpot) {
            ...
            // Bonus: se l'agente ha AP extra (grazie a Blitz),
            // può teletrasportarsi E sparare nello stesso turno
            if (a.ap > 3) score += 500;
...
if (cardId === 'C10') {
    // Upgrade asincrono: l'AI invia direttamente la sua scelta pre-calcolata
    finalizeAsyncCard(slotIndex, cardId, chosenAgent._aiUpgradeChoice);
    delete chosenAgent._aiUpgradeChoice;

} else if (cardId === 'C02') {
    // ── AIRDROP: esecuzione diretta e sincrona ───────────────
    // NON usiamo card.apply() (che chiama setActionMode e aspetta
    // un click) né setTimeout (che scatta dopo che masterPlan è
    // già costruito e validActionTargets è stato svuotato).
    // Eseguiamo il teletrasporto direttamente, replicando
    // esattamente la logica di carduse.js senza passare dalla pipeline.
    const dropTarget = chosenAgent._aiDropTarget;
    delete chosenAgent._aiDropTarget;

    if (dropTarget) {
        const targetCell = grid.get(getKey(dropTarget.q, dropTarget.r));

        // Sicurezza: la cella potrebbe essere stata occupata
        // tra la pianificazione e l'esecuzione (raro ma possibile)
        if (targetCell && targetCell.type === 'empty' && !targetCell.entity) {

            playSFX('move');

            // Sposta fisicamente l'agente sulla griglia
            grid.get(getKey(chosenAgent.q, chosenAgent.r)).entity = null;
            targetCell.entity = chosenAgent;
            chosenAgent.q     = targetCell.q;
            chosenAgent.r     = targetCell.r;

            // Scala AP (la carta costa 3)
            chosenAgent.ap -= 3;

            // Conquista CP se l'agente atterra su uno
            ...
            // Sync multiplayer: invia come ACTION_CARD con destinazione
            // I client riceveranno il messaggio e replicheranno il teletrasporto
            // tramite receiveRemoteCardAction → card.apply() → setActionMode.
            // Per i client l'Airdrop rimane nel flusso normale (umano),
            // quindi non c'è problema: il client non è async.
            if (isOnline) {
                sendOnlineMessage({
                    type:          'ACTION_CARD',
                    cardId:        'C02',
                    ...
                    targetAgentId: chosenAgent.id,
                    aiDropQ:       chosenAgent.q,
                    aiDropR:       chosenAgent.r,
                });
            }
        }
    }
    // Non chiamiamo showCardMessage per non intasare l'UI durante l'AI
...
/**
 * BFS ghost: trova tutte le celle raggiungibili attraverso i muri
 * e per ognuna calcola il valore tattico dei bersagli sparabili.
 * Restituisce la cella di atterraggio migliore con il suo score,
 * o null se lo Spettro non porta vantaggio rispetto al movimento normale.
 *
 * @param {Object} agent  - agente reale (con .q, .r, .mov, .rng, .dmg, .faction)
 * @param {number} faction
 * @returns {{ cell: {q,r}, score: number, killable: boolean } | null}
 */
function _aiGetGhostShootingSpots(agent, faction) {
    ...
    while (queue.length > 0) {
        ...
        if (curr.dist > 0) {
            ...
            // Consideriamo solo celle vuote come destinazione finale
            // (lo Spettro ATTRAVERSA i muri ma non ci si FERMA dentro)
            if (currCell && currCell.type === 'empty' && !currCell.entity) {
                let spotScore = 0;
                let killable  = false;

                // Simula il tiro da questa cella in tutte le direzioni
                const rng = agent.rng + (currCell.terrain === 'altura' ? 1 : 0);

                hexDirections.forEach(dir => {
                    for (let d = 1; d <= rng; d++) {
                        const tq   = curr.q + dir.q * d;
                        const tr   = curr.r + dir.r * d;
                        const tc   = grid.get(getKey(tq, tr));
                        if (!tc) break;
                        ...
                        if (tc.entity) {
                            if (tc.entity.faction !== faction) {
                                const effectiveHp = tc.terrain === 'copertura'
                                    ? tc.entity.hp + 1
                                    : tc.entity.hp;

                                if (tc.entity.type === 'hq') {
                                    spotScore += effectiveHp <= agent.dmg ? 15000 : 6000;
                                } else {
                                    if (effectiveHp <= agent.dmg) {
                                        spotScore += 20000;
                                        killable   = true;
                                    } else {
                                        spotScore += 8000 + Math.floor(
                                            (agent.dmg / effectiveHp) * 5000
                                        );
                                    }
                                }
                            }
                            break; // il tiro si ferma alla prima entità
                        }
                        if (tc.type === 'wall' || tc.type === 'barricade') break;
                    }
                });

                // Valore extra se questa cella è raggiungibile SOLO via Spettro
                // (cioè non è raggiungibile con movimento normale)
                const isGhostOnly = _aiIsGhostOnlyCell(agent, curr.q, curr.r);
                if (isGhostOnly) spotScore += 4000;

                if (spotScore > bestScore) {
                    bestScore = spotScore;
                    bestSpot  = { cell: { q: curr.q, r: curr.r }, score: spotScore, killable };
                }
            }
        }

        // Espandi BFS: lo Spettro attraversa muri ma non entità
        if (curr.dist < agent.mov) {
            hexDirections.forEach(dir => {
                ...
                if (!nc || nc.entity) return; // entità bloccano sempre
                visited.add(key);
...
/**
 * Verifica se una cella è raggiungibile SOLO con il movimento Spettro
 * e non con il BFS normale (usato per il bonus "ghost-only").
 */
function _aiIsGhostOnlyCell(agent, tq, tr) {
    // BFS normale, max mov passi
    const visited = new Set([getKey(agent.q, agent.r)]);
    const queue   = [{ q: agent.q, r: agent.r, dist: 0 }];
    ...
    while (queue.length > 0) {
        const curr = queue.shift();
        if (getKey(curr.q, curr.r) === target) return false; // raggiungibile normalmente
        ...
        if (curr.dist < agent.mov) {
            hexDirections.forEach(dir => {
                ...
                if (nc.type !== 'empty') return; // muri bloccano BFS normale
                visited.add(key);
                queue.push({ q: nq, r: nr, dist: curr.dist + 1 });
            });
        }
    }
    return true; // non trovato nel BFS normale → ghost-only
}
...
function _planBestAction(va, faction, virtualOccupied, virtualHP) {
    ...
    // --- A0. GHOST TARGET (prima mossa Spettro suggerita dal valutatore) ---
// Se il valutatore C06 ha calcolato una cella di atterraggio ottimale
// attraverso i muri, la inseriamo come candidato a priorità molto alta.
// Questo garantisce che il masterPlan usi subito la libertà di movimento
// dello Spettro invece di pianificare un percorso normale.
if (va.ghostTarget) {
    const gt    = va.ghostTarget;
    const gtKey = getKey(gt.q, gt.r);
    // Verifica che la cella sia ancora libera e raggiungibile
    if (!virtualOccupied.has(gtKey) && !va.visited.has(gtKey)) {
        // Ricalcola il valore tattico dalla cella di destinazione
        const simulatedVA      = { ...va, q: gt.q, r: gt.r };
        const targetsFromGhost = getVirtualTargets(simulatedVA, faction, virtualHP, false);

        let ghostScore = 95000; // base alta: movimento Spettro è quasi sempre ottimo
        for (const t of targetsFromGhost) {
            const effectiveHp = virtualHP.get(getKey(t.q, t.r)) ?? t.hp;
            if (t.type !== 'hq') {
                if (effectiveHp <= va.ref.dmg)      ghostScore = Math.max(ghostScore, 190000);
                else                                 ghostScore = Math.max(ghostScore, 130000);
            } else {
        ...
        candidates.push({
            score:  ghostScore,
            action: { type: 'move', q: gt.q, r: gt.r, cost: 1 },
        });

        // Consuma il ghostTarget: dopo la prima mossa non serve più
        va.ghostTarget = null;
    }
}
    ...
    while (queue.length > 0) {
        ...
        if (curr.dist < va.ref.mov) {
            hexDirections.forEach(dir => {
                ...
                // Lo Spettro attraversa muri e barricate ma non celle occupate da entità
                if (isGhost) {
                    if (cell.entity) return; // entità bloccano sempre
                } else {
