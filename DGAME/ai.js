...
async function executeAITurn() {
    ...
    let iterations     = 0;
    const MAX_ITER = Math.min(300, Math.max(90, vAgents.length * 4));
    ...
    for (const step of masterPlan) {
        ...
        // --- CALCOLO RITARDO DINAMICO (Istantaneo nella Nebbia) ---
        ...
        if (typeof coopState !== 'undefined' && coopState.active) {
            ...
            for (let p = 1; p <= window._coopHumanPlayers; p++) {
                ...
                for (const h of humans) {
                    if (h.hp > 0) {
                        const viewRange = h.rng + 1; // La visuale in coop è Gittata + 1
                        // Se l'inizio o la fine del movimento sono visibili, tieni il ritardo
                        if (hexDistance(h, selectedAgent) <= viewRange || 
                            hexDistance(h, {q: step.q, r: step.r}) <= viewRange) {
            ...
            if (!isVisible) currentDelay = 0; // Azzera il timer!
        }
        // ----------------------------------------------------------
        ...
        // Se era un tiro su agente nemico, controlla se è davvero morto.
        // Il piano virtuale può sbagliare (scudi, coperture, immunità):
        // finché l'agente è ancora vivo e abbiamo AP, continuiamo a sparare.
        if (step.type === 'shoot' && step.subType === 'agent' && currentPlayer === aiFaction) {
            const targetStillAlive = step.targetRef && step.targetRef.hp > 0;
            if (targetStillAlive) {
                while (selectedAgent.ap > 0 && step.targetRef.hp > 0 && currentPlayer === aiFaction) {
                    const retryCell = grid.get(getKey(step.targetRef.q, step.targetRef.r));
                    if (!retryCell) break;
                    validActionTargets = [{ q: step.q, r: step.r, target: step.targetRef, isEnemy: true }];
                    currentActionMode = 'shoot';
                    executeAction(retryCell);
                    if (currentDelay > 0) drawGame();
                    if (currentDelay > 0) await delay(currentDelay);
                }
            }
...
function _scoreAction(va, action, faction, virtualOccupied, virtualHP, claimedTargets) {
    if (action.type === 'shoot') {
        ...
        if (!cell) return 0;
        // FIX: fallback all'HP reale dell'entità, non al valore magico 999
        const realHp      = cell.entity?.hp ?? cell.hp ?? 999;
        const effectiveHp = virtualHP.get(key) ?? realHp;
...
function _bfsDistToTarget(sq, sr, target, virtualOccupied) {
    ...
    while (queue.length > 0) {
        ...
        for (const dir of hexDirections) {
            ...
            if ((cell.type === 'empty' || cell.type === 'water') && !virtualOccupied.has(key)) {
...
function _planBestAction(va, faction, virtualOccupied, virtualHP, claimedTargets) {
    ...
    for (const t of combatTargets) {
        ...
        const tKeyA  = getKey(t.q, t.r);
        // FIX: controlla se il bersaglio ha ancora lo scudo attivo (primo colpo assorbito)
        const isFirstHitA     = virtualHP.get(tKeyA) === undefined;
        const hasActiveShield = isFirstHitA && (t.targetRef?.shielded > 0);
        let effectiveHp = virtualHP.get(tKeyA) ?? t.hp;
        if (cell?.terrain === 'copertura') effectiveHp += 1;
        // Con scudo attivo il primo colpo è azzerato: il bersaglio non è killabile in 1 colpo
        if (hasActiveShield) effectiveHp = Math.max(effectiveHp, va.ref.dmg + 1);
        ...
        const vhpNow         = virtualHP.get(tKeyA);
        const alreadyDamaged = (vhpNow !== undefined && vhpNow < realEntityHp);
    ...
    for (const m of moves) {
        ...
        for (const t of tft) {
            const tCellL1   = grid.get(getKey(t.q, t.r));
            const tKeyL1    = getKey(t.q, t.r);
            const isFirstL1 = virtualHP.get(tKeyL1) === undefined;
            const shieldL1  = isFirstL1 && (t.targetRef?.shielded > 0);
            let ehp         = virtualHP.get(tKeyL1) ?? t.hp;
            if (tCellL1?.terrain === 'copertura') ehp += 1;
            if (shieldL1) ehp = Math.max(ehp, va.ref.dmg + 1); // scudo: non killabile al 1° colpo
            const realHpEnt  = t.targetRef?.hp ?? t.hp;
            const wasHit     = (virtualHP.get(tKeyL1) ?? t.hp) < realHpEnt;
            if (t.type !== 'hq') {
                if (ehp <= va.ref.dmg) {
        ...
        if (lookaheadScore < 100000) {
            ...
            for (const m2 of movesFromM) {
                ...
                for (const t2 of tft2) {
                    const tCellL2   = grid.get(getKey(t2.q, t2.r));
                    const tKeyL2    = getKey(t2.q, t2.r);
                    const isFirstL2 = virtualHP.get(tKeyL2) === undefined;
                    const shieldL2  = isFirstL2 && (t2.targetRef?.shielded > 0);
                    let ehp2        = virtualHP.get(tKeyL2) ?? t2.hp;
                    if (tCellL2?.terrain === 'copertura') ehp2 += 1;
                    if (shieldL2) ehp2 = Math.max(ehp2, va.ref.dmg + 1);
                    const realHp2   = t2.targetRef?.hp ?? t2.hp;
                    const wasHit2   = (virtualHP.get(tKeyL2) ?? t2.hp) < realHp2;
                    if (t2.type !== 'hq') {
                        if (ehp2 <= va.ref.dmg)
...
function _applyVirtualAction(va, action, virtualOccupied, virtualHP) {
    ...
    const targetCell = grid.get(tKey);
    const targetEntity = targetCell?.entity;
    const currentVHP = virtualHP.get(tKey) ?? (targetEntity?.hp ?? targetCell?.hp ?? 0);
    const isFirstHit = virtualHP.get(tKey) === undefined; // nessun danno virtuale ancora
    // FIX: il campo si chiama "shielded" (contatore turni), non "shieldBuff"
    const hasShield  = isFirstHit && (targetEntity?.shielded > 0);
    const actualDmg  = hasShield ? 0 : va.ref.dmg;
    const newHP      = currentVHP - actualDmg;
    // Se lo scudo assorbe: segna l'HP reale in virtualHP (così il secondo colpo
    // nello stesso turno fa danno normale, dato che lo scudo è stato consumato)
    virtualHP.set(tKey, hasShield ? currentVHP : newHP);
    if (!hasShield && newHP <= 0) virtualOccupied.delete(tKey);
...
function getHuntingTarget(va, faction, virtualHP, claimedTargets) {
    ...
    // --- NUOVA LOGICA COOP: Istinto di prossimità per i mostri ---
    if (faction === 9) { // Se è la fazione dei mostri
        let closestHuman = null;
        let minDist = Infinity;

        for (let p = 1; p <= 8; p++) {
            if (!players[p] || p === faction) continue;
            players[p].agents.forEach(human => {
                if (human.hp > 0) {
                    const d = hexDistance({ q: va.q, r: va.r }, human);
                    // I mostri scelgono il bersaglio più vicino a loro stessi
                    if (d < minDist) {
                        minDist = d;
                        closestHuman = human;
                    }
    ...
    // --- Fine logica mostri (prosegue con logica standard per umani AI) ---
    ...
    hexDirections.forEach(dir => {
        for (let d = 1; d <= currentRng; d++) {
            ...
            if (cell.entity) {
                ...
                if (vhp !== undefined && vhp <= 0) continue; // ← continua il for d, non blocca la direzione
...
function getVirtualMoves(va, virtualOccupied) {
    const moves      = [];
    const originCell = grid.get(getKey(va.q, va.r));
    const isGhost = !!(va.ref?.spectreBuff || va.ref?.infiltrateBuff || false);
