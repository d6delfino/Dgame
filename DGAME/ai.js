...
async function _aiExecuteAirdropCombo(aiFaction) {
    ...
    if (dropSlot === -1) return false;

    // --- PERCORSO COMBO (con Blitz): uccisione garantita ---
    const myAgents = pData.agents.filter(a => a.hp > 0 && a.ap >= 3);
    if (blitzSlot === -1 || myAgents.length === 0) {
        // --- PERCORSO SOLO AIRDROP: avvicinamento tattico sicuro ---
        if (blitzSlot !== -1) return false; // ha blitz ma nessun agente con 3AP: lascia fare la combo dopo
        const myAgentsAny = pData.agents.filter(a => a.hp > 0 && a.ap > 0);
        if (myAgentsAny.length === 0) return false;

        let bestAgent = null, bestCell = null, bestScore = -Infinity;

        for (const agent of myAgentsAny) {
            const globalTarget = _findBestGlobalTarget(agent, aiFaction);
            if (!globalTarget) continue;
            const currentDist = hexDistance(agent, globalTarget);

            grid.forEach(cell => {
                if (cell.type !== 'empty' || cell.entity) return;
                const newDist = hexDistance(cell, globalTarget);
                // L'airdrop vale solo se avvicina davvero al nemico
                if (newDist >= currentDist) return;

                // Sicurezza: non atterrare circondati
                if (_aiCountEnemiesNear(cell, aiFaction, agent.rng) > 1) return;

                // Score: distanza dal nemico (meno è meglio) + bonus terreno sicuro
                const cellObj = grid.get(getKey(cell.q, cell.r));
                const isThreatened = _aiIsCellThreatened(cell, aiFaction);
                let score = (currentDist - newDist) * 100; // guadagno in avvicinamento
                if (!isThreatened)          score += 5000;
        ...
        if (!bestAgent || !bestCell) return false;

        console.log(`[AI V24] Airdrop tattico (senza Blitz): ${bestAgent.id} → (${bestCell.q},${bestCell.r})`);
        selectedAgent = bestAgent;
        pData.usedCards[`slot_${dropSlot}`] = true;
        grid.get(getKey(bestAgent.q, bestAgent.r)).entity = null;
        bestCell.entity = bestAgent;
        bestAgent.q = bestCell.q;
        bestAgent.r = bestCell.r;
        bestAgent.ap = 0;

        const cpKey = getKey(bestAgent.q, bestAgent.r);
        if (controlPoints.has(cpKey) && controlPoints.get(cpKey).faction !== bestAgent.faction) {
            controlPoints.get(cpKey).faction = bestAgent.faction;
            if (typeof showCPCapture === 'function') showCPCapture(bestAgent);
        }

        playSpecialVFX(bestAgent, '#a0ff00', '🪂 INFILTRAZIONE!');
        if (isOnline) sendOnlineMessage({
            type: 'ACTION_CARD', cardId: 'C02', slotIndex: dropSlot,
            actingPlayer: aiFaction, targetAgentId: bestAgent.id,
            aiDropQ: bestAgent.q, aiDropR: bestAgent.r
        });
        drawGame();
        await delay(800);
        return true;
    ...
    // Ha sia Blitz che agenti con 3AP: prosegue con la combo letale
...
async function executeAITurn() {
    ...
    const myAgents  = players[aiFaction].agents.filter(a => a.hp > 0 && !a._isLair);
    ...
    // Ordina agenti: Priorità assoluta al BOSS (hp > 9) o a chi ha nemici a tiro.
    myAgents.sort((a, b) => {
        // Se uno dei due è il Boss, deve agire per primo
        const aIsBoss = a.hp > 9 ? 0 : 1;
        const bIsBoss = b.hp > 9 ? 0 : 1;
...
function _aiGetBestRealShootTarget(agent, faction) {
    ...
    for (const dir of hexDirections) {
        ...
        for (let d = 1; d <= currentRng; d++) {
            ...
            if (cell.entity) {
                // FIX COOP: Tratta gli alleati umani come "se stesso" (blocca la mira amica, non spara)
                const isCoopAlly = (typeof coopState !== 'undefined' && coopState.active && faction !== 9 && cell.entity.faction !== 9);
                if (cell.entity.faction === faction || isCoopAlly) break; 
...
function _findBestGlobalTarget(agent, faction) {
    ...
    for (let p = 1; p <= totalPlayers; p++) {
        if (!players[p] || p === faction) continue;
        // FIX COOP: Se sono un alleato umano, ignoro gli altri alleati umani
        if (typeof coopState !== 'undefined' && coopState.active && faction !== 9 && p !== 9) continue;
    ...
    for (let p = 1; p <= totalPlayers; p++) {
        if (!players[p] || p === faction) continue;
        // FIX COOP: Non attaccare gli HQ degli alleati
        if (typeof coopState !== 'undefined' && coopState.active && faction !== 9 && p !== 9) continue;
...
function _findOptimalPath(agent, target, allowSmashing) {
    ...
    let head = 0; // <-- NUOVO: Cursore di lettura
    let maxSteps = 4000; // Sicurezza per evitare blocchi del browser

    // <-- NUOVO: Controllo tramite cursore (head < queue.length)
    while (head < queue.length && maxSteps > 0) {
        maxSteps--;
        const curr = queue[head++]; // <-- NUOVO: Legge e avanza senza riallocare l'array
    ...
    for (let slotIndex = 0; slotIndex < pData.cards.length; slotIndex++) {
            ...
            for (const a of myAgents) {
                ...
                for (const dir of hexDirections) {
                    ...
                    for (let d = 1; d <= doubleRng; d++) {
                        ...
                        // FIX COOP: Il cecchino ignora gli alleati
                        const isCoopAlly = (typeof coopState !== 'undefined' && coopState.active && aiFaction !== 9 && cell.entity && cell.entity.faction !== 9);
                        const isEnemy = cell.entity && cell.entity.faction !== aiFaction && !isCoopAlly && cell.entity.hp > 0;
                        const isWallOrBarricade = (cell.type === 'wall' || cell.type === 'barricade');
                        const isAlly = cell.entity && (cell.entity.faction === aiFaction || isCoopAlly);
        ...
        // --- C04 EMP (Raggio FISSO 5) ---
            ...
            myAgents.forEach(a => {
                ...
                grid.forEach(c => {
                    const t = c.entity;
                    // FIX COOP: Non usare l'EMP contro gli alleati
                    const isCoopAlly = (typeof coopState !== 'undefined' && coopState.active && aiFaction !== 9 && t && t.faction !== 9);
                    if (t && t.faction !== aiFaction && !isCoopAlly && t.hp > 0 && hexDistance(a, c) <= 5) {
...
function _aiTryFortinoRealtime(aiFaction) {
    ...
    for (let p = 1; p <= totalPlayers; p++) {
        ...
        // FIX COOP: Non ingabbiare gli alleati
        if (typeof coopState !== 'undefined' && coopState.active && aiFaction !== 9 && p !== 9) continue;
        players[p].agents.forEach(e => {
            if (e.hp > 0) {
    ...
    if (builds.length > 0) {
        ...
        builds.forEach(pos => {
            ...
            cell.customSpriteId = typeof getFactionBarricadeId === 'function' 
                                      ? getFactionBarricadeId(aiFaction) 
...
function _aiIsCellThreatened(cellPos, faction) {
    for (let p = 1; p <= totalPlayers; p++) {
        if (!players[p] || p === faction) continue;
        // FIX COOP: Le truppe alleate non sono una minaccia da cui scappare
        if (typeof coopState !== 'undefined' && coopState.active && faction !== 9 && p !== 9) continue;
...
function _aiFindSafeMove(agent, faction, visitedThisTurn) {
    ...
    // <-- NUOVO: Controllo tramite cursore
    while(head < queue.length) {
        const curr = queue[head++]; // <-- NUOVO: Legge e avanza
