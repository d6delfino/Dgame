...
function _applyVirtualAction(va, action, virtualOccupied, virtualHP) {
    ...
    const hasShield  = isFirstHit && targetEntity?.shieldBuff;
    const actualDmg  = hasShield ? 0 : va.ref.dmg;
    // Se scudo: primo colpo assorbito, rimuovi il buff virtualmente
    const newHP = currentVHP - actualDmg;
    virtualHP.set(tKey, hasShield ? currentVHP : newHP); // HP invariati se scudo assorbe
