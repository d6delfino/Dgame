...
function _aiEvaluateAndPlayCards(aiFaction) {
    ...
    const myAgents = pData.agents.filter(a => a.hp > 0 && a.ap > 0);
    if (!myAgents.length) return;

    // Agente migliore per offesa (danno alto, in range di qualcuno)
    const bestAttacker = myAgents.slice().sort((a, b) => b.dmg - a.dmg)[0];
    // Agente più ferito
    const mostWounded  = myAgents.slice().sort((a, b) => (a.hp / a.maxHp) - (b.hp / b.maxHp))[0];
    // Agente con più AP rimasti
    const mostAP       = myAgents.slice().sort((a, b) => b.ap - a.ap)[0];
    ...
    pData.cards.forEach((cardId, slotIndex) => {
        ...
        // Costo AP minimo richiesto
        const apCost = card.apCost || 0;

        // Decidi se e con quale agente usare la carta
        let chosenAgent = null;
        let shouldPlay  = false;

        switch (cardId) {

            case 'C01': // Blitz (+1 AP) — usa sempre se l'attaccante ha ancora bersagli
                if (_aiHasEnemiesInRange(bestAttacker, aiFaction)) {
                    chosenAgent = bestAttacker;
                    shouldPlay  = true;
                }
                break;

            case 'C03': // Cecchino — usa se l'attaccante ha nemici in range
                if (_aiHasEnemiesInRange(bestAttacker, aiFaction)) {
                    chosenAgent = bestAttacker;
                    shouldPlay  = true;
                }
            ...
            case 'C05': // Esplosivo — usa se ci sono 2+ nemici raggruppati vicino al target
                if (_aiEnemiesNearTarget(bestAttacker, aiFaction) >= 2) {
                    chosenAgent = bestAttacker;
                    shouldPlay  = true;
                }
                break;

            case 'C07': // Scudo — usa sull'agente più ferito se è sotto il 50% HP
                if (mostWounded.hp < mostWounded.maxHp * 0.5) {
                    chosenAgent = mostWounded;
                    shouldPlay  = true;
                }
                break;

            case 'C09': // EMP — usa se ci sono 2+ nemici entro 5 celle dall'attaccante
                if (_aiEnemiesInEMPRange(bestAttacker, aiFaction) >= 2) {
                    chosenAgent = bestAttacker;
                    shouldPlay  = true;
                }
                break;

            case 'C04': // Medikit — usa sull'agente più ferito se mancano 2+ HP
                if ((mostWounded.maxHp - mostWounded.hp) >= 2) {
                    chosenAgent = mostWounded;
                    shouldPlay  = true;
                }
                break;

            case 'C10': // Upgrade — aggiungi sempre DMG all'attaccante principale
                if (mostAP.ap >= 1) {
                    chosenAgent = bestAttacker;
                    shouldPlay  = true;
                }
            ...
            // C02 Fortino, C06 Spettro, C08 Airdrop — troppo situazionali,
            // richiedono interazione col canvas. Saltiamo per ora.
        }

        if (!shouldPlay || !chosenAgent) return;
        if (apCost > 0 && chosenAgent.ap < apCost) return;

        // Attiva la carta come se fosse il giocatore umano
        selectedAgent = chosenAgent;
        if (!pData.usedCards) pData.usedCards = {};
        pData.usedCards[slotKey] = true;
        card.apply(aiFaction);

        console.log(`[AI] Carta usata: ${card.name} su agente ${chosenAgent.id?.split('-')[0]}`);
    });
}
