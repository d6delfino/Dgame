...
function receiveRemoteCardAction(data) {
    ...
    // L'host ha già spostato fisicamente l'agente e ci manda
    // le coordinate di destinazione (aiDropQ/aiDropR).
    // Il client replica il teletrasporto direttamente, senza
    // passare per setActionMode (che aspetterebbe un click).
    if (data.cardId === 'C02' && data.aiDropQ !== undefined && selectedAgent) {
        ...
        updateIngameCardsUI();
        drawGame();
        return; // ← esce senza chiamare card.apply()
    }
