...
const CARD_DEFINITIONS = {
    ...
    C05: {
        ...
        apply(faction) {
            if (!selectedAgent.demoBuff) {
                ...
                selectedAgent.demoCount   = 0; // FIX: Inizializza il contatore
            }
            selectedAgent.demoCount += 1; // FIX: Aumenta il contatore ad ogni stack
            selectedAgent.dmg += selectedAgent.originalDmg;
            const multiplier = selectedAgent.demoCount + 1; // Reso più preciso visivamente
    ...
    C09: {
      ...
      apply(faction) {
        // La carta si applica ESCLUSIVAMENTE all'agente selezionato
        ...
        if (!target.medikitTempBuff) {
            ...
            if (target.type !== 'hq') target.mov += 1; // HQ non ottiene passi
            target.rng    += 1;
            target.dmg    += 1;
            target.maxHp  += 1;
    ...
    C10: {
        ...
        apply(faction, payload) {
            ...
            if (payload.mov > 0 && selectedAgent.type !== 'hq') selectedAgent.mov += payload.mov; // HQ non ottiene passi
...
function _updateUpgradeView(overlay, baseStats, alloc, pointsLeft, confirmBtn) {
    ...
    for (const id of ['hp', 'mov', 'rng', 'dmg']) {
        ...
        overlay.querySelectorAll(`[data-stat="${id}"]`).forEach(btn => {
            const delta    = parseInt(btn.dataset.delta);
            const isHQMove = (selectedAgent && selectedAgent.type === 'hq' && id === 'mov');
            const disabled = (delta < 0 && alloc[id] <= 0) || (delta > 0 && pointsLeft <= 0) || isHQMove;
...
function _activateIngameCard(slotIndex, cardId) {
    ...
    if (card?.needsAgent && (!selectedAgent || selectedAgent.faction !== currentPlayer)) {
        playSpecialVFX({ q: 0, r: 0 }, '#ff3333', 'SELEZIONA UN BERSAGLIO VALIDO!');
        return;
    }
    
    if (selectedAgent.type === 'hq' && cardId === 'C02') {
        playSpecialVFX({ q: 0, r: 0 }, '#ff3333', 'LA BASE NON SALTA!');
        return;
    }

    if (selectedAgent.type === 'hq' && cardId === 'C06') {
        playSpecialVFX({ q: 0, r: 0 }, '#ff3333', 'LA BASE NON SI MUOVE!');
        return;
    }
...
function receiveRemoteCardAction(data) {
    ...
    // RICERCA BERSAGLIO PIÙ ROBUSTA (Agenti + HQ)
    if (data.targetAgentId) {
        if (pData.hq && pData.hq.id === data.targetAgentId) {
            // È l'HQ!
            selectedAgent = pData.hq;
        } else {
            // Cerchiamo tra gli agenti
            const agent = pData.agents.find(a => a.id === data.targetAgentId);
            if (agent) {
                selectedAgent = agent;
            } else {
                console.warn(`[Host] Entità ${data.targetAgentId} non trovata. Tento fallback sul primo agente vivo.`);
...
function updateIngameCardsUI() {
    ...
    pData.cards.forEach((cardId, slotIndex) => {
        ...
        const cost     = card.apCost || 0;
        const isHQAirdrop = selectedAgent && selectedAgent.type === 'hq' && cardId === 'C02';
        const isHQSpettro = selectedAgent && selectedAgent.type === 'hq' && cardId === 'C06';
        ...
        if (isUsed) {
            ...
            btn.disabled = true;
            btn.title    = `La Base non può essere paracadutata.`;
        } else if (isHQSpettro) {
            btn.style.cssText = `flex:1;display:flex;flex-direction:column;align-items:center;gap:2px;padding:2px 4px;min-height:0;border:2px solid #555;background:rgba(0,0,0,0.5);opacity:0.6;cursor:not-allowed;border-radius:4px;`;
            btn.disabled = true;
            btn.title    = `Spettro non ha effetto: la Base è immobile (MOV 0).`;
        } else if (missingAP) {
            btn.style.cssText = `flex:1;display:flex;flex-direction:column;align-items:center;gap:2px;padding:2px 4px;min-height:0;border:2px solid #555;background:rgba(0,0,0,0.5);opacity:0.6;cursor:not-allowed;border-radius:4px;`;
            btn.disabled = true;
