...
const CARD_DEFINITIONS = {
    ...
    C03: {
        ...
        needsAgent: true,
        description: 'Aumenta la gittata del valore base. Lo sparo danneggia ed attraversa il primo bersaglio colpendo anche il secondo lungo la linea di tiro, ignorando gli scudi potenziati.',
        apply(faction) {
            // Se è la prima volta che si usa in questo turno, salva la gittata base
            ...
            // Aggiunge la gittata base a quella attuale (es: 2 -> 4 -> 6 -> 8)
            selectedAgent.rng += selectedAgent.originalRng;
            
            // Calcola il moltiplicatore per l'effetto visivo (es: 4/2 = 2, 6/2 = 3)
    ...
    C05: {
        ...
        needsAgent: true,
        description: 'Sul bersaglio aggiunge il tuo danno base al danno attuale, ed applica la metà del danno totale a tutte le caselle adiacenti.',
        apply(faction) {
            // Se è la prima volta che si usa in questo turno, salva il danno base
            if (!selectedAgent.demoBuff) {
                selectedAgent.originalDmg = selectedAgent.dmg;
                selectedAgent.demoBuff    = true;
            }
            
            // Aggiunge il danno base a quello attuale (es: 2 -> 4 -> 6 -> 8)
    ...
    C07: {
        id: 'C07', name: 'Scudo Potenziato', icon: '🛡️', color: '#00ffff',
        needsAgent: true,
        description: 'Se è attivo annulla il prossimo attacco subito dal tuo agente, resta anche nei prossimi turni.',
        apply(faction) {
    ...
    C09: {
        ...
        needsAgent: true,
        description: 'Tutti i nemici entro 5 caselle perdono 1 AP al loro prossimo turno e disattiva tutti i loro scudi potenziati.',
        apply(faction) {
            let found = false;
            grid.forEach(cell => {
                if (
                    ...
                    hexDistance(selectedAgent, cell.entity) <= 5
                ) {
                    // Applica il debuff agli AP
                    cell.entity.empDebuff = (cell.entity.empDebuff || 0) + 1;
                    
                    // Controlla e disabilita lo scudo se presente
            ...
            if (!found) {
                playSpecialVFX(selectedAgent, this.color, 'Nessun bersaglio EMP');
            } else {
                drawGame(); // Aggiorna il canvas per far sparire gli aloni degli scudi distrutti
            }
...
function updateIngameCardsUI() {
    ...
    pData.cards.forEach((cardId, slotIndex) => {
        ...
        // FIX: Calcolo degli AP mancanti per disabilitare visivamente il bottone
        const cost = card.apCost || 0;
