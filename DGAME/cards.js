...
const CARD_DEFINITIONS = {
    ...
    C04: {
        ...
        needsAgent: true,
        description: `Tutti i nemici entro 5 caselle perdono ${GAME.EMP_DEBUFF_AP} AP al prossimo turno, perdono gli scudi, il buff Spettro e la copertura della nebbia.`,
        apply(faction) {
            let found = false;
            grid.forEach(cell => {
...
function initCardSelectionUI() {
    ...
    toggleBtn.style.cssText = 'width:100%; margin-bottom:4px; font-size:13px; padding:12px 20px;';

    // Funzione centrale per mantenere SEMPRE l'emoji grande
    const updateToggleHTML = () => {
        ...
        toggleBtn.innerHTML = `
            <span style="font-size:3.0em; vertical-align:middle; line-height:1; display:inline-block; margin-top:-0.25em; margin-bottom:-0.25em; position:relative; top:4px;">🃏</span>
            Carte Strategiche 
            &nbsp;<span id="card-selection-count" style="color:${color};font-size:13px;">carte (${n}/3)</span> 
            &nbsp;${arrow}
        `;
    };
    ...
    const originalToggleCardSelection = _toggleCardSelection;
    _toggleCardSelection = function(cardId) {
        originalToggleCardSelection(cardId);
        updateToggleHTML();   // ← forza aggiornamento dopo selezione
    };
...
function _updateCardCount() {
    ...
    // Forza aggiornamento completo del pulsante con emoji grande
...
function receiveRemoteCardAction(data) {
    ...
    // --- FIX CRITICO: Forza il cambio di selectedAgent localmente sull'Host ---
    // Mettiamo in pausa la selezione dell'Host per simulare esattamente 
    // l'ambiente del Client ed evitare crash di lettura proprietà.
    ...
    if (data.cardId === 'C02' && data.aiDropQ !== undefined && selectedAgent) {
        ...
        // Ripristina l'agente che l'Host stava guardando
    ...
    // Applica l'effetto della carta (C10 leggerà `payload` e modificherà `selectedAgent` senza crashare)
