...
function endTurn(fromNetwork = false, forceHost = false) {
    // Ora se forceHost è true, l'Host bypassa il blocco e forza la fine del turno
    if (isOnline && currentPlayer !== myPlayerNumber && !fromNetwork && !isHostAITurn() && !forceHost) return;
    ...
    if (!fromNetwork) {
        ...
        if (isOnline) {
            ...
            // Full state sync: l'host invia lo stato completo dopo resetTurnState
            setTimeout(() => {
...
function resetTurnState() {
    ...
    // Rimuovi immunità primo turno quando il giocatore inizia il suo turno  ← AGGIUNGI
    // Appena tocca a questo giocatore, i suoi agenti e il suo HQ diventano vulnerabili.
players[currentPlayer].agents.forEach(a => { 
    a.firstTurnImmune = false; 
});
...
function handleEntityDeath(entity, killerFaction = null) {
    ...
    if (entity.type === 'agent') {
        ...
        // --- NUOVO: Premio per distruzione base ---
...
function applyMedikit(agent) {
    ...
    let missingHp = agent.maxHp - agent.hp;
    let healAmount = Math.min(3, missingHp);
