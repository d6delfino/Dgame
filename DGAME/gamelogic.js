...
function updateActivePlayerBorders() {
    ...
    if (surrenderBtn) {
        ...
        const canSurrender = state === 'PLAYING' && players[localPlayer]?.agents?.length > 0;
...
function surrenderCurrentPlayer() {
    ...
    if (!confirm(`Confermi la resa di ${players[surrenderingPlayer]?.name || 'P' + surrenderingPlayer}?`)) return;
    playSFX('explosion');

    if (isOnline && !isHost) {
        // Il client manda la richiesta all'host e aspetta il FULL_STATE_SYNC
        sendOnlineMessage({ type: 'SURRENDER', faction: surrenderingPlayer });
        return;
    }

    // Host (o locale): applica direttamente
    _applySurrender(surrenderingPlayer);
}

function _applySurrender(faction) {
    (players[faction]?.agents || []).forEach(a => { a.hp = 0; });
