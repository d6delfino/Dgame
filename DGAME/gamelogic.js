    ...
    if (currentActionMode === 'heal') {
        ...
        if (targetData || fromNetwork) {
            ...
            if (actualTarget.shielded) {
                ...
                // Immunità primo turno: blocca il danno  ← AGGIUNGI
                if (actualTarget.firstTurnImmune) {       // ← AGGIUNGI
                    playSpecialVFX(actualTarget, '#00ffff', '🛡️ IMMUNE AL PRIMO TURNO!');  // ← AGGIUNGI
                    drawLaserBeam(selectedAgent, targetCell);                // ← AGGIUNGI
                    success = true;                                          // ← AGGIUNGI
                    return { success, actionCost };                          // ← AGGIUNGI
                } 
                ...
                if (actualTarget.hp <= 0) {
                    ...
                    else if (actualTarget.type === 'agent') {
                        // Passiamo chi ha ucciso (selectedAgent è sempre l'attaccante)
                        handleEntityDeath(actualTarget, selectedAgent.faction);
                    } 
                    else if (actualTarget.type) {
                        // HQ o altre entità (nessuna ricompensa)
...
function handleEntityDeath(entity, killerFaction = null) {
    ...
    if (entity.type === 'agent') {
        // ─────────────────────────────────────────────────────
        // +2 CREDITS PER UCCISIONE AGENTE (ora funziona su TUTTI i kill)
        // ─────────────────────────────────────────────────────
