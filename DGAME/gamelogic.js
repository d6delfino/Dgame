...
// ============================================================
// HELPER: CATTURA PUNTO DI CONTROLLO
// ============================================================
// Logica centralizzata per catturare un CP quando un agente vi si sposta.
// Usata da _executeStandardAction (move) e dall'handler card_airdrop in carduse.js.
// In precedenza questo blocco era duplicato in entrambi i posti.

function captureControlPoint(agent) {
    const key = getKey(agent.q, agent.r);
    if (!controlPoints.has(key)) return;
    const cp = controlPoints.get(key);
    if (cp.faction !== agent.faction) {
        cp.faction = agent.faction;
        showCPCapture(agent);
    }
}
...
function resetTurnState() {
    ...
    // 1. Reset standard dello stato di input
    selectedAgent      = null;
    ...
    // 2. Rimuovi immunità primo turno: appena tocca a questo giocatore,
    //    i suoi agenti e il suo HQ diventano vulnerabili.
    ...
    // 3. Hook di pre-reset (pulizia buff, EMP, ecc.) — registrati da carduse.js
    _turnResetHooks.forEach(fn => fn());

    // 4. Reddito crediti ---
    // turnCount viene incrementato solo quando tocca a P1; al primissimo resetTurnState
    // vale ancora 0 (o 1 se P1 inizia), ma in ogni caso il turno 1 del primo giocatore
    // non deve generare reddito: i crediti iniziali vengono già dal setup.
    const isVeryFirstTurn = (turnCount <= 1 && currentPlayer === _firstPlayerOfGame);
    if (!isVeryFirstTurn && players[currentPlayer].hq && players[currentPlayer].hq.hp > 0) {
        ...
        showCreditIncome(currentPlayer, income);
    }
...
function _executeStandardAction(targetCell, fromNetwork) {
    ...
    if (currentActionMode === 'heal') {
        ...
        if (targetData || fromNetwork) {
            ...
            // Crea una mappa per il singolo bersaglio e usa la funzione centralizzata
...
// ============================================================
/**
 * Applica danni a una serie di bersagli, gestendo scudi, immunità, terreni, medikit e morte.
 * @param {Map} damageMap - Mappa dei danni (chiave: entità/ostacolo, valore: danno da infliggere)
 * @param {Object} attacker - L'agente che sta attaccando (per i premi in crediti)
 */
function resolveCombatDamage(damageMap, attacker) {
    damageMap.forEach((totalDmg, actualTarget) => {
        ...
        // 2. GESTIONE IMMUNITÀ PRIMO TURNO
        else if (actualTarget.firstTurnImmune) {
            finalDmg = 0;
        ...
        if (actualTarget.hp <= 0) {
            ...
            else if (actualTarget.type === 'agent' || actualTarget.type === 'hq') {
                handleEntityDeath(actualTarget, attacker.faction);
            } 
...
function handleEntityDeath(entity, killerFaction = null) {
    ...
    if (entity.type === 'agent') {
        if (killerFaction) awardKillReward(killerFaction);
        const list = owner.agents;
        const idx  = list.findIndex(a => a.id === entity.id);
        if (idx !== -1) list.splice(idx, 1);
    } else if (entity.type === 'hq') {
        if (killerFaction) awardBaseDestroyReward(killerFaction);
        owner.hq = null;
    }
}
...
function checkWinConditions() {
    ...
    if (factionsWithAgents.length === 1) {
        ...
        setTimeout(() => showGameOverlay(
            'ULTIMI AGENTI!',
            `La fazione ${winner.name.toUpperCase()} è l'ultima con agenti operativi e VINCE!`,
            winner.color
        ), 300);
...
function applyMedikit(agent) {
    ...
    if (agent.hp >= agent.maxHp) return;

    const healAmount = Math.min(GAME.MEDIKIT_HEAL, agent.maxHp - agent.hp);
    agent.hp += healAmount;
    agent.medikitBuff = false;
