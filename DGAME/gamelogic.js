...
function initTimerUI() {
    ...
    timerUI.onclick = () => { 
        // Blocco di 4 secondi per evitare doppi click accidentali in locale
        if (window._turnStartTime && Date.now() - window._turnStartTime < 2000) {
            showTemporaryMessage("⏳ Attendi un attimo prima di passare...", 1500);
            return;
        }
...
function endTurn(fromNetwork = false, forceHost = false) {
    ...
    if (!isOnline || isHost || fromNetwork) {
        if (!fromNetwork) playSFX('airdrop');
...
function resetTurnState() {
    ...
    // Registra l'istante esatto di inizio turno per il blocco anti-doppio click
    window._turnStartTime = Date.now(); 
    ...
    // ── FASE 3: Ripristino AP agenti e HQ ────────────────────
    ...
    const currentHq = players[currentPlayer].hq;
    if (currentHq && currentHq.hp > 0) currentHq.ap = GAME.AP_PER_TURN;
...
function updateUI() {
    ...
    if (selectedAgent && selectedAgent.faction === currentPlayer && (selectedAgent.type === 'agent' || selectedAgent.type === 'hq')) {
    ...
    } else if (selectedAgent && selectedAgent.faction !== currentPlayer && (selectedAgent.type === 'agent' || selectedAgent.type === 'hq')) {
        // Entità nemica selezionata: mostra le stat in sola lettura
        infoPanel.style.display = 'flex';
        document.getElementById('info-hp').innerText  = `${selectedAgent.hp}/${selectedAgent.maxHp}`;
        document.getElementById('info-mov').innerText = selectedAgent.mov;
        document.getElementById('info-rng').innerText = selectedAgent.rng;
    ...
    const isActiveEntity = !!(selectedAgent && selectedAgent.faction === currentPlayer);
    document.getElementById('btn-move').disabled  = !(isActiveEntity && selectedAgent.ap > 0 && selectedAgent.mov > 0);
    document.getElementById('btn-shoot').disabled = !(isActiveEntity && selectedAgent.ap > 0);
    document.getElementById('btn-build').disabled =
    document.getElementById('btn-heal').disabled  = !(isActiveEntity && selectedAgent.ap >= 2);
...
function cancelAction() {
    ...
    // FIX CHIRURGICO: Rimette le carte "in sospeso" nel mazzo 
    // se il giocatore annulla l'azione cliccando a vuoto o cambiando idea.
    if (selectedAgent) {
...
function calculateValidHeals() {
    ...
    grid.forEach(cell => {
        if (
            cell.entity &&
            (cell.entity.type === 'agent' || cell.entity.type === 'hq') &&
...
// Su touch, il browser genera un click "sintetico" ~300ms dopo il touchend
// già gestito manualmente: questo debounce scarta quell'eco duplicato
// senza toccare preventDefault (per non riaprire il bug resa/musica).
let _lastCanvasClickTime = 0;

function handleCanvasClick(e) {
    const now = Date.now();
    if (now - _lastCanvasClickTime < 400) return;
    ...
    // Notifica gli altri giocatori online dell'entità selezionata (Agente o HQ)
    if (isOnline && selectedAgent && (selectedAgent.type === 'agent' || selectedAgent.type === 'hq')) {
        sendOnlineMessage({
            type: 'AGENT_SELECT',
            q: selectedAgent.q,
            r: selectedAgent.r,
...
function executeRemoteAction(data) {
    ...
    if (data.agentBuffs) {
        ...
        selectedAgent.demoCount      = data.agentBuffs.demoCount      || 0; // FIX: Ripristina contatore
        ...
        // FIX MULTIPLAYER: Ripristina il flag temporaneo del MediBuff (C09).
        // Senza questo, il reset di fine turno (carduse.js) può disallinearsi:
        // l'host sa di dover sottrarre il bonus mov/maxHp, il client remoto no.
        selectedAgent.medikitTempBuff = data.agentBuffs.medikitTempBuff || false;
        ...
        // FIX MULTIPLAYER: Ripristina la Gittata originale per la corretta pulizia di fine turno
        if (data.agentBuffs.rng !== undefined) {
            selectedAgent.rng         = data.agentBuffs.rng;
            selectedAgent.originalRng = data.agentBuffs.originalRng;
        }
        // FIX MULTIPLAYER: Ripristina Mov e MaxHp, stesso motivo di dmg/rng sopra
        // (uniche due statistiche toccate da MediBuff che non venivano risincronizzate)
        if (data.agentBuffs.mov !== undefined) {
            selectedAgent.mov = data.agentBuffs.mov;
        }
        if (data.agentBuffs.maxHp !== undefined) {
            selectedAgent.maxHp = data.agentBuffs.maxHp;
        }
    }
...
function showRemoteAgentStats(data) {
    ...
    // Accetta sia agenti che HQ
    if (!cell?.entity || !(cell.entity.type === 'agent' || cell.entity.type === 'hq')) return;
...
function executeAction(targetCell, fromNetwork = false) {
    ...
    // --- SNAPSHOT BUFF: Catturiamo lo stato PRIMA di qualsiasi calcolo (Cruciale per self-damage) ---
    const buffsSnapshot = !fromNetwork ? {
        ...
        demoCount:      selectedAgent.demoCount      || 0, // FIX: Salva contatore per la rete
        ...
        originalRng:    selectedAgent.originalRng   || selectedAgent.rng,
        mov:             selectedAgent.mov,
        maxHp:           selectedAgent.maxHp,
        medikitTempBuff: selectedAgent.medikitTempBuff || false,
    } : null;
...
function _executeStandardAction(targetCell, fromNetwork) {
    ...
    if (currentActionMode === 'heal') {
        ...
        if ((targetCell.entity?.type === 'agent' || targetCell.entity?.type === 'hq')
            && targetCell.entity.hp < targetCell.entity.maxHp) {
...
function resolveCombatDamage(damageMap, attacker) {
    damageMap.forEach((baseDmg, target) => {
        ...
        if (finalDmg > 0) {
            ...
            if ((target.type === 'agent' || target.type === 'hq') && target.medikitBuff && target.hp < target.maxHp) {
...
function showGameOverlay(title, message, color) {
  setTimeout(() => {
    ...
    // --- INIZIO NUOVA GESTIONE ICONA VITTORIA ---
    const icon = document.createElement('div');
    icon.style.cssText = 'margin-bottom:20px; display:flex; justify-content:center; align-items:center;';

    // ECCEZIONE CHIRURGICA: Vittoria Coop
    if (typeof coopState !== 'undefined' && coopState.active && color === '#FFD700') {
        const img = document.createElement('img');
        img.src = 'img/coopwin.png';
        img.style.cssText = `
            height: 16.5em; max-height: 40vh; width: auto; object-fit: contain; 
            filter: drop-shadow(0 0 30px ${color}); animation: pulse 2s infinite;
        `;
        img.onerror = () => { icon.innerHTML = '<span style="font-size:8em;">🏆</span>'; };
        icon.appendChild(img);
    } 
    else {
        // LOGICA NORMALE PER TUTTE LE ALTRE MODALITÀ O SCONFITTE
        // 1. Trova l'ID estetico (cosmeticFaction) partendo dal colore passato alla funzione
        let hqId = null;
        for (let p = 1; p <= 8; p++) {
            if (players[p] && players[p].color === color) {
                hqId = players[p]._cosmeticFaction || p;
                break;
            }
        }

        // 2. Tenta di caricare l'HQ, altrimenti fallback sulla coppa
        const isCoopVictory = title.includes('MISSIONE COMPLETATA');
        if (hqId || isCoopVictory) {
        const img = document.createElement('img');
        img.src = isCoopVictory ? 'img/coopwin.png' : `img/HQ${hqId}.png`;
            ...
            img.onerror = () => {
                icon.innerHTML = '<span style="font-size:8em;">🏆</span>';
            };
    ...
    // --- FINE NUOVA GESTIONE ICONA VITTORIA ---
...
function _applySurrender(faction) {
    ...
    if (players[faction]?.hq) {
        ...
        // <--- INIZIO NUOVO EFFETTO ESPLOSIONE EMOJI --->
        if (typeof hexToPixel === 'function') {
            const p = hexToPixel(players[faction].hq.q, players[faction].hq.r);
            
            // 1. Inietta l'animazione CSS se non esiste ancora
            if (!document.getElementById('surrender-boom-anim')) {
                const style = document.createElement('style');
                style.id = 'surrender-boom-anim';
                style.innerHTML = `
                    @keyframes surrenderBoom {
                        0%   { transform: translate(-50%, -50%) scale(0.5); opacity: 1; }
                        25%  { transform: translate(-50%, -50%) scale(2.5); opacity: 1; }
                        50%  { transform: translate(-50%, -50%) scale(1.5); opacity: 1; }
                        75%  { transform: translate(-50%, -50%) scale(3.5); opacity: 1; }
            ...
            // 2. Crea l'emoji gigante
            const boomEl = document.createElement('div');
            boomEl.innerText = '💥';
            boomEl.style.cssText = `
                position: fixed;
                left: ${p.x}px;
                top: ${p.y}px;
                font-size: 60px; /* Dimensione base, verrà moltiplicata dal CSS */
                z-index: 10000;
                pointer-events: none;
                filter: drop-shadow(0 0 15px #ff8800);
                animation: surrenderBoom 1.5s ease-out forwards;
            `;
            document.body.appendChild(boomEl);

            // 3. Rimuove l'elemento HTML dopo che l'animazione è finita
            setTimeout(() => boomEl.remove(), 1500);
        }
        // <--- FINE NUOVO EFFETTO ESPLOSIONE EMOJI --->
