...
window._net_buildSnapshot = function(forPlayer) {
    ...
    const filteredSectors = campaignState.sectors.map(s => {
        ...
        let outObj = {
            ...
            // RIMOSSI: x e y. I Client calcolano i pixel localmente. Inviarli spreca solo KB.
            blocked: s.blocked, income: s.income,
            // RIMOSSO: specialization (è una meccanica vecchia non più usata)
        ...
        // Usa l'array globale per iniettare tutto senza duplicare codice
...
function _clientOpenCreditSelector(data) {
    const { 
        sectorId, sectorOwner, defCredits, canSabotage, 
        nukeCost, availCredits, nukeUnlocked, artCharges, canArtillery 
    } = data;
    
    const p = window.myPlayerNumber;
    const canAttack = _isSectorReachable(sectorId, p);

    // --- AGGIUNTA VARIABILI INFILTRATI ---
    const infiltratiPossible = _isInfiltratiReachable(sectorId, p);
    const canInfiltrati      = infiltratiPossible && availCredits >= 4;
    // -------------------------------------
    ...
    const modal = _gui_createModalBase(p, "⚔️ ORDINE AZIONE", `Bersaglio: Settore ${sectorId}`, content, [
        { id: 'btn-attack',    label: canAttack ? 'INVIA ATTACCO' : 'FUORI PORTATA TRUPPE', primary: true,  disabled: !canAttack || availCredits < 4 },
        { id: 'btn-nuke', label: `☢️ NUCLEARIZZA (💰${nukeCost})`, primary: true, disabled: !canSabotage },
        { id: 'btn-artillery', label: `🎯 ARTIGLIERIA (Salve: ${artCharges || 0})`, primary: true, disabled: !canArtillery },
        { id: 'btn-infiltrati', label: `🕵️ INFILTRATI (${infiltratiPossible ? 'Disponibile' : 'Non hai Infiltrati'})`, primary: true, disabled: !canInfiltrati },
        { id: 'btn-cancel', label: 'ANNULLA', primary: false }
    ]);

    const nukeBtn = modal.querySelector('#btn-nuke');
    if (!nukeUnlocked) {
        nukeBtn.innerText = "☢️ NUCLEARIZZA (Serve Silo)";
    } else if (!canSabotage && availCredits >= nukeCost) {
        nukeBtn.innerText = "☢️ NUCLEARIZZA (Fuori Portata)";
    }
    ...
    // --- AGGIUNTO LISTENER BOTTONE INFILTRATI ---
    const btnInfiltrati = modal.querySelector('#btn-infiltrati');
    if (btnInfiltrati) {
        btnInfiltrati.onclick = () => {
            const val = parseInt(slider.value) || 4;
            modal.remove();
            // L'host (come da _hostApplyInfiltratiAttack) aspetta {sectorId, credits}
            _net_clientSend('INFILTRATI_ATTACK', { sectorId: sectorId, credits: val }); 
        };
    }
    // --------------------------------------------

    modal.querySelector('#btn-nuke').onclick = () => {
        if (!confirm(`☢️ ATTENZIONE: NUCLEARIZZAZIONE\n\nIl settore sarà BLOCCATO per il turno successivo.\nVuoi procedere?`)) return;
        modal.remove();
    ...
    modal.querySelector('#btn-artillery').onclick = () => {
        ...
        modal.remove();
        _net_clientSend('ARTILLERY_STRIKE', { sectorId: sectorId });
    };
...
function _applySnapshot(snap) {
    ...
    if (snap.playersMeta && window.players) {
        Object.entries(snap.playersMeta).forEach(([p, meta]) => {
            ...
            window.players[pNum].isDisconnected = !!meta.isDisconnected;
        });
    }
}
