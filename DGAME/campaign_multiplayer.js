    ...
    return {
        ...
        playersMeta: (() => {
            ...
            for (let p = 1; p <= campaignState.numPlayers; p++) {
                if (window.players?.[p]) out[p] = { name: players[p].name, color: players[p].color };
            }
            return out;
        })(),
    };
...
window.handleClientReceivedData = function(data) {
    switch (data.type) {
        ...
        case 'CAMPAIGN_INCOME_NOTICE': _clientHandleIncomeNotice(data); return;
...
function _clientHandleIncomeNotice(data) {
    if (data.campaignSnap) {
        const incomingSeq = data.campaignSnap.seq || 0;
        if (incomingSeq >= (campaignSeq || 0)) {
            campaignSeq = incomingSeq;
    ...
    _clientShowIncomeNotice(data.earned);
}
...
function _clientShowBattleResults(winnerFaction, sectorId, results) {
    const participants = campaignState.currentBattleParticipants || [];
    const n            = campaignState.numPlayers;
    const winnerColor  = COLORS['p' + winnerFaction] || '#ffffff';
...
function _clientShowIncomeNotice(earned) {
    ['cn-waiting-overlay', 'cn-result-overlay', 'campaign-summary-overlay'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.remove();
    });
    ...
    for (let p = 1; p <= n; p++) {
        ...
        <div style="color:${c}; margin:10px 0; font-size:clamp(14px, 4.5vw, 20px); font-weight:bold; border-left:4px solid ${c}; padding-left:10px; display:flex; flex-direction:column; gap:4px;">
            <div>${name}: <span style="color:#fff;">+${e}</span> <span style="color:#aaa; font-weight:normal; font-size:0.85em;">rendita</span></div>
    ...
    overlay.style.cssText = `position:fixed; top:0; left:0; width:100%; height:100%;
        background:rgba(0,0,0,0.92); z-index:99995; display:flex; align-items:center; 
    ...
    overlay.innerHTML = `
        <div style="background:rgba(0,0,10,0.98); border:3px solid #FFD700; border-radius:15px;
            ...
            <h2 style="color:#FFD700; margin:0 0 15px; font-size:clamp(22px, 6vw, 36px); text-transform:uppercase;">
                💰 RISCOSSIONE
            </h2>
            ...
            <button class="action-btn"
                style="width:100%; border:3px solid #FFD700; color:#FFD700; padding:clamp(12px, 4vw, 18px); 
                       font-size:clamp(18px, 5vw, 24px); font-weight:bold; background:rgba(0,0,0,0.5); cursor:pointer; border-radius:10px;"
                onclick="document.getElementById('cn-income-overlay').remove(); if(typeof _net_clientSend === 'function') _net_clientSend('SYNC_REQUEST', {}); _prepareMapDOM(); renderCampaignMap();">
                AVANTI ▶
            </button>
        </div>`;
    document.body.appendChild(overlay);
...
function _fixConfirmButton() {
    ...
    const actDiv = document.getElementById('campaign-actions');
    if (!actDiv) return;
    const btn = actDiv.querySelector('.action-btn:first-child');
    if (!btn) return;
    const hasOrders = (campaignState.pendingOrders?.[myPlayerNumber] || []).length > 0;
    btn.disabled = !hasOrders;
    btn.style.opacity = hasOrders ? '1' : '0.5';
