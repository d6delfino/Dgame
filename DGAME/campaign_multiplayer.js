...
function startOnlineCampaign(numPlayers) {
    ...
    const _launch = (fresh) => {
        ...
        if (!fresh && save) {
            ...
            // Broadcast immediato ai client già connessi
            setTimeout(() => {
    ...
    if (save) {
        // Mostra il dialogo di scelta sopra all'overlay di init campagna
        ...
        modal.innerHTML = `
            <div style="background:rgba(5,10,20,0.98);border:3px solid #FFD700;border-radius:12px;
                ...
                <p style="color:#aaa;font-size:13px;margin:0 0 24px;">Round ${save.turnCount} — Salvata il ${dateStr}</p>
                <div style="display:flex;flex-direction:column;gap:12px;">
                    <button id="cn-btn-resume" class="action-btn"
                        style="border:2px solid #FFD700;color:#FFD700;background:transparent;padding:14px;font-size:16px;cursor:pointer;">
                        ▶ RIPRENDI CAMPAGNA
                    </button>
                    <button id="cn-btn-new" class="action-btn"
                        style="border:2px solid #ff4444;color:#ff4444;background:transparent;padding:12px;font-size:14px;cursor:pointer;">
        ...
        modal.querySelector('#cn-btn-resume').onclick = () => {
            modal.remove();
            _launch(false);
        };
...
function _doHostBroadcast() {
    ...
    if (typeof saveCampaignSnapshot === 'function') saveCampaignSnapshot();
}
