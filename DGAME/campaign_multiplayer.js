...
function _hostHandleAction(data, fromPlayer) {
    ...
    switch (action) {
        ...
        case 'ARTILLERY_STRIKE':
            _hostApplyArtilleryStrike(payload.sectorId, fromPlayer);
            _net_broadcast({ type: 'CAMPAIGN_STATE_SYNC', state: _net_buildSnapshot() });
            return;

        case 'INFILTRATI_ATTACK':
            _hostApplyInfiltratiAttack(payload.sectorId, payload.credits, fromPlayer);
            _hostSendStateTo(fromPlayer);
            return;
...
// — registra solo l'intenzione nel _roundLog, senza applicare il danno
function _hostApplyArtilleryStrike(targetSectorId, fromPlayer) {
    const artCharges = (campaignState.artilleryCharges && campaignState.artilleryCharges[fromPlayer]) || 0;
    if (artCharges <= 0) return;
    if (!_isArtilleryReachable(targetSectorId, fromPlayer)) return;

    // Scala le cariche subito per impedire di sparare due volte
    campaignState.artilleryCharges[fromPlayer]--;

    // Registra solo l'intenzione — il danno viene applicato in processConflicts FASE 0
    if (!campaignState._roundLog) campaignState._roundLog = [];
    campaignState._roundLog.push({ type: 'ARTILLERY', p: fromPlayer, sid: targetSectorId });
}
...
function _hostValidateAndOpenSelector(targetId, fromPlayer) {
    ...
    // Calcolo raggiungibilità lato Host
    const unitReachable = _isSectorReachable(targetId, fromPlayer);
    const artReachable  = _isArtilleryReachable(targetId, fromPlayer);
    ...
    if (!unitReachable && !(artReachable && artCharges > 0) && !nukeReachable && !infiltratiReachable) {
        _hostSendStateTo(fromPlayer);
        return;
    }
    ...
    const canArtillery = artReachable && artCharges > 0;
