/* ============================================================
   ...
   MESSAGGI CLIENT → HOST  (tutti via CAMPAIGN_ACTION)
     ...
     INFILTRATI_ATTACK     attacco con upgrade infiltrati (monouso)
...
window._net_buildSnapshot = function(forPlayer) {
    ...
    // ── Ordini di movimento unità: stessa logica, visibili solo al proprietario finché non si risolve il round ──
    let filteredUnitMoves;
    if (revealAll) {
        filteredUnitMoves = _deepCopy(campaignState.pendingUnitMoves || {});
    } else {
        filteredUnitMoves = {};
        if (campaignState.pendingUnitMoves && campaignState.pendingUnitMoves[forPlayer]) {
            filteredUnitMoves[forPlayer] = _deepCopy(campaignState.pendingUnitMoves[forPlayer]);
        }
    }
    ...
    const filteredSectors = campaignState.sectors.map(s => {
        ...
        // --- LA MAGIA DELLA NEBBIA DI GUERRA ---
        // Se qualcuno lo ha sbloccato questo turno e non sono io (e non siamo a fine round)
        const isFakeBlocked = (!revealAll && s._bonifiedThisRoundBy && s._bonifiedThisRoundBy !== forPlayer);
        
        // Mente al client sulla rendita se il settore è nemico ed è in fase PLANNING
        const displayIncome = isMine ? s.income : (pre?.income ?? s.income);
        
        let outObj = {
            id: s.id, 
            owner: isFakeBlocked ? 0 : s.owner, 
            blocked: isFakeBlocked ? true : s.blocked, // Mente al client: "Ehi, è ancora bloccato!"
            income: isFakeBlocked ? 0 : displayIncome,
            terrain: s.terrain, // proprietà pubblica e permanente, nessuna nebbia di guerra
    ...
    const playersMeta = {};
    for (let p = 1; p <= campaignState.numPlayers; p++) {
        if (players && players[p]) { // rimosso "window."
            playersMeta[p] = {
                name:             players[p].name,
                color:            players[p].color,
                _cosmeticFaction: players[p]._cosmeticFaction,
                isDisconnected:   !!players[p].isDisconnected,
            };
        }
    }
    ...
    return {
        ...
        pendingUnitMoves:  filteredUnitMoves,
        ...
        playersMeta,
        onlineAIFactions:  Array.from(typeof onlineAIFactions !== 'undefined' ? onlineAIFactions : []),
        artilleryCharges:  campaignState.artilleryCharges || {},
        _roundLog:         revealAll
                                ? _deepCopy(campaignState._roundLog || [])
                                : (campaignState._roundLog || []).filter(e => e.p === forPlayer).map(e => _deepCopy(e)),
        confirmedList:     Array.from(_confirmedPlayers),
        units:             _deepCopy(campaignState.units || []),   // niente fog of war: sempre visibili a tutti
    };
};
...
window._net_applyTurnState = function() {
    ...
    // Durante setup o battaglia non mostrare nessun overlay di pianificazione
    if (window.state === 'SETUP_P1' || window.state === 'PLAYING') return;
...
window._net_showOrderSentOverlay = function() {
    ...
    if (actDiv) actDiv.querySelectorAll('button')
        ...
        .forEach(b => { 
            // SOLUZIONE UI: Nasconde il pulsante e abbassa lo Z-index per impedire click multipli
...
registerClientMessageHandler('CAMPAIGN_OPEN_CREDIT_SELECTOR', data => {
    showCreditSelector(data.sectorId, data);
});
registerClientMessageHandler('CAMPAIGN_OPEN_BONIFICA',        _clientOpenBonifica);
registerClientMessageHandler('CAMPAIGN_NUKE_ANIMATION', function(data) {
    for (const strike of (data.strikes || [])) {
        _queueCampaignAnim(() => _playNukeAnimation(strike.fromX, strike.fromY, strike.toX, strike.toY));
    }
});
registerClientMessageHandler('CAMPAIGN_ARTILLERY_ANIMATION', function(data) {
    for (const strike of (data.strikes || [])) {
        _queueCampaignAnim(() => _playArtilleryAnimation(strike.fromX, strike.fromY, strike.toX, strike.toY));
    }
});
registerClientMessageHandler('CAMPAIGN_MINEFIELD_ANIMATION', function(data) {
    for (const strike of (data.strikes || [])) {
        _queueCampaignAnim(() => _playMinefieldAnimation(strike.fromX, strike.fromY, strike.toX, strike.toY));
    }
});
registerClientMessageHandler('CAMPAIGN_REVOLT_ANIMATION', function(data) {
    if (data.sectors && data.sectors.length > 0) {
        _queueCampaignAnim(() => _playRevoltAnimation(data.sectors));
    }
});
registerClientMessageHandler('CAMPAIGN_INFILTRATI_ANIMATION', function(data) {
    for (const strike of (data.strikes || [])) {
        _queueCampaignAnim(() => _playInfiltratiAnimation(strike.fromX, strike.fromY, strike.toX, strike.toY));
    }
});
registerClientMessageHandler('CAMPAIGN_CONFLICT_SUMMARY', async function(data) {
    await _waitCampaignAnimations();
    _clientHandleConflictSummary(data);
});
...
window.setupHostConnection = function(c, playerNum) {
    ...
    // --- RICONNESSIONE: ripristina stato normale ---
    if (isCampaignOnline && isHost && campaignState.isActive) {
        if (players[playerNum]) players[playerNum].isDisconnected = false;
        console.log(`[Campaign] P${playerNum} riconnesso.`);
    ...
    // --- DISCONNESSIONE DURANTE PLANNING ---
    // Se il giocatore si disconnette, viene marcato e auto-confermato
    // così gli altri non restano bloccati ad aspettare.
    c.on('close', () => {
        if (!isCampaignOnline || !isHost || !campaignState.isActive) return;
        if (players[playerNum]) players[playerNum].isDisconnected = true;

        console.log(`[Campaign] P${playerNum} disconnesso. Auto-confermato.`);

        // Solo in fase di planning: auto-conferma per sbloccare gli altri
        if (campaignState.phase === 'PLANNING' && !_confirmedPlayers.has(playerNum)) {
            _hostApplyConfirm(playerNum, true); // isSkip=true: pensa al rimborso internamente
        }

        // Aggiorna la mappa per tutti (mostra la X rossa)
    });
};
...
function startOnlineCampaign(numPlayers) {
    if (!isOnline || !isHost) {
        showTemporaryMessage("⚠️ SOLO L'HOST PUÒ AVVIARE LA CAMPAGNA!", 3500);
        return;
    }
...
function _hostHandleAction(data, fromPlayer) {
    ...
    switch (action) {
        ...
        case 'INFILTRATE_UNIT':
            infiltrateCampaignUnit(payload.unitId, payload.destSectorId, fromPlayer);
            _hostSendStateTo(fromPlayer);
            return;
        ...
        case 'CREATE_UNIT':
            _hostApplyCreateUnit(payload.sectorId, payload.value, payload.unitType, fromPlayer);
            _hostSendStateTo(fromPlayer);
            return;

        case 'MOVE_UNIT':
            moveCampaignUnit(payload.unitId, payload.destSectorId, fromPlayer);
            _hostSendStateTo(fromPlayer);
            return;

        case 'UNIT_ARTILLERY_FIRE':
            fireCampaignUnitArtillery(payload.unitId, payload.targetSectorId, fromPlayer);
            _hostSendStateTo(fromPlayer);
            return;
...
/** Valida e applica la creazione di un'unità richiesta da un client. */
function _hostApplyCreateUnit(sectorId, value, unitType, fromPlayer) {
    const sector = campaignState.sectors[sectorId];
    const playerCredits = campaignState.credits[fromPlayer] || 0;
    const v = parseInt(value);

    // VALIDAZIONE: settore esistente e di proprietà del richiedente, valore intero positivo, fondi sufficienti
    if (!sector || sector.owner !== fromPlayer || !Number.isInteger(v) || v < 1 || playerCredits < v) {
        console.warn(`[Host] Creazione unità negata a P${fromPlayer}: settore ${sectorId}, valore ${value}`);
        _hostSendStateTo(fromPlayer);
        return;
    }
    unitType = 'agent'; // l'unità Artiglieria non si crea più da qui: nasce comprando l'upgrade di settore

    campaignState.credits[fromPlayer] -= v;
    createCampaignUnit(sectorId, fromPlayer, v, unitType);
}
...
function _hostApplyConfirm(fromPlayer, isSkip) {
    ...
    // 1. Aggiungi al set logico
    _confirmedPlayers.add(fromPlayer);
    
    // 2. FIX CRITICO: Sincronizza lo stato locale dell'Host per la UI
    campaignState._confirmedList = Array.from(_confirmedPlayers);
    ...
    if (_allPlayersConfirmed()) {
        ...
        // 3. Notifica gli altri client e aggiorna la mappa locale dell'Host
        _doHostBroadcast(); 
        renderCampaignMap(); 
    }
}
...
function _allPlayersConfirmed() {
    for (let p = 1; p <= campaignState.numPlayers; p++) {
        ...
        const isDisconnected = !!players[p]?.isDisconnected;
        const isAI = typeof onlineAIFactions !== 'undefined' && onlineAIFactions.has(p);
        
        // Salta: nessun settore, oppure disconnesso, oppure controllato dall'AI 
        // (L'AI gioca le battaglie tattiche, ma salta la pianificazione strategica)
        if (!isActive || isDisconnected || isAI) continue;
        
        if (!_confirmedPlayers.has(p)) return false;
    }
    return true;
}
...
function _hostApplyUpgrade(sectorId, upgradeKey, cost, fromPlayer) {
    ...
    // 1. VALIDAZIONE: Il settore esiste? Il giocatore ha i soldi?
    if (!sector || playerCredits < cost) {
        console.warn(`[Host] Upgrade negato a P${fromPlayer}: Crediti insufficienti (${playerCredits}/${cost})`);
        _hostSendStateTo(fromPlayer); // Risincronizza il client per correggere la sua UI
        return;
    }

    // 2. CASO SPECIALE: Bonifica
    if (upgradeKey === 'bonifica') {
        ...
        sector._bonifiedThisRoundBy = fromPlayer;
    ...
    // 3. UPGRADE STANDARD: Il giocatore è il proprietario?
    if (sector.owner !== fromPlayer) {
        console.warn(`[Host] Upgrade negato a P${fromPlayer}: Non è il proprietario del settore ${sectorId}`);
        return;
    }

    // 4. ESECUZIONE: Sottrae una volta sola e applica lo stato
    campaignState.credits[fromPlayer] -= cost;
    _applySectorUpgradeLocale(fromPlayer, sectorId, upgradeKey, cost);
    
    // Notifica tutti i client del cambio di stato economico e strutturale
    _hostSendStateTo(fromPlayer);
}
...
function _hostApplySabotage(targetSectorId, fromPlayer) {
    ...
    // CORREZIONE: Rimossa la restrizione "target.owner <= 0". 
    // Ora i client possono nuclearizzare i settori neutrali proprio come l'Host.
    if (!target || target.owner === fromPlayer) return;

    // CONTROLLO RIGOROSO: Il giocatore ha sbloccato le armi nucleari?
    const bonuses = getPlayerCampaignBonuses(fromPlayer);
    if (!bonuses.nukeUnlocked) {
        console.warn(`[Host] P${fromPlayer} ha tentato Nuke senza Silo!`);
        return; 
    }

    // Costo fisso (rimossa vecchia logica della specializzazione)
    const realCost = CAMPAIGN.NUCLEARIZE_COST;
...
function _hostApplyInfiltratiAttack(targetSectorId, credits, fromPlayer) {
    // Validazione: deve avere l'upgrade e il settore deve essere raggiungibile
    if (!_isInfiltratiReachable(targetSectorId, fromPlayer)) {
        console.warn(`[Host] P${fromPlayer} ha tentato Infiltrati senza l'upgrade!`);
        return;
    }
    if ((campaignState.credits[fromPlayer] || 0) < credits) return;

    // Consuma l'upgrade (monouso)
    const sourceSector = campaignState.sectors.find(
        s => s.owner === fromPlayer && s.infiltratiUpgrade === true
    );
    if (!sourceSector) return;
    sourceSector.infiltratiUpgrade = false;

    // Registra l'ordine con coordinate sorgente per l'animazione
    _applyOrderWithCredits(targetSectorId, credits, fromPlayer, {
        isInfiltrati: true,
        fromX: sourceSector.x,
        fromY: sourceSector.y,
    });
}
...
function _hostValidateAndOpenSelector(targetId, fromPlayer) {
    ...
    if (target.blocked) {
        if (target._nuclearized) { _hostSendStateTo(fromPlayer); return; }
        const isAdj = campaignState.adj[targetId]?.some(nbId => campaignState.sectors[nbId]?.owner === fromPlayer);
        if (!isAdj) { _hostSendStateTo(fromPlayer); return; }
        const conn = clientConns[fromPlayer];
        if (conn?.open) {
            try { conn.send({ type: 'CAMPAIGN_OPEN_BONIFICA', sectorId: targetId, availCredits: campaignState.credits[fromPlayer] || 0 }); } catch(e) {}
        }
        return;
    }
    ...
    // Calcolo raggiungibilità lato Host (l'attacco classico non esiste più: attaccano le unità)
    const nukeReachable = _isNukeReachable(targetId, fromPlayer);
    const infiltratiReachable = _isInfiltratiReachable(targetId, fromPlayer);

    if (!nukeReachable && !infiltratiReachable) {
        _hostSendStateTo(fromPlayer);
        return;
    }
    ...
    const canSabotage = nukeReachable && (campaignState.credits[fromPlayer] || 0) >= nukeCost;
    // FIX: rimossi canArtillery/artCharges/artReachable — codice morto residuo di quando
    // l'artiglieria passava da questo selettore a crediti. `artReachable` non era mai
    // definita: il ReferenceError bloccava la funzione PRIMA dell'invio del messaggio al
    // client, quindi il popup Nuke/Infiltra non si apriva mai online. L'artiglieria oggi
    // è gestita come unità persistente tramite fireCampaignUnitArtillery, non più da qui.
    ...
    if (conn?.open) {
        try {
            conn.send({
                ...
                infiltratiPossible: infiltratiReachable,
...
function _clientHandleStateSync(data) {
    ...
    // FIX: Rimuove il selettore colore rimasto appeso dopo la riconnessione
...
function _clientHandleBattleStart(data) {
    ...
    // FIX CRITICO: Aggiunto 'cn-result-overlay' per distruggere risultati di battaglie precedenti
    ['cn-income-overlay','eco-credit-modal','cn-order-sent-overlay',
     'campaign-summary-overlay', 'cn-result-overlay'].forEach(id => document.getElementById(id)?.remove());
    ...
    for (let p = 1; p <= window.totalPlayers; p++) {
        ...
        // RIMOSSA LA MANOMISSIONE DELLA RETE
    ...
    if (isParticipant) {
        ...
        // Modalità spettatore: riceve il GAME_STATE come tutti,
        // ma _applySpectatorGameState lo gestisce senza avviare il setup.
        window._isSpectating = true;
        window._spectatedSectorId = data.sectorId;
        _showSpectatorBanner(data.sectorId, data.factions);
    }
}
...
function _clientHandleBattleResult(data) {
    ...
    // Pulizia stato spettatore
    window._isSpectating = false;
    window._spectatedSectorId = null;
    document.getElementById('cn-spectator-banner')?.remove();
    const ctrl = document.getElementById('controls-panel');
    if (ctrl) { ctrl.style.pointerEvents = 'auto'; ctrl.style.opacity = '1'; }
...
window.finishPlayerTurn = function() {
    ...
    if (!isHost) {
        ...
        window._clientLockedRound = campaignState.turnCount;  // ← stessa variabile del client
        _net_showOrderSentOverlay();
        _hostSelfConfirm();
    }
    renderCampaignMap(); // Ridisegna la mappa per nascondere i pulsanti +/-
};
...
window._doStartNextPlanningRound = function() {
    ...
    campaignState._confirmedList = [];
    
    // PULIZIA UI HOST: Sicurezza extra all'inizio di ogni nuovo round
    document.getElementById('cn-order-sent-overlay')?.remove();
    
    // FIX CRITICO: Assicurati che l'Host torni correttamente allo stato mappa dopo le battaglie
    window.state = 'CAMPAIGN_MAP';
    ['setup-overlay','controls-panel','gameover-overlay'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.style.display = 'none';
    });
        ...
        <div style="background:rgba(10,15,30,0.98);border:3px solid ${winnerColor};border-radius:15px;
            ...
            <button class="action-btn"
                ...
                onclick="if (typeof playSFX === 'function') playSFX('click');
                         ...
                         if (window.state !== 'SETUP_P1' && window.state !== 'PLAYING') {
...
// rimossa e unificata con l'host
...
function _doHostBroadcast() {
    ...
    if (typeof clientConns !== 'undefined') {
        Object.entries(clientConns).forEach(([pNumStr, conn]) => {
            ...
            if (conn?.open) {
                safeSend(conn, { type:'CAMPAIGN_STATE_SYNC', state:_net_buildSnapshot(pNum) }, `Host -> P${pNum}`);
            }
        });
    }
...
function _hostSendStateTo(playerNum) {
    ...
    if (c?.open) {
        safeSend(c, { type:'CAMPAIGN_STATE_SYNC', state:_net_buildSnapshot(playerNum) }, `Host -> P${playerNum} (Singolo)`);
    }
}
...
function _applySnapshot(snap) {
    ...
    if (snap.onlineAIFactions) {
        onlineAIFactions = new Set(snap.onlineAIFactions);
    }

    Object.assign(campaignState, {
        isActive:                   snap.isActive,
        hqSlots:                    snap.hqSlots           || [],
        numPlayers:                 snap.numPlayers,
        currentPlayer:              snap.currentPlayer,
        credits:                    snap.credits           || {},
        victoryThreshold:           snap.victoryThreshold  || CAMPAIGN.VICTORY_THRESHOLD,
        phase:                      snap.phase,
        turnCount:                  snap.turnCount         || 1,
        pendingMoves:               snap.pendingMoves      || {},
        pendingOrders:              snap.pendingOrders     || {},
        pendingUnitMoves:           snap.pendingUnitMoves  || {},
        ...
        _roundLog:                  snap._roundLog || [],
        _confirmedList:             snap.confirmedList || [],
    ...
    if (snap.sectors?.length > 0) {
        ...
        snap.sectors.forEach(ss => {
            ...
            _nuclearCooldown: ss._nuclearCooldown || 0,
        ...
        if (ss.terrain !== undefined) s.terrain = ss.terrain;
        });
    }

    if(snap.artilleryCharges) {
        campaignState.artilleryCharges = snap.artilleryCharges;
    }

    if (snap.playersMeta && typeof players !== 'undefined') {
        Object.entries(snap.playersMeta).forEach(([p, meta]) => {
            const pNum = Number(p);
            if (!players[pNum]) players[pNum] = {};
            if (meta.name)             players[pNum].name             = meta.name;
            if (meta.color)            players[pNum].color            = meta.color;
            if (meta._cosmeticFaction) players[pNum]._cosmeticFaction = meta._cosmeticFaction;
            if (meta._cosmeticFaction) players[pNum]._colorConfirmed  = true;
            players[pNum].isDisconnected = !!meta.isDisconnected;
        });
    }
}
        ...
        <div id="setup-header">
            ...
            <div style="display:flex; justify-content:center; align-items:center; gap:15px; margin-bottom:10px;">
                <div id="setup-points-display">
                    <span id="pts-count" style="font-weight:bold;font-size:1.4em">10</span>
                </div>
                <button class="action-btn p1-theme" onclick="addNewAgentToMarket()"
                        style="font-size:12px;padding:10px 20px;margin:0;">
                    + Nuovo Agente (Costo: ${GAME.AGENT_COST})
                </button>
            </div>
        </div>
        <div id="agents-market"></div>
        <div id="card-selection-panel"></div>
        <div id="setup-footer" style="display:flex; gap:10px; align-items:stretch; width:100%;">
            <button class="action-btn p1-theme" id="confirm-setup-btn" onclick="confirmPlayerSetup()"
                    style="flex:1;font-size:16px;padding:15px;margin:0;">
                PRONTO
            </button>
        </div>`;
}
...
function _fixConfirmButton() {
    ...
    // RIPRISTINO UI: Riporta il pulsante visibile e allo z-index normale
    btn.style.display     = ''; 
...
// ============================================================
// SPETTATORE — funzioni di supporto
// ============================================================

function _showSpectatorBanner(sectorId, factions) {
    document.getElementById('cn-spectator-banner')?.remove();
    const fNames = factions.map(f => players[f]?.name || 'P' + f).join(' vs ');
    const banner = document.createElement('div');
    banner.id = 'cn-spectator-banner';
    banner.style.cssText = `
        position:fixed; top:0; left:0; width:100%; z-index:200100;
        background:rgba(0,0,0,0.78); border-bottom:2px solid #cc00ff;
        display:flex; align-items:center; justify-content:center; gap:16px;
        padding:8px 16px; box-sizing:border-box;
        font-family:'Courier New',monospace; font-size:13px; color:#cc00ff;
        pointer-events:none;
    `;
    banner.innerHTML = `
        <span style="font-size:18px;">👁️</span>
        <span style="font-weight:bold; letter-spacing:1px;">SPETTATORE</span>
        <span style="color:#888;">|</span>
        <span style="color:#fff;">Settore ${sectorId}</span>
        <span style="color:#888;">|</span>
        <span style="color:#aaa;">${fNames}</span>
        <span style="color:#FFD700; margin-left: 15px; font-size: 11px;">(In attesa del setup...)</span>
    `;
    document.body.appendChild(banner);
}

function _applySpectatorGameState(data) {
    // Applica tema visivo
    if (data.state.themeId) {
        const t = bgOptions.find(t => t.id === data.state.themeId);
        if (t) applyTheme(t);
    }
    ...
    // Ricostruisce mappa e agenti
    receiveGameState(data.state);
    window.state = 'PLAYING';

    // Mostra canvas, nasconde e disabilita i controlli
