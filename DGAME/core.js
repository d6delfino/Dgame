...
function playSpecialVFX(target, color, text) {
    ...
    slot.el.style.textShadow = `0 0 12px ${color},0 0 4px #000`;
...
function createParticles(x, y, color, amount = 10, isExplosion = false) {
    for (let i = 0; i < amount; i++) {
        ...
        particles.push({
            x: x, y: y,
            px: x, py: y, // <--- Aggiunto: memorizza la posizione di partenza
            vx: Math.cos(angle) * speed,
            vy: Math.sin(angle) * speed,
            life: 1.0,
            decay: Math.random() * 0.02 + 0.01, // Decadimento leggermente più rapido per 30fps
            color: color,
            size: Math.random() * 8 + 4 // Particelle leggermente più fini sono più fluide
        });
    }
}
...
function updateAndDrawParticles() {
    for (let i = particles.length - 1; i >= 0; i--) {
        ...
        // 1. Salva la posizione attuale come "precedente" per il prossimo frame
        p.px = p.x;
        p.py = p.y;

        // 2. Applica spostamento
        p.x += p.vx;
        p.y += p.vy;

        // 3. Applica attrito costante (0.92 è più "morbido" a 30fps)
        p.vx *= 0.94;
        p.vy *= 0.94;
        ...
        if (p.life <= 0) {
            ...
            ctx.strokeStyle = p.color; // Passiamo da fill a stroke
            ...
            ctx.beginPath();
            // Trucco per la fluidità: Disegna una scia dalla posizione precedente a quella attuale
...
function vfxRenderLoop(timestamp) {
    ...
    if (state === 'PLAYING' || state === 'CAMPAIGN_MAP') {
        ...
        // Disegna il fendente Melee (Mezzaluna dinamica)
        for (let i = activeSlashes.length - 1; i >= 0; i--) {
            let slash = activeSlashes[i];
            
            // Velocità lenta per godersi il movimento
            slash.progress += 0.09; 

            if (slash.progress >= 1.0) {
                activeSlashes.splice(i, 1);
            } else if (slash.progress > 0) {
                
                // Genera particelle solo nel momento esatto dell'impatto (metà animazione)
                if (!slash.particlesSpawned && slash.progress > 0.3) {
                    // Usa slash.color (fazione) invece del bianco
                ...
                // --- LOGICA STREAK (La lama che scorre) ---
                // Calcoliamo l'inizio e la fine della linea in base al tempo
                // Questo crea l'effetto "taglio" che viaggia nello spazio
                const fullLength = HEX_SIZE * 2.2;
                
                // La "testa" della lama corre avanti
                const head = -fullLength + (fullLength * 2 * Math.min(1, slash.progress * 1.5));
                // La "coda" della lama la segue con un leggero ritardo
                const tail = -fullLength + (fullLength * 2 * Math.max(0, (slash.progress - 0.25) * 1.5));
                
                // Opacità: splende al centro, svanisce ai bordi
                const alpha = Math.sin(slash.progress * Math.PI);

                ctx.globalAlpha = alpha;
                ctx.lineCap = 'round';
                
                // 1. GLOW ESTERNO (Colore fazione)
                ctx.shadowBlur = 20;
                ...
                // 2. ANIMA BIANCA (Effetto velocità)
                ctx.shadowBlur = 0;
                ctx.strokeStyle = '#ffffff';
                ...
                // 3. FLASH DI IMPATTO (Solo al centro dell'esagono)
    ...
    setTimeout(() => requestAnimationFrame(vfxRenderLoop), 1000 / (_isMobile ? 15 : 15));
}

const _isMobile = /Mobi|Android|iPhone|iPad|iPod|Tablet/i.test(navigator.userAgent);
// Parte in automatico
