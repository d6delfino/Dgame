...
let waterCells = [];            // Cache per le celle d'acqua animate
...
// --- GIOCATORI ---
let players = {
    1: { hq: null, agents: [], color: COLORS.p1, name: 'Verde',  credits: 0, _cosmeticFaction: 1 },
    2: { hq: null, agents: [], color: COLORS.p2, name: 'Viola',  credits: 0, _cosmeticFaction: 2 },
    3: { hq: null, agents: [], color: COLORS.p3, name: 'Blu',    credits: 0, _cosmeticFaction: 3 },
    4: { hq: null, agents: [], color: COLORS.p4, name: 'Oro',    credits: 0, _cosmeticFaction: 4 },
    5: { hq: null, agents: [], color: COLORS.p5, name: 'Rosso',  credits: 0, _cosmeticFaction: 5 },
    6: { hq: null, agents: [], color: COLORS.p6, name: 'Bianco', credits: 0, _cosmeticFaction: 6 },
    7: { hq: null, agents: [], color: COLORS.p7, name: 'Grigio', credits: 0, _cosmeticFaction: 7 },
    8: { hq: null, agents: [], color: COLORS.p8, name: 'Rosa',   credits: 0, _cosmeticFaction: 8 },
};
...
function applyTheme(themeObj) {
    ...
    for (let i = 1; i <= THEME_WALL_COUNT; i++) {
        ...
        if (!customImages[key]) {
            ...
            customImages[key].addEventListener('load', () => invalidateStaticLayer());
            customImages[key].src = url;
        }
    }
}
...
function preloadFixedSprites() {
    ...
    // 4. Precarica le barricate specifiche per ogni fazione
    const barricadePrefixes = ['EC', 'ZE', 'ME', 'GU', 'HE', 'AL', 'RO'];
    barricadePrefixes.forEach(prefix => {
        const key = `${prefix}1`;
        const url = `img/${key}.png`;
        customSpriteFiles[key] = url;
        if (!customImages[key]) {
    ...
    // --- NUOVO: 5. Precarica Immagini Terreno Campagna ---
    const terrainImages = ['camp-forest.png', 'camp-sand.png', 'camp-mountain.png', 'camp-sea.png'];
    terrainImages.forEach(filename => {
        const url = `img/${filename}`;
        customSpriteFiles[filename] = url; 
        if (!customImages[filename]) {
            customImages[filename] = new Image();
            customImages[filename].src = url;
        }
    });
}

// --- HELPER BARRICATE DI FAZIONE ---
function getFactionBarricadeId(factionId) {
    if (!players || !players[factionId]) return THEME_BARRICADE_ID || 'EC1';
    const cosmetic = players[factionId]._cosmeticFaction || factionId;
    const prefixMap = {
        1: 'EC', // Verde
        2: 'ZE', // Viola
        3: 'ME', // Blu
        ...
        6: 'AL', // Bianco
        7: 'RO', // Grigio
        8: 'ME'  // Rosa (Usa il muro Blu)
    };
    return (prefixMap[cosmetic] || 'EC') + '1';
}
...
// --- NUOVO: Centra la telecamera sull'HQ del giocatore ---
window.focusCameraOnLocalPlayer = function() {
    // In Coop la telecamera è già gestita da uno zoom cinematico specifico, non interferiamo
    if (typeof coopState !== 'undefined' && coopState.active) return;

    // In locale usiamo chi ha il turno iniziale, online la fazione di chi guarda lo schermo
    const pNum = (typeof isOnline !== 'undefined' && isOnline && myPlayerNumber > 0) ? myPlayerNumber : currentPlayer;
    ...
    // Priorità 1: Quartier Generale
    if (pData.hq && pData.hq.hp > 0) {
        targetQ = pData.hq.q;
        targetR = pData.hq.r;
        found = true;
    } 
    // Priorità 2: Il primo agente vivo (se l'HQ è distrutto o assente)
    else if (pData.agents && pData.agents.length > 0) {
        const agent = pData.agents.find(a => a.hp > 0);
        if (agent) {
            targetQ = agent.q;
            targetR = agent.r;
            found = true;
    ...
    if (found) {
        // Leggero zoom tattico all'inizio della partita su Desktop per maggiore immersività
        if (window.innerWidth > 900 && HEX_SIZE < 40) {
            HEX_SIZE = 40; 
            invalidateStaticLayer(); // Dobbiamo ridisegnare i muri se cambiamo lo zoom
        }
        
        // Calcolo matematico per mettere l'esagono (q, r) esattamente al centro dello schermo
        const _SQRT3 = Math.sqrt(3);
        offsetX = -(HEX_SIZE * (_SQRT3 * targetQ + (_SQRT3 / 2) * targetR));
        offsetY = -(HEX_SIZE * (1.5 * targetR));
        clampCamera();
    }
};
...
function resizeCanvas() {
    ...
    // --- MODIFICA: Forza ricalcolo del layer statico e vincoli telecamera ---
    invalidateStaticLayer();
    clampCamera();
    
    // drawGame ora intercetta anche altre visualizzazioni e si affida al Master Loop a 15FPS
    if (typeof state !== 'undefined' && (state === 'PLAYING' || state === 'CAMPAIGN_MAP')) drawGame();
}

let _selectionGlowCache = {};
function _getSelectionGlow(color) {
    const key = color + '_' + Math.round(HEX_SIZE);
    if (_selectionGlowCache[key]) return _selectionGlowCache[key];
    const r = HEX_SIZE * 1.1;
    const grad = ctx.createRadialGradient(0, 0, 0, 0, 0, r);
    grad.addColorStop(0,    color + 'ff');
    grad.addColorStop(0.4,  color + 'cc');
    grad.addColorStop(0.85, color + '55');
    grad.addColorStop(1,    color + '00');
    _selectionGlowCache[key] = grad;
    return grad;
}
...
// --- STATIC LAYER: ridisegna solo se invalidato (ora include anche sfondo + overlay) ---
    if (_staticDirty) {
        _staticCanvas.width  = canvas.width;
        _staticCanvas.height = canvas.height;
        
        waterCells = []; // <-- SVUOTA LA CACHE DELL'ACQUA
        ...
        // --- SFONDO + OVERLAY: ora disegnati UNA VOLTA SOLA per rebuild, non più ogni frame ---
        if (bgOffscreenReady) {
        ...
        grid.forEach(cell => {
            ...
            if (cell.type === 'wall')           fill = '#777';
                ...
                waterCells.push(cell); // <-- SALVA LA CELLA NELLA CACHE
            }
    ...
    // Incolla il layer statico (sfondo + overlay + muri/acqua/terreni) in una sola drawImage
    ctx.save();
    ctx.setTransform(1, 0, 0, 1, 0, 0);
    ...
    const _now = Date.now();
    waterCells.forEach(cell => { // <-- USA LA CACHE INVECE DI TUTTA LA GRIGLIA
        const p     = hexToPixel(cell.q, cell.r);
    ...
    // --- PASSATA UNIFICATA: Entità, Terreni e HP Ostacoli (Z-Order rigoroso) ---
    grid.forEach(cell => {
        const hasEntity      = !!cell.entity;
        ...
        // MASTER GUARD: Se la casella è semplice terra vuota, salta istantaneamente
        if (!hasEntity && !hasHpIcon && !hasTerrainIcon) return;

        const p = hexToPixel(cell.q, cell.r);

        // 1. LIVELLO BASE: Entità (Agente o HQ) e i suoi HP
        if (hasEntity) {
            let isVisible = true;
            // Gestione Nebbia Modalità Coop
            if (typeof coopState !== 'undefined' && coopState.active &&
                ...
                if (!coopState.currentVisible || !coopState.currentVisible.has(_key)) isVisible = false;
            }
            ...
            if (isVisible) {
                ...
                // --- ALONE DI SELEZIONE (Glow) ---
                // Appare sia per Agenti che per HQ se selezionati
                if (cell.entity === selectedAgent) {
                    ...
                    ctx.fillStyle   = _getSelectionGlow(fColor);
                    ctx.beginPath();
                    // L'alone dell'HQ è leggermente più grande (2.2) rispetto all'agente (1.9)
                    const auraRadius = cell.entity.type === 'hq' ? 2.2 : 1.9;
                    ctx.arc(0, 0, HEX_SIZE * auraRadius, 0, Math.PI * 2);
                    ctx.fill();
                    ctx.restore();
                }
                ...
                if (cell.entity.type === 'agent') {
                    ...
                    // Gli agenti pulsano anche dimensionalmente
                    if (cell.entity === selectedAgent) {
                ...
                // HP Entità (Disegnati indipendentemente per non ereditare scale/pulsazioni)
                ctx.font      = `bold ${Math.round(HEX_SIZE * 0.4)}px Courier New`;
                ctx.textAlign = 'center';
                // Bordatura nera per leggibilità
                //ctx.strokeStyle = '#000';
                //ctx.lineWidth = 3;
                //ctx.strokeText(cell.entity.hp, drawX, drawY - HEX_SIZE * 0.62);
                ctx.fillStyle = '#fff';
                ctx.fillText(cell.entity.hp, drawX, drawY - HEX_SIZE * 0.62);
            }
        }

        // 2. LIVELLO INTERMEDIO: Icona Terreno (Disegnata DOPO l'entità, così rimane sopra)
        if (hasTerrainIcon) {
            const xOff = hasEntity ? HEX_SIZE * 0.45 : 0;
            const yOff = hasEntity ? HEX_SIZE * 0.65 : HEX_SIZE * 0.20;
            ctx.save();
        ...
        // 3. LIVELLO SUPERIORE: HP degli ostacoli (Muri e Barricate)
        if (hasHpIcon) {
            const xOff = hasEntity ? HEX_SIZE * 0.38 : 0;
            const yOff = hasEntity ? HEX_SIZE * 0.55 : HEX_SIZE * 0.62;
            ctx.font      = `bold ${Math.round(HEX_SIZE * 0.45)}px Courier New`;
            // Bordatura nera per leggibilità
            //ctx.strokeStyle = '#000';
            //ctx.lineWidth = 3;
            //ctx.strokeText(cell.hp, p.x + xOff, p.y + yOff);
            ctx.fillStyle = '#fff';
...
function showTemporaryMessage(text, duration = 3000, silent = false) {
    ...
    if (!el) {
        ...
        el.style.cssText = `
            ...
            border-radius: 4px; z-index: 500000; text-align: center; pointer-events: none;
    ...
    if (!silent && typeof playSFX === 'function') playSFX('shield');
...
function createParticles(x, y, color, amount = 10, isExplosion = false) {
    for (let i = 0; i < amount; i++) {
        ...
        // La velocità scala in proporzione all'esagono (0.4 per esplosioni, 0.2 per scintille)
        const speed = Math.random() * (isExplosion ? (HEX_SIZE * 0.4) : (HEX_SIZE * 0.2));
        particles.push({
            x: x, y: y, px: x, py: y,
            ...
            color: color,
            // La dimensione scala in proporzione all'esagono (tra il 15% e il 40% di HEX_SIZE)
            size: HEX_SIZE * (Math.random() * 0.25 + 0.15)
        });
    }
}
