...
function drawGame() {
    ...
    grid.forEach(cell => {
        ...
        if (cell.type === 'water') {
            ...
            const now = Date.now();
            // Ondulazione: opacità e fill oscillano sfasati per cella
            const wave    = Math.sin(now / 900 + cell.q * 0.8 + cell.r * 0.5);
            const waveX   = Math.sin(now / 2200 + cell.r * 0.7 + cell.q * 0.3);
            const alpha   = 0.15 + wave * 0.10;
            const yOffset = wave  * HEX_SIZE * 0.12;
            const xOffset = waveX * HEX_SIZE * 0.08;

            // Ridisegna il fill con alpha dinamico sopra quello statico
            ctx.save();
            ...
            ctx.restore(); // Oppure usa 🕳️ se vuoi fare un "crepaccio"
    ...
    controlPoints.forEach(cp => {
        ...
        // Pulsazione alone per CP neutri
        if (cp.faction === 0) {
    ...
    grid.forEach(cell => {
        ...
        // 1. SCIA DI MOVIMENTO (Motion Blur) e Lerp
        let drawX = p.x;
        let drawY = p.y;
        ...
        if (cell.entity._moveTime && (now - cell.entity._moveTime < 300)) {
            ...
            // Calcolo posizione intermedia fluida
            drawX = oldP.x + (p.x - oldP.x) * progress;
            drawY = oldP.y + (p.y - oldP.y) * progress;
            
            // Disegna i "fantasmi" della scia
        ...
        // 2. RESPIRO E SQUASH (Solo per gli Agenti)
        if (cell.entity.type === 'agent') {
            // Respiro base (Oscillazione sinusoidale basata sul tempo)
            let scaleX = 1 + Math.sin(now / 300 + cell.entity.q) * 0.03;
            let scaleY = 1 - Math.sin(now / 300 + cell.entity.q) * 0.04;

            // PULSAZIONE AGENTE SELEZIONATO: +15% uniforme con oscillazione visibile
            if (cell.entity === selectedAgent) {
                const pulse = 1.35 + Math.sin(now / 200) * 0.08;
                scaleX *= pulse;
                scaleY *= pulse;
                // Glow del colore di fazione
                const fColor = players[cell.entity.faction]?.color || '#ffffff';
                ctx.shadowBlur  = 18 + Math.sin(now / 200) * 8;
                ctx.shadowColor = fColor;
            }
            
            // 3. EFFETTO IMPATTO (Hit Flash & Squash Estremo)
        ...
        // DISEGNO FINALE DELL'IMMAGINE
        if (img?.complete && img.naturalWidth !== 0) {
            const mult    = cell.entity.type === 'hq' ? 1.5 : 1.2;
            const imgSize = HEX_SIZE * mult;
            // Traslato il centro perché abbiamo già fatto ctx.translate
    ...
    // --- NUOVO: CROCE MEDICA IN PRIMISSIMO PIANO ---
    if (currentActionMode === 'heal') {
        validActionTargets.forEach(t => {
            const p = hexToPixel(t.q, t.r);
            ctx.save();
            ctx.fillStyle = 'rgba(0, 255, 136, 0.95)'; // Verde leggermente trasparente
            ctx.font = `bold ${Math.round(HEX_SIZE * 1.2)}px Arial`; // Croce enorme
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle'; // Centra perfettamente in verticale
            ctx.shadowBlur = 15;
            ctx.shadowColor = '#00ff88'; // Glow verde intenso
...
function drawLaserBeam(attacker, victim) {
    ...
    // Aggiunge semplicemente il laser al motore VFX a 60FPS
    activeLasers.push({
        p1: { x: p1.x, y: p1.y },
        p2: { x: p2.x, y: p2.y },
        life: 1.0
...
function drawMeleeSlash(attacker, victim) {
    ...
    // Calcoliamo l'angolo di rotazione per far puntare il fendente verso il bersaglio
    const dx = p2.x - p1.x;
    const dy = p2.y - p1.y;
    const angle = Math.atan2(dy, dx);
    
    // Il fendente viene disegnato a metà strada tra attaccante e vittima
    ...
    activeSlashes.push({
        ...
        angle: angle,
        life: 1.0,
        color: attacker.faction ? players[attacker.faction].color : '#ffffff'
    });
}
...
// Avvolge drawGame per eseguire i draw hook registrati dai moduli
// (carduse.js ecc.). Va installato DOPO che drawGame è definita.
...
// ============================================================
// VFX ENGINE: PARTICELLE E RENDER LOOP (60 FPS)
// ============================================================
...
function createParticles(x, y, color, amount = 10, isExplosion = false) {
    for (let i = 0; i < amount; i++) {
        ...
        // --- VELOCITÀ INIZIALE AUMENTATA ---
        // Portata a 6 per i colpi normali e 12 per le esplosioni di morte
        ...
        particles.push({
            ...
            life: 1.0,
            // Decadimento leggermente più lento per permettere il viaggio lungo
...
function updateAndDrawParticles() {
    for (let i = particles.length - 1; i >= 0; i--) {
        ...
        // --- FRIZIONE RIDOTTA ---
        // Passando da 0.95 a 0.98 le particelle mantengono l'inerzia molto più a lungo,
        // coprendo una distanza molto più ampia sulla mappa.
        p.vx *= 0.98;
        p.vy *= 0.98;
...
// Avviamo un loop a 60FPS per gestire respiro e particelle
function vfxRenderLoop() {
    if (state === 'PLAYING' || state === 'CAMPAIGN_MAP') {
        drawGame(); 
        
        // Disegna i laser in corso
        for (let i = activeLasers.length - 1; i >= 0; i--) {
            let laser = activeLasers[i];
            laser.life -= 0.025; // Velocità di dissolvenza
        ...
        // Disegna gli slash Melee (Spada)
        for (let i = activeSlashes.length - 1; i >= 0; i--) {
            let slash = activeSlashes[i];
            
            // --- VELOCITÀ DIMEZZATA/TRIPLICATA (da 0.08 a 0.025) ---
            slash.life -= 0.025; 
            ...
            if (slash.life <= 0) {
                ...
                // --- DIMENSIONE INCREMENTATA ---
                // Parte da 0.8 e arriva a 2.5 volte la dimensione originale
                const scale = 0.8 + (1 - slash.life) * 1.7; 
                ctx.scale(scale, scale);
                
                ctx.globalAlpha = slash.life;
                ctx.lineCap = 'round';
                
                // 1. OMBRA/BAGLIORE MASSICCIO
                ctx.beginPath();
                ctx.strokeStyle = slash.color;
                // Spessore raddoppiato (da 16 a 30)
                ctx.lineWidth = 30 * slash.life; 
                ctx.globalCompositeOperation = "lighter";
                ctx.shadowBlur = 25; // Glow molto più forte
                ctx.shadowColor = slash.color;
                // Arco ampliato a 180 gradi (PI / 2)
                ctx.arc(-HEX_SIZE * 0.1, 0, HEX_SIZE * 0.7, -Math.PI / 2, Math.PI / 2);
                ctx.stroke();
                
                // 2. NUCLEO BIANCO ROVENTE
                ctx.beginPath();
                ctx.strokeStyle = '#ffffff';
                // Spessore aumentato (da 6 a 12)
                ctx.lineWidth = 12 * slash.life; 
                ctx.arc(-HEX_SIZE * 0.1, 0, HEX_SIZE * 0.7, -Math.PI / 2, Math.PI / 2);
                ctx.stroke();
    ...
    requestAnimationFrame(vfxRenderLoop);
}
// Parte in automatico
requestAnimationFrame(vfxRenderLoop);
