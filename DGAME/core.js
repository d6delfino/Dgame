...
function drawGame() {
    ...
    grid.forEach(cell => {
        ...
        if (cell.type === 'water') {
            ...
            ctx.fillStyle = '#fff';
            ctx.fillText('🌊', p.x, p.y + HEX_SIZE * 0.27); // Oppure usa 🕳️ se vuoi fare un "crepaccio"
    ...
    grid.forEach(cell => {
        ...
        if (cell.entity.type === 'agent') {
            ...
            if (cell.entity._hitTime && (now - cell.entity._hitTime < 600)) {
                ...
                ctx.filter = 'brightness(300%) sepia(100%) hue-rotate(300deg) saturate(500%)'; // Diventa bianchissimo/rosso
            }
...
function createParticles(x, y, color, amount = 10, isExplosion = false) {
    for (let i = 0; i < amount; i++) {
        // Calcoliamo un angolo casuale tra 0 e 360 gradi
        const angle = Math.random() * Math.PI * 2;
        // Velocità casuale per ogni particella
        const speed = Math.random() * (isExplosion ? 5 : 2.5);

        particles.push({
            x: x,
            y: y,
            // Esplosione radiale perfetta usando seno e coseno
...
function updateAndDrawParticles() {
    for (let i = particles.length - 1; i >= 0; i--) {
        ...
        // Applichiamo la velocità (movimento lineare)
        p.x += p.vx;
        p.y += p.vy;
        
        // --- ATTRITO DELL'ARIA (Frizione) ---
        // Le particelle rallentano uniformemente in tutte le direzioni.
        // 0.95 le fa fermare dolcemente vicino al punto di origine.
        p.vx *= 0.95;
        p.vy *= 0.95;
        
        // --- GRAVITÀ ELIMINATA ---
        // Nessuna aggiunta a p.vy, così le particelle non "cadono" mai.
