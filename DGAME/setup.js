...
function renderAgentMarket() {
    ...
    setupData.agents.forEach((agent, index) => {
        ...
        card.className = 'market-agent-card';

        // Visuale agente
        const visual = document.createElement('div');
        visual.className = 'agent-visual';
        ...
        if (agent.customSpriteId && customSpriteFiles[agent.customSpriteId]) {
            spriteHtml = `<img src="${customSpriteFiles[agent.customSpriteId]}"
                style="width:45px;height:45px;object-fit:contain;margin-bottom:5px;"
                onerror="this.outerHTML='<span class=\\'sprite-preview\\'>${agent.sprite}</span>'">`;
        }
        visual.innerHTML = `
            ${spriteHtml}
            <div style="width:20px;height:20px;border-radius:50%;
                background:${players[currentPlayer].color};margin:0 auto 5px auto;
                box-shadow:0 0 10px ${players[currentPlayer].color}"></div>
            Op. ${index + 1}
        `;
        ...
        removeBtn.style.borderColor = '#522';
        removeBtn.style.color     = '#f88';
...
function buildWaitMessage() {
    ...
    for (let p = 1; p <= onlineTotalPlayers; p++) {
        if (!playersReady[p])
            missing.push(`<span style="color:${colors[p]}">${names[p]}</span>`);
    }
