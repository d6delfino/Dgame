...
function _getTakenFactions() {
    ...
    for (let p = 1; p <= totalPlayers; p++) {
        ...
        // FIX: In modalità Campagna i colori scelti all'inizio non possono MAI essere rubati
...
/**
 * Se il giocatore non ha ancora una fazione cosmetica assegnata
 * (es. all'apertura del suo turno di setup), ne sceglie una a caso
 * tra quelle libere (non prese da altri). Se non ci sono slot liberi,
 * ricade sullo slot numerico del giocatore (comportamento precedente).
 */
function _ensureCosmeticFaction(pNum) {
    if (players[pNum]._cosmeticFaction) return; // già scelta/confermata
    ...
    const def = _FACTION_DEFS[slot - 1];
    players[pNum]._cosmeticFaction = slot;
    players[pNum].color            = def.color;
    players[pNum].name             = def.name;
}
...
function applyFactionCosmetic(factionSlot) {
    ...
    // --- CONTROLLO ANTI-DUPLICATI (solo in locale) ---
    // In multiplayer online i conflitti di colore sono gestiti
    // interamente dall'host tramite REQUEST_COLOR / COLOR_RESPONSE /
    // LOBBY_UPDATE. Il client non deve prendere decisioni autonome
    // sui colori degli altri giocatori con dati potenzialmente incompleti.
    if (!window.isOnline) {
        for (let p = 1; p <= totalPlayers; p++) {
            if (p === currentPlayer) continue;
...
function updateSetupUI() {
    ...
    _ensureCosmeticFaction(pNum); // Assegna una fazione casuale libera se non ancora scelta
    ...
    // Aggiorna il costo mostrato sul bottone "+ Nuovo Agente"
    const agentCostLabel = document.getElementById('new-agent-cost-label');
    if (agentCostLabel) agentCostLabel.textContent = GAME.AGENT_COST;
...
function renderAgentMarket() {
    ...
    setupData.agents.forEach((agent, index) => {
        ...
        const statTypes = [
            { id: 'hp',  icon: '💚', label: 'Vita',  max: AGENT_STAT_MAX.hp  },
            { id: 'mov', icon: '👣', label: 'Passi', max: AGENT_STAT_MAX.mov },
            { id: 'rng', icon: '🎯', label: 'Tiro',  max: AGENT_STAT_MAX.rng },
            { id: 'dmg', icon: '⚔️', label: 'Danno', max: AGENT_STAT_MAX.dmg },
        ];
...
// Stesso controllo, ma per le carte — saltato per i turni AI, che per design
// non selezionano carte dalla UI (le genera automaticamente getFinalCardSelection)
if (cardSelectionData.selected.length === 0 && !(isAIActive() && currentPlayer > 1)) {
    showTemporaryMessage('⚠️ DEVI SELEZIONARE ALMENO UNA CARTA!', 3000);
    playSFX('shield');
    return;
}
        ...
        if (currentPlayer < totalPlayers) {
            ...
            if (isAIActive()) {
                setTimeout(() => {
                    ...
                    // --- FIX: RISOLUZIONE CONFLITTI COLORE BOT ---
                    let currentAiSlot = players[currentPlayer]._cosmeticFaction ?? currentPlayer;
                    const takenColors = new Set();
                    // Guarda i colori già confermati dai giocatori precedenti
                    for (let p = 1; p < currentPlayer; p++) {
                        takenColors.add(players[p]._cosmeticFaction ?? p);
                    }
                    // Se il colore di default del bot è già preso, fagli prendere il primo libero
                    if (takenColors.has(currentAiSlot)) {
                        for (let s = 1; s <= 8; s++) {
                            if (!takenColors.has(s)) {
                                players[currentPlayer]._cosmeticFaction = s;
                                players[currentPlayer].color = _FACTION_DEFS[s-1].color;
                                players[currentPlayer].name  = _FACTION_DEFS[s-1].name;
                                break;
                            }
                        }
                    }
