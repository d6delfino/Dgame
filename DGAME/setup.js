...
function updateSetupUI() {
    ...
    // Testo + emoji grandi, ma pulsante più basso
    confirmBtn.style.padding = '6px 20px';        // ← ridotto ulteriormente
    confirmBtn.style.fontSize = '1.4em';
    confirmBtn.style.lineHeight = '1';            // ← molto compatto
    confirmBtn.style.minHeight = 'auto';
    
    confirmBtn.innerHTML = `<span style="font-size:1.8em; vertical-align:middle; line-height:1;">▶️</span> Avvia Battaglia`;
...
function renderAgentMarket() {
    ...
    setupData.agents.forEach((agent, index) => {
        ...
        function buildSpriteContent(slot) {
            ...
            if (url) {
                return `<img src="${url}"
                    style="width:56px;height:56px;object-fit:contain;display:block;pointer-events:none; transform:scale(1.05);"
        ...
        spriteWrap.style.cssText = `width:58px;height:58px;display:flex;align-items:center;
        justify-content:center;cursor:pointer;border-radius:8px;border:2px solid ${pColor}55;`;
        ...
        agentInfo.style.cssText = 'display:flex;align-items:center;gap:2px;';
        ...
        namePart.innerHTML = `
            ...
            <div style="font-weight:bold;font-size:15px;color:#fff;">AGENTE ${index + 1}</div>`;
...
function addNewAgentToMarket() {
    if (setupData.agents.length >= GAME.MAX_AGENTS) return alert(`Massimo ${GAME.MAX_AGENTS} agenti.`);
...
function initAIToggleUI() {
    ...
    aiToggleDiv.style.cssText = `
        margin:4px 0; text-align:center; border:1px solid #444;
        padding:6px; background:rgba(0,0,0,0.3); border-radius:5px;
    `;
    ...
    // Inseriamo DOPO il pannello delle carte
    const cardPanel = document.getElementById('card-selection-panel');
    if (cardPanel && cardPanel.parentNode) {
        cardPanel.parentNode.insertBefore(aiToggleDiv, cardPanel.nextSibling);
    } else {
        const confirmBtn = document.getElementById('confirm-setup-btn');
