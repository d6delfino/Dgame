...
function _recruitAgentFromShop() {
    ...
    // Trova il primo slot sprite non ancora usato dagli agenti esistenti
    const usedSlots = pData.agents.map(a => {
        if (a.customSpriteId && a.customSpriteId.startsWith(factionData.prefix)) {
            return parseInt(a.customSpriteId.replace(factionData.prefix, ''));
        }
        return -1;
    });
    let shopSlot = 1;
    while (usedSlots.includes(shopSlot)) shopSlot++;
    if (shopSlot > factionData.count) shopSlot = (shopSlot % factionData.count) || factionData.count;
