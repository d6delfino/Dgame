...
function showNotificationBanner(html, color, opts = {}) {
    ...
    const animName = top !== null ? 'creditFadeCentered' : 'creditFadeFixed';
    msg.style.cssText = `
...
function initCreditShopUI() {
    ...
    creditRow.innerHTML = `
        ...
        <button id="credit-shop-btn" class="action-btn"
                style="flex:1; font-size:18px; padding:7px 6px; border-color:#888; color:#888; opacity:0.4; cursor:not-allowed;"
                onclick="toggleCreditShop()" disabled>
            <span style="font-size:20px; line-height:1;">🛒</span> BASE
        </button>
    `;
    ...
    // --- Pannello negozio come overlay indipendente ---
...
function updateCreditShopBtn() {
    ...
    const isShopOpen = document.getElementById('credit-shop-panel')?.style.display !== 'none';
    const enabled   = canAct && (credits > 0 || isShopOpen);
...
function toggleCreditShop() {
    ...
    if (isOpen) {
        ...
        _shopOwnerFaction = null;
    } else {
        _shopOwnerFaction = currentPlayer;
        // Adatta posizione e larghezza all'orientamento corrente
        const isLandscape = window.innerWidth > window.innerHeight;
        if (isLandscape && window.innerWidth <= 950) {
            panel.style.width    = (window.innerWidth - 185) + 'px';
            panel.style.right    = '170px';
            panel.style.left     = 'auto';
            panel.style.bottom   = '5px';
            panel.style.maxHeight = (window.innerHeight - 10) + 'px';
        } else {
            panel.style.width    = '380px';
            panel.style.right    = '195px';
            panel.style.left     = 'auto';
...
function _renderRecruitSection(panel, pData, credits, color) {
    ...
    const maxReached = agentCount >= GAME.MAX_AGENTS;
    const baseCost   = GAME.CREDIT_AGENT_BASE;

    // Inizializza le stat temporanee se non esistono
    if (!_shopAgentStats) {
        // Calcola il primo slot sprite libero come default
        const _initCosmetic = players[currentPlayer]._cosmeticFaction ?? currentPlayer;
        const _initFaction  = FACTION_PREFIXES[_initCosmetic];
        const _usedSlots    = (players[currentPlayer].agents || []).map(a =>
            a.customSpriteId?.startsWith(_initFaction.prefix)
                ? parseInt(a.customSpriteId.replace(_initFaction.prefix, ''))
                : -1
        );
        let _initSlot = 1;
        while (_usedSlots.includes(_initSlot)) _initSlot++;
        if (_initSlot > _initFaction.count) _initSlot = (_initSlot % _initFaction.count) || _initFaction.count;
        _shopAgentStats = { hp: 1, mov: 1, rng: 1, dmg: 1, spriteSlot: _initSlot };
    }
    ...
    if (maxReached) {
        const msg = document.createElement('div');
        msg.style.cssText = `color:#555; font-size:12px; font-style:italic;`;
        msg.innerText = `Massimo ${GAME.MAX_AGENTS} agenti per fazione.`;
        section.appendChild(msg);
        panel.appendChild(section);
        return;
    }
    ...
    function _buildShopSpriteContent(slot) {
        const id  = `${prefix}${slot}`;
        const url = customSpriteFiles[id];
        if (url) return `<img src="${url}" style="width:125px;height:125px;object-fit:contain;display:block;pointer-events:none;">`;
        return `<span style="font-size:30px;line-height:1;">${players[currentPlayer].agents[0]?.sprite || '🧍'}</span>`;
    }

    // Aggiornato lo stile base per coprire tutta l'altezza e mezza larghezza
    const btnStyle = `background:none;border:none;cursor:${canRecruit ? 'pointer' : 'default'};
        position:absolute;top:-10px;bottom:-10px;width:55px;z-index:2;`;

    const spWrap = document.createElement('div');
    spWrap.style.cssText = `width:86px;height:86px;display:flex;align-items:center;justify-content:center;pointer-events:none;`;
    spWrap.innerHTML = _buildShopSpriteContent(_shopAgentStats.spriteSlot);

    const spPrevBtn = document.createElement('button');
    // Sborda di 15px verso sinistra
    spPrevBtn.style.cssText = btnStyle + 'left:-15px;';
    spPrevBtn.innerHTML = ''; // Nessuna freccia visibile
    spPrevBtn.disabled  = !canRecruit;

    const spNextBtn = document.createElement('button');
    // Sborda di 15px verso destra
    spNextBtn.style.cssText = btnStyle + 'right:-15px;';
    spNextBtn.innerHTML = ''; // Nessuna freccia visibile
    spNextBtn.disabled  = !canRecruit;

    spPrevBtn.onclick = (e) => {
        e.stopPropagation();
        const cur = _shopAgentStats.spriteSlot || 1;
        _shopAgentStats.spriteSlot = cur <= 1 ? maxSprites : cur - 1;
        spWrap.innerHTML = _buildShopSpriteContent(_shopAgentStats.spriteSlot);
    };
    spNextBtn.onclick = (e) => {
        e.stopPropagation();
        const cur = _shopAgentStats.spriteSlot || 1;
        _shopAgentStats.spriteSlot = cur >= maxSprites ? 1 : cur + 1;
        spWrap.innerHTML = _buildShopSpriteContent(_shopAgentStats.spriteSlot);
    };

    // Contenitore relativo ESATTAMENTE COME ERA ALL'INIZIO (non tocca il layout)
    const spriteCol = document.createElement('div');
    spriteCol.style.cssText = `position:relative;display:flex;align-items:center;justify-content:center;`;
    spriteCol.appendChild(spPrevBtn);
    spriteCol.appendChild(spWrap);
    spriteCol.appendChild(spNextBtn);

    // Layout a due colonne: stat+footer a sinistra, picker a destra
    const bodyRow = document.createElement('div');
    bodyRow.style.cssText = `display:flex; gap:10px; align-items:stretch;`;
    ...
    statTypes.forEach(stat => {
        ...
        // Allineamento dinamico con larghezza ottimizzata per il pannello shop (175px)
        item.style.cssText = `display:flex; align-items:center; background:#0a0a10; border:1px solid #333; padding:4px 8px; border-radius:4px; width:175px; box-sizing:border-box; flex-shrink:0;`;
        item.innerHTML = `
            <span style="font-size:10px; color:#888; text-transform:uppercase; display:flex; align-items:center; gap:4px; white-space:nowrap;">
                <span style="font-size:12px;">${stat.icon}</span>${stat.label}
            </span>
    ...
    // Aggiunto flex-wrap per evitare che la scritta "(manca X cr)" spinga orizzontalmente tutto il layout
    footer.style.cssText = `display:flex; justify-content:flex-start; align-items:center; gap:10px; margin-top:auto; flex-wrap:wrap;`;
    footer.innerHTML = `
        <span style="color:${canRecruit ? color : '#555'}; font-size:13px;">
            Costo: <strong>${totalCost} cr</strong>
            ${credits < totalCost ? `<span style="color:#ff3333; font-size:11px;"> (manca ${totalCost - credits})</span>` : ''}
        </span>
    `;
...
window._shopStatChange = _shopStatChange;
...
function _recruitAgentFromShop() {
    ...
    if ((pData.agents?.length || 0) >= GAME.MAX_AGENTS) return;
    if (!pData.hq?.hp) return;

    pData.credits -= cost;
    _shopAgentStats = null;   // reset per il prossimo reclutamento

    // Recupera la fazione cosmetica scelta dal giocatore (può differire da currentPlayer)
    const cosmeticFaction = players[currentPlayer]._cosmeticFaction ?? currentPlayer;
    const factionData     = FACTION_PREFIXES[cosmeticFaction];

    // Usa lo slot sprite scelto nel picker; se non impostato, calcola il primo libero
    let shopSlot = stats.spriteSlot ?? (() => {
        const usedSlots = pData.agents.map(a =>
            a.customSpriteId?.startsWith(factionData.prefix)
                ? parseInt(a.customSpriteId.replace(factionData.prefix, ''))
                : -1
        );
        let s = 1;
        while (usedSlots.includes(s)) s++;
        if (s > factionData.count) s = (s % factionData.count) || factionData.count;
        return s;
    })();
    ...
    const newAgent = {
        ...
        customSpriteId: `${factionData.prefix}${shopSlot}`,
...
function _renderCardReplaceSection(panel, pData, credits, color) {
    ...
    // LAYOUT SUPERIORE: Contenitore flessibile per Slot (sinistra) e Conferma (destra)
    const topRow = document.createElement('div');
    topRow.style.cssText = `display:flex; justify-content:space-between; align-items:flex-end; margin-bottom:10px;`;

    // COLONNA SINISTRA: Step 1 — scegli quale slot rimpiazzare
    const leftCol = document.createElement('div');
    ...
    slotsRow.style.cssText = `display:flex; gap:6px; flex-wrap:wrap;`; // margin-bottom tolto, lo gestisce topRow
    ...
    leftCol.appendChild(slotsRow);
    topRow.appendChild(leftCol);

    // COLONNA DESTRA: Costo e pulsante Conferma
    ...
    rightCol.innerHTML = `
        <span style="color:${canConfirm ? color : '#555'}; font-size:12px; text-align:right; line-height:1.2;">
            Costo: <strong>${GAME.CREDIT_CARD_REPLACE} cr</strong>
            ${!canAfford ? '<br><span style="color:#ff3333;font-size:10px;">(insufficienti)</span>' : ''}
        </span>
    `;
    const confirmBtn = document.createElement('button');
    confirmBtn.className = 'action-btn';
    confirmBtn.style.cssText = `font-size:12px; padding:6px 12px; margin:0; border-color:${canConfirm ? color : '#444'}; color:${canConfirm ? color : '#444'}; ${canConfirm ? '' : 'opacity:0.5;cursor:not-allowed;'}`;
    ...
    // Step 2 — scegli la nuova carta (tutte le disponibili, mostrate solo se c'è uno slot selezionato)
    if (_replaceSlotIndex !== null) {
        ...
        // Rimosso il margin-bottom finale perché ora è l'ultimo elemento
        cardGrid.style.cssText = `display:grid; grid-template-columns:repeat(5,1fr); gap:5px;`;
...
function awardBaseDestroyReward(killerFaction) {
    ...
    const reward = GAME.REWARD_HQ_DESTROY;
    players[killerFaction].credits = (players[killerFaction].credits || 0) + reward;
