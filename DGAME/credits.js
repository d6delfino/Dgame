...
function _renderRecruitSection(panel, pData, credits, color) {
    ...
    const btnStyle = `background:none;border:none;color:${color};font-size:22px;cursor:pointer;padding:0 4px;line-height:1;opacity:${canRecruit ? '1' : '0.4'};`;

    const spWrap = document.createElement('div');
    // Rimossi completamente i bordi e il background, mantenendo le dimensioni per stabilità del layout
    spWrap.style.cssText = `width:86px;height:86px;display:flex;align-items:center;justify-content:center;
    ...
    spWrap.onclick = (e) => { e.stopPropagation(); if (canRecruit) spNextBtn.click(); };

    // Contenitore relativo: frecce in overlay ai lati, come nel setup normale
    ...
    spPrevBtn.style.cssText += `position:absolute;left:0;top:50%;transform:translateY(-50%);z-index:1;`;
    spNextBtn.style.cssText += `position:absolute;right:0;top:50%;transform:translateY(-50%);z-index:1;`;
