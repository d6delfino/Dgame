 ...
 * 
 * Chiamala nel punto in cui rilevi la morte di un agente
 * (solitamente dentro gamelogic.js → executeRemoteAction o resolveAttack).
 */
function awardKillReward(killerFaction) {
    if (!killerFaction || !players[killerFaction]) return;

    const pData = players[killerFaction];
    const reward = 2;

    // Assegna i crediti
    pData.credits = (pData.credits || 0) + reward;

    // VFX bello (stesso stile del reddito turno)
    const msg = document.createElement('div');
    msg.style.cssText = `
        position:fixed; bottom:90px; right:20px; z-index:9998;
        background:rgba(5,5,15,0.92); border:2px solid ${players[killerFaction].color};
        border-radius:6px; padding:8px 16px; font-family:'Courier New',monospace;
        font-size:14px; color:${players[killerFaction].color}; font-weight:bold;
        box-shadow:0 0 12px ${players[killerFaction].color}66;
        animation:creditFade 2.2s ease-out forwards; pointer-events:none;
    `;
    msg.innerText = `💀 +${reward} crediti (uccisione)`;
    document.body.appendChild(msg);
    setTimeout(() => msg.remove(), 2200);

    _ensureCreditFadeStyle();

    // Aggiorna subito il contatore crediti in UI
    updateUI();

    // Multiplayer: non serve nessun messaggio extra!
    // executeRemoteAction viene eseguito sia dall'host che dai client,
    // quindi i crediti vengono aggiornati ovunque automaticamente.
}
...
function awardBaseDestroyReward(killerFaction) {
    ...
    // VFX specifico per la base (più grande e visibile)
    const msg = document.createElement('div');
    msg.style.cssText = `
        position:fixed; bottom:120px; right:20px; z-index:9998;
        background:rgba(5,5,15,0.95); border:3px solid ${players[killerFaction].color};
        border-radius:8px; padding:12px 24px; font-family:'Courier New',monospace;
        font-size:18px; color:${players[killerFaction].color}; font-weight:bold;
        box-shadow:0 0 25px ${players[killerFaction].color};
        animation:creditFade 2.5s ease-out forwards; pointer-events:none;
        text-align:center;
    `;
    msg.innerHTML = `🏰 +${reward} CREDITI<br><span style="font-size:12px;opacity:0.8;">BASE NEMICA DISTRUTTA!</span>`;
    document.body.appendChild(msg);
