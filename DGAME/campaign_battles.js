...
function _bat_showBattleResultsUI(winnerFaction, sectorId, results) {
    ...
    const creditsHtml = participants.map(faction => {
        ...
        const destination = isWinner
            ? `→ 📦 Nel Settore: <b>${sectorAlloc}</b>`
            : `→ 🏦 Alla Banca: <b>+${r.total}</b>`;

        return `<div style="color:${c}; font-size:16px; margin:8px 0; border-left:3px solid ${c}; padding:5px 10px; background:rgba(255,255,255,0.02);">
            <div style="font-weight:bold; font-size:18px;">${name} ${isWinner ? '🏆' : '💀'}</div>
            <div style="color:#aaa; font-size:13px;">
                Negozio: ${r.shopResidual} + Agenti vivi: ${r.survivorValue} = <b>${r.total}</b>
            </div>
            <div style="font-size:15px; margin-top:3px;">${destination}</div>
        </div>`;
    }).join('');
    ...
    const ownedHtml = Array.from({ length: n }, (_, i) => i + 1).map(p => {
        ...
        const c   = COLORS['p' + p];
        return `<span style="color:${c}; margin:0 8px; font-weight:bold; font-size:14px;">${players[p]?.name || 'P'+p}: 🏴${cnt}</span>`;
    }).join('');

    const overlay = document.createElement('div');
    overlay.id = 'battle-results-overlay'; // ID per facile rimozione
    overlay.style.cssText = `position:fixed; top:0; left:0; width:100%; height:100%;
        background:rgba(0,0,0,0.95); z-index:2000000;
        display:flex; flex-direction:column; align-items:center; justify-content:center;
        font-family:Courier New,monospace; text-align:center; padding:20px; box-sizing:border-box;`;
    
    overlay.innerHTML = `
        <div style="background:rgba(10,15,30,0.98); border:2px solid ${winnerColor}; border-radius:15px; padding:30px; max-width:550px; width:95%; box-shadow:0 0 30px rgba(0,0,0,1);">
            <h1 style="color:${winnerColor}; text-shadow:0 0 15px ${winnerColor}; margin:0 0 5px; font-size:28px;">⚔️ BATTAGLIA CONCLUSA</h1>
            <h2 style="color:#fff; margin:0 0 20px; font-size:20px;">VINCITORE: <span style="color:${winnerColor}">${winnerName.toUpperCase()}</span></h2>
            
            <div style="background:rgba(255,255,255,0.03); border:1px solid #333; border-radius:10px; padding:15px; margin-bottom:20px; text-align:left; max-height:40vh; overflow-y:auto;">
                <p style="color:#888; font-size:12px; margin:0 0 10px; text-transform:uppercase; text-align:center;">Resoconto Crediti</p>
                ${creditsHtml}
            </div>
            
            <div style="background:rgba(0,0,0,0.3); border:1px solid #222; border-radius:8px; padding:10px; margin-bottom:25px;">
                ${ownedHtml}
            </div>
            
            <button class="action-btn" id="results-continue-btn"
                style="padding:15px 60px; border:2px solid ${winnerColor}; color:${winnerColor}; background:transparent; cursor:pointer; font-size:22px; font-weight:bold; width:100%;">
                AVANTI ▶
            </button>
        </div>`;
