/* ============================================================
   config.js — Costanti globali e Gestione Temi
   ============================================================ */

const bgOptions = [
    { id: 'S1', path: 'img/sfondo1.jpg', prefix: 'HE', count: 20 },
    { id: 'S2', path: 'img/sfondo2.jpg', prefix: 'ZE', count: 20 },
    { id: 'S3', path: 'img/sfondo3.jpg', prefix: 'OB', count: 22 }
];

// Variabili globali del tema
let SELECTED_BG_ID, THEME_WALL_PREFIX, THEME_WALL_COUNT, THEME_BARRICADE_ID;
...
/**
 * Applica un tema specifico caricando sfondo e muri corrispondenti
 */
function applyTheme(themeObj) {
    ...
    // Imposta l'ID della barricata come il primo elemento della lista tematica
    THEME_BARRICADE_ID = THEME_WALL_PREFIX + "1";

    console.log(`Tema Applicato: ${SELECTED_BG_ID} (Barricata: ${THEME_BARRICADE_ID})`);

    // Registra e carica dinamicamente i muri del tema
    for (let i = 1; i <= THEME_WALL_COUNT; i++) {
        const spriteKey = `${THEME_WALL_PREFIX}${i}`;
        const url = `img/${spriteKey}.png`;
        customSpriteFiles[spriteKey] = url;

        if (!customImages[spriteKey]) {
            const img = new Image();
            img.src = url;
            img.onload = () => { customImages[spriteKey] = img; };
        }
    }
}

// Caricamento iniziale (Casuale per Host/Single Player)
const initialTheme = bgOptions[Math.floor(Math.random() * bgOptions.length)];
applyTheme(initialTheme);

// Caricamento icone fisse (HQ e Agenti)
...
let turnCount = 0; let turnCounterUI; let totalPlayers = 2; let state = 'SETUP_P1'; let currentPlayer = 1;
