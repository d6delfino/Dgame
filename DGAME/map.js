...
function buildEmptyGrid(effectiveRadius) {
...
function generateProceduralMap() {
    ...
    grid.forEach(cell => {
        ...
        if (farFromAll) {
            ...
            } else if (rand < GAME.WALL_DENSITY + GAME.WATER_DENSITY) {
                // Genera l'ostacolo d'acqua
                cell.type = 'water';
                cell.hp = 0; cell.maxHp = 0; // Indistruttibile
            }
        }
    });
...
function createHQ(faction) {
    const cosmeticFaction = players[faction]?._cosmeticFaction ?? faction;
    const hq = {
        id: `hq_${faction}`, type: 'hq', faction,
        sprite: SPRITE_POOLS.hqs[cosmeticFaction - 1],
        customSpriteId: 'HQ' + cosmeticFaction,
        hp: GAME.HQ_HP, maxHp: GAME.HQ_HP, 
        mov: GAME.HQ_MOV, rng: GAME.HQ_RNG, dmg: GAME.HQ_DMG, ap: 0,
        q: 0, r: 0,
...
function receiveGameState(netState) {
    ...
    // Piazza entità — salta i giocatori non partecipanti (spettatori/disconnessi)
    for (let f = 1; f <= totalPlayers; f++) {
        if (players[f]?.isDisconnected) continue;
