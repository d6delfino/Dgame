...
function initAsHost() {
    ...
    peer.on('connection', (c) => {
        ...
        // 1. Controlla se è una riconnessione (il client invia il proprio numero)
        if (c.metadata && c.metadata.playerNumber) {
            assignedPlayer = c.metadata.playerNumber;
        } else {
            // 2. Assegnazione iniziale: cerca il primo slot libero
        ...
        // Sovrascrive la vecchia connessione "fantasma" con la nuova
...
function _hostSendFullSync(targetPlayerNum = null) {
    ...
    const msg = {
        ...
        state: {
            themeId:          SELECTED_BG_ID, // <-- Necessario per chi entra a metà partita
            timeLeft:         timeLeft,       // <-- Sincronizza i secondi rimanenti
            walls, terrains,
...
function _applyFullStateSync(st) {
    ...
    // 1. Ricostruzione fisica della griglia (fondamentale per chi entra a metà)
    grid.clear();
    ...
    // 4. Ripristino Giocatori, HQ e Agenti (Clonazione profonda per eliminare agenti morti)
    if (st.players) {
        for (let p = 1; p <= totalPlayers; p++) {
    ...
    // 5. Se il client era nel Menu di Setup (join a metà partita), passalo al gioco attivo
    if (state !== 'PLAYING') {
        state = 'PLAYING';
