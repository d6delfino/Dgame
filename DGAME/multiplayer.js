...
function handleClientReceivedData(data) {
    ...
    if (data.type === 'ASSIGN_PLAYER') {
        ...
        // CORREZIONE 1: Resetta gli agenti e apri il Setup SOLO se la partita non è iniziata.
        // Se siamo in PLAYING, stiamo semplicemente recuperando una disconnessione!
        if (state !== 'PLAYING') {
            setupData = freshSetupData();
            for (let p = 1; p <= 4; p++) { 
                players[p].hq = null; 
...
function _applyFullStateSync(st) {
    ...
    // 1. Aggiorna in modo "safe" i giocatori senza distruggere i colori e i nomi
    if (st.players) {
        for (let p = 1; p <= totalPlayers; p++) {
            ...
            // CORREZIONE CRITICA: Copia profonda per clonare esattamente gli agenti e l'HQ dell'Host
            if (src.agents) players[p].agents = JSON.parse(JSON.stringify(src.agents));
            if (src.hq) players[p].hq = JSON.parse(JSON.stringify(src.hq));
            else players[p].hq = null;
        }
    }

    // 2. Aggiorna Punti di Controllo
    if (st.controlPoints) {
        controlPoints.clear();
    ...
    // Se la griglia è vuota (es. ricaricamento forzato della pagina), dobbiamo prima generare gli esagoni
    if (grid.size === 0 && typeof generateProceduralMap === 'function') {
        generateProceduralMap();
    }

    // 3. Pulisce la griglia dalle vecchie entità e riapplica Muri/Terreni inviati dall'Host
    grid.forEach(cell => cell.entity = null);
    
    if (st.walls) {
        st.walls.forEach(w => {
            const cell = grid.get(getKey(w.q, w.r));
            if (cell) { cell.hp = w.hp; cell.maxHp = w.maxHp; cell.type = w.type; cell.sprite = w.sprite; cell.customSpriteId = w.customSpriteId; }
        });
    }
    if (st.terrains) {
    ...
    // 4. Riposiziona fisicamente tutte le pedine aggiornate sulla griglia
    ...
    // 5. Usciamo dalla prigione della schermata di Setup (se eravamo bloccati lì a causa della disconnessione)
    if (state !== 'PLAYING' && state !== 'GAME_OVER') {
        state = 'PLAYING';
        document.getElementById('setup-overlay').style.display = 'none';
        document.getElementById('controls-panel').style.display = 'block';
        if (typeof autoFitMap === 'function') autoFitMap();
    }
