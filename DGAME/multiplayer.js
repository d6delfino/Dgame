...
function setupHostConnection(c, playerNum) {
    ...
    // FIX BUG 1: Monitoraggio ICE anche lato host (necessario per rilevare
    // disconnessioni su mobile dove gli eventi 'close'/'error' non scattano)
...
function handleHostReceivedData(data, fromPlayer) {
    if (data.type === 'SETUP_DONE') {
        // FIX BUG 2: salviamo nel buffer invece di scrivere direttamente su players[].
        // hostStartGame() resetta players[] DOPO che questo messaggio può arrivare,
        // quindi il dato andrebbe perso. tryHostStart() legge dal buffer.
...
function tryHostStart() {
    ...
    // FIX BUG 2: applica gli agenti dal buffer prima di generare la mappa.
    // I client possono aver inviato SETUP_DONE prima che hostStartGame() resettasse
    // players[], quindi i dati reali sono nel buffer e non in players[].
