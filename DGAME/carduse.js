...
registerActionHandler('shoot', function (targetCell, fromNetwork) {
    ...
    // Applica danni (rispettando scudi)
    damageMap.forEach((totalDmg, actualTarget) => {
        ...
        if (actualTarget.shielded) {
            ...
            if (selectedAgent.sniperPierce && isMainTarget) {
                // Cecchino: perfora lo scudo senza consumarlo, il danno passa normalmente
                playSpecialVFX(actualTarget, '#ff3333', '🎯 SCUDO PERFORATO!');
            } else {
                // Scudo cumulabile: consuma 1 livello e annulla il danno
        ...
        // if (finalDmg > 0) actualTarget.hp -= finalDmg;
        ...
        if (actualTarget.hp <= 0) {
            if (actualTarget.q !== undefined &&
                ...
                // La struttura viene distrutta; un eventuale agente Spettro
                // sopravvive ed è ora esposto sulla cella diventata empty
...
// ============================================================
// DRAW HOOK — alone scudo elettronico (C07)
// ============================================================
