        ...
        if (selectedAgent.demoBuff) {
                ...
                hexDirections.forEach(dir => {
                    ...
                    if (adjCell) {
                        ...
                        } else if (adjCell.entity && adjCell.entity.faction !== currentPlayer) {
                        ...
                        if (typeof drawMeleeSlash === 'function') {
                            // L'origine è il centro dell'esplosione, il colore è Arancio (#ff8800)
                            const centerExplosion = { q: item.cell.q, r: item.cell.r, faction: null };
                            drawMeleeSlash(centerExplosion, adjCell, '#ff8800');
                        }
                    }
                });
            }
    ...
    for (let p = 1; p <= totalPlayers; p++) {
        ...
        players[p].agents.forEach(a => {
            ...
            if (a.medikitTempBuff) {
                a.mov   = a._medikitOrigMov;
                a.rng   = a._medikitOrigRng;
                a.dmg   = a._medikitOrigDmg;
                // Riduciamo maxHp ma clamppiamo hp per non andare in negativo
                a.maxHp = a._medikitOrigMaxHp;
