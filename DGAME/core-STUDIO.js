...
function applyTheme(themeObj) {
    ...
    mapBackground.onload = () => {
        ...
        invalidateStaticLayer(); // NUOVO: forza il rebuild ora che lo sfondo è pronto
    };
...
// --- STATIC LAYER: ridisegna solo se invalidato (ora include anche sfondo + overlay) ---
    if (_staticDirty) {
        ...
        // Niente più scambi pericolosi di variabili globali!
        const dpr = window.devicePixelRatio || 1;
        ...
        if (bgOffscreenReady) {
            ...
            _staticCtx.drawImage(bgOffscreen, bgX, bgY, bgW, bgH);
        } else {
            _staticCtx.fillStyle = COLORS.bg || '#000';
            _staticCtx.fillRect(0, 0, window.innerWidth, window.innerHeight);
        }

        _staticCtx.fillStyle = 'rgba(0, 0, 0, 0.75)';
        _staticCtx.fillRect(0, 0, window.innerWidth, window.innerHeight);
        ...
        grid.forEach(cell => {
            ...
            // Passiamo esplicitamente _staticCtx
            drawHex(_staticCtx, cell.q, cell.r, stroke, fill, 1);
            ...
            if (cell.type === 'wall' || cell.type === 'barricade') {
                ...
                if (cell.customSpriteId && customImages[cell.customSpriteId]?.complete
                    ...
                    _staticCtx.drawImage(customImages[cell.customSpriteId], p.x - imgSize / 2, p.y - imgSize / 2, imgSize, imgSize);
                } else {
                    _staticCtx.font      = `${Math.round(HEX_SIZE * 0.75)}px Arial`;
                    _staticCtx.textAlign = 'center';
                    _staticCtx.fillStyle = '#fff';
                    _staticCtx.fillText(cell.sprite || '', p.x, p.y + HEX_SIZE * 0.27);
                }
            }
        });
    ...
    waterCells.forEach(cell => { // <-- USA LA CACHE INVECE DI TUTTA LA GRIGLIA
        ...
        drawHex(ctx, cell.q, cell.r, null, `rgba(0, 160, 255, 0.4)`, 0); // <-- Aggiunto ctx
        ctx.restore();
    ...
    validActionTargets.forEach(t => {
        ...
        drawHex(ctx, t.q, t.r, stroke, fill); // <-- Aggiunto ctx
    });
    ...
    controlPoints.forEach(cp => {
        ...
        drawHex(ctx, cp.q, cp.r, cpColor, cpColor + '11', cp.faction > 0 ? 2.5 : 1.5); // <-- Aggiunto ctx
        ctx.restore();
...
// Aggiunto "targetCtx" come primo parametro per renderla a prova di bomba
function drawHex(targetCtx, q, r, stroke, fill, width = 1) {
