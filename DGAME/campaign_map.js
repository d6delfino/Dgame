...
function _initSectorProperties() {
    ...
    const upgradePool = window.CAMPAIGN_UPGRADE_DEFS.filter(d =>
        !['nukeUnlockUpgrade', 'icbmUpgrade'].includes(d.key) // troppo potenti come regalo iniziale
    );
    ...
    placed.forEach(sectorId => {
        ...
        if (upg.key === 'mineUpgrade') s.income += 2;
        // Artiglieria su neutrali: non assegna cariche (nessun owner)
...
function renderCampaignMap() {
    ...
    for (let p = 1; p <= n; p++) {
        ...
        const isLiveConnected = !window.isCampaignOnline
    || p === 1                          // l'host è sempre connesso
    || (typeof clientConns !== 'undefined' && clientConns[p])  // host: connessione attiva
    || (typeof window.myPlayerNumber !== 'undefined' && p === window.myPlayerNumber); // io stesso

        let statusIcon = isConfirmed ? ' ✅' : '';
        if (players[p].isDisconnected && !isLiveConnected) {
            statusIcon = ' <span style="color:#ff3333; text-shadow: 0 0 5px #f00;">❌</span>';
        }
...
/**
 * Utility Robusta per la pressione continua
 */
function _setupCreditAutoRepeat(btn, sectorId, delta) {
    
    // Funzione interna per fermare tutto
    const stopGlobalRepeating = () => {
        ...
        // Rimuoviamo i listener globali per non sporcare la memoria
        window.removeEventListener('pointerup', stopGlobalRepeating);
        window.removeEventListener('pointercancel', stopGlobalRepeating);
    };

    const startRepeating = (e) => {
        // Se sta già andando un ciclo, fermalo (sicurezza)
        stopGlobalRepeating();
        ...
        // 2. Agganciamo lo STOP alla finestra intera (window)
        // Così se il pulsante sparisce perché la mappa si ridisegna, lo stop funziona ancora
        window.addEventListener('pointerup', stopGlobalRepeating);
        window.addEventListener('pointercancel', stopGlobalRepeating); // Per i touch interrotti

        // 3. Avvia il timer di 1 secondo per la ripetizione
        _creditAutoTimeout = setTimeout(() => {
            _creditAutoInterval = setInterval(() => {
                allocSectorCredit(sectorId, delta);
            }, 75); // Velocità ripetizione
        }, 1000); // Attesa iniziale
    };

    // Applichiamo al pulsante
    btn.onpointerdown = startRepeating;
    
    // Protezione per il menu contestuale (tasto destro)
