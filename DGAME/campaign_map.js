...
function handleSectorClick(targetId) {
    ...
    // Sicurezza online: Blocca i click se non è il tuo turno (sia per host che per client)
    if (window.isCampaignOnline && p !== window.myPlayerNumber) {
    ...
    // GESTIONE PROPRIETÀ: Se il settore è MIO, apri il pannello upgrade LOCALMENTE.
    // Questo vale sia in locale, sia per l'host, sia per il client!
    if (target.owner === p) {
    ...
    // Se è un attacco e sono un client online: delega all'host
