...
function renderCampaignMap() {
    ...
    overlay.innerHTML = `
        ...
        <div style="position:absolute; top:45px; right:5px; z-index:100001; display:flex; flex-direction:column; gap:5px; align-items:flex-end;">
            <button id="campaign-info-btn" style="background:rgba(0,0,0,0.8); color:${pColor}; border:1px solid ${pColor}; padding:10px 20px; cursor:pointer; font-family:'Courier New'; font-weight:bold; border-radius:5px; font-size:14px;">ⓘ INFO</button>
        </div>
...
function allocSectorCredit(sectorId, delta, fromNetPlayer = null) {
    ...
    if (p === localHumanPlayer) {
        ...
        const hqId = campaignState.hqSlots[p - 1];
