...
function _initSectorProperties() {
    ...
    const upgradePool = window.CAMPAIGN_UPGRADE_DEFS.filter(d =>
        !['nukeUnlockUpgrade', 'icbmUpgrade', 'revoltUpgrade'].includes(d.key) // troppo potenti come regalo iniziale
    );
...
function renderCampaignMap() {
    ...
    || (typeof window.myPlayerNumber !== 'undefined' && p === window.myPlayerNumber); // io stesso
        ...
        if (isAI) {
            // Dopo 12 sec di disconnessione subentra il bot
            statusIcon = ' 🤖';
        } else if (players[p].isDisconnected && !isLiveConnected) {
            // Nei primi 12 sec o prima della conversione
