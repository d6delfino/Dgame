...
function _coopDrawFog() {
    ...
    // 1. CREA "L'IMPRONTA DIGITALE" DEGLI UMANI
    let currentHash = "";
    for (let p = 1; p <= window._coopHumanPlayers; p++) {
        (players[p].agents || []).forEach(a => {
    ...
    // 2. CONTROLLA LA CACHE: Qualcuno si è mosso o ha cambiato Gittata?
    if (coopState._lastFogHash !== currentHash || !coopState.currentVisible) {
        
        // C'è stato un cambiamento! Ricalcoliamo la matematica (usando i cerchi esagonali ottimali)
        ...
        for (let p = 1; p <= window._coopHumanPlayers; p++) {
            (players[p].agents || []).forEach(agent => {
                ...
                // Matematica esagonale per raggio variabile (range)
        ...
        // Usa un blocco protetto: se la logica delle quest fallisce, la grafica continuerà a disegnare la nebbia!
        try {
        ...
        // Salviamo i risultati in memoria per il prossimo frame
        coopState.currentVisible = visible;
        coopState._lastFogHash = currentHash;
    }

    // Recuperiamo il Set visibile dalla memoria (che sia appena calcolato o vecchio)
    const visible = coopState.currentVisible;

    ctx.save();
    
    // 3. DISEGNO GPU OTTIMIZZATO: Tutti gli esagoni NERI PURI in un colpo solo
    ctx.beginPath();
    grid.forEach(cell => {
        const key = getKey(cell.q, cell.r);
        if (!visible.has(key) && !coopState.cellsExplored.has(key)) {
    ...
    // 4. DISEGNO GPU OTTIMIZZATO: Tutti gli esagoni GRIGI (Esplorati ma bui) in un colpo solo
    ctx.beginPath();
    grid.forEach(cell => {
        const key = getKey(cell.q, cell.r);
        if (!visible.has(key) && coopState.cellsExplored.has(key)) {
