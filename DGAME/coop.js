...
const COOP = {
    ...
    STARTING_CREDITS:     8,    // crediti da spendere nel setup iniziale
    ...
    MONSTER_DMG_MIN:      1,
    MONSTER_DMG_MAX:      3,
    MONSTER_CARDS_TURN:   5,    // turno da cui i mostri usano le carte (5)
    MONSTER_HARD_TURN:   11,    // turno da cui i mostri ottengono statistiche complete (11)
    BOSS_FORCED_TURN:    20,    // se il boss non è ancora apparso, spawna comunque
...
// Mappatura cosmeticFaction → indice bgOptions per il tema mappa coop
const COOP_FACTION_THEME = {
    1: 2,  // Verde  → S3 (EC)
    2: 1,  // Viola  → S2 (ZE)
    3: 4,  // Blu    → S5 (ME)
    4: 3,  // Oro    → S4 (GU)
...
(function _installCoopWinHook() {
    ...
    window.checkWinConditions = function() {
        if (coopState.active) {
            //_coopVictory('TEST VITTORIA'); // ← TEMP: rimuovere dopo il test
...
function _coopRegisterHooks() {
    ...
    registerTurnResetHook(function () {
        ...
        if (currentPlayer === firstAliveHuman) {
            ...
            // TIMER BOSS: al turno BOSS_FORCED_TURN, se non è ancora apparso, forza lo spawn
            if (coopState.turnCount >= COOP.BOSS_FORCED_TURN &&
                !coopState.bossSpawned &&
                coopState.bossesDefeated < coopState.totalBossesToKill &&
                coopState.exitCell) {
                coopState.exitExplored = true;
                _coopSpawnBoss(true);
                if (typeof showNotificationBanner === 'function') {
                    showNotificationBanner(
                        '☠️ Il Boss ha sentito il vostro odore — sta arrivando!',
                        '#ff3333', { top: '80px', duration: 6000 }
                    );
                }
...
function _coopAssignRandomMonsterFaction() {
    ...
    // Applica il tema mappa corrispondente alla fazione AI scelta
    applyTheme(bgOptions[COOP_FACTION_THEME[chosen]]);
}
...
function _coopCreateMonster(baseHp) {
    ...
    // --- SCALING INIZIALE ---
    // Fino al turno 10 (compreso), i mostri hanno statistiche deboli (da 1 a 2)
    if (coopState.turnCount < COOP.MONSTER_HARD_TURN) {
        hp  = baseHp || (1 + Math.floor(Math.random() * 2));
        //dmg = 1 + Math.floor(Math.random() * 2);
        dmg = 1;
        rng = 1 + Math.floor(Math.random() * 2);
        mov = 1 + Math.floor(Math.random() * 2);
    } else {
        // Da MONSTER_HARD_TURN in poi, statistiche complete
        hp  = baseHp || (COOP.MONSTER_HP_MIN + Math.floor(Math.random() * (COOP.MONSTER_HP_MAX - COOP.MONSTER_HP_MIN + 1)));
        dmg = COOP.MONSTER_DMG_MIN + Math.floor(Math.random() * (COOP.MONSTER_DMG_MAX - COOP.MONSTER_DMG_MIN + 1));
        rng = COOP.MONSTER_RNG_MIN + Math.floor(Math.random() * (COOP.MONSTER_RNG_MAX - COOP.MONSTER_RNG_MIN + 1));
        mov = COOP.MONSTER_MOV;
    }
    ...
    return {
        ...
        hp, maxHp: hp, mov: mov, rng, // Usa la variabile mov dinamica
...
function _coopSpawnBoss(silent = false) {
    ...
    const boss = {
        id: `coop_boss_${coopState.bossesDefeated + 1}`, type: 'agent', faction: COOP.MONSTER_FACTION,
...
function _coopProgressExploreQuest(newCells) {
    // FIX: Assicura che le variabili esistano sul Client per evitare crash silenziosi!
    if (!coopState.cellsExplored) coopState.cellsExplored = new Set();
    if (!coopState.cornersExplored) coopState.cornersExplored = new Set();
...
function _coopShowRecruitPanel(faction, village) {
    ...
    const renderStats = () => {
        const statTypes = [
            { id: 'hp',  icon: '💚', label: 'Vita' },
            { id: 'mov', icon: '👣', label: 'Passi' },
            { id: 'rng', icon: '🎯', label: 'Tiro' },
            { id: 'dmg', icon: '⚔️', label: 'Danno' },
        ];

        const statsHtml = `
            <div style="display:flex; flex-direction:column; gap:6px; margin:15px 0;">
                ${statTypes.map(s => `
                    <div style="display:flex; align-items:center; background:#0a0a10; border:1px solid #333; padding:6px 10px; border-radius:4px; box-sizing:border-box;">
                        <span style="font-size:11px; color:#888; text-transform:uppercase; display:flex; align-items:center; gap:6px; white-space:nowrap;">
                            <span style="font-size:14px;">${s.icon}</span>${s.label}
                        </span>
                        <div style="margin-left:auto; display:flex; align-items:center; gap:10px;">
                            <button class="stat-btn" onclick="window._coopRecAdjust('${s.id}',-1)" style="width:24px; height:24px; padding:0;">-</button>
                            <span id="coop-stat-${s.id}" style="font-size:18px; font-weight:bold; color:#fff; width:20px; text-align:center;">${tmpAgent[s.id]}</span>
                            <button class="stat-btn" onclick="window._coopRecAdjust('${s.id}',1)" style="width:24px; height:24px; padding:0;">+</button>
                        </div>
                    </div>
                `).join('')}
            </div>
            <div style="background:rgba(255,215,0,0.05); border:1px solid rgba(255,215,0,0.2); padding:10px; border-radius:6px; margin-bottom:15px;">
                <div style="color:#FFD700; font-size:13px; margin-bottom:2px;">Bonus Potenziamento: <b id="coop-extra-pts">${extraPoints}</b></div>
                <div style="color:#fff; font-size:15px; font-weight:bold;">INVESTIMENTO TOTALE: 💰${COOP.VILLAGE_RECRUIT_COST + extraPoints}</div>
            </div>
        `;
    ...
    // Selettore sprite identico a quello del setup
    const pColor = players[faction].color;
    let currentSlot = parseInt(tmpAgent.customSpriteId.replace(fData.prefix, '')) || 1;

    function buildSpriteContent(slot) {
        const id  = `${fData.prefix}${slot}`;
        const url = customSpriteFiles[id];
        if (url) {
            return `<img src="${url}" style="width:100px;height:100px;object-fit:contain;display:block;pointer-events:none;">`;
        }
        return `<span style="font-size:64px;line-height:1;">${tmpAgent.sprite}</span>`;
    }

    function applySlot(slot) {
        currentSlot = slot;
        tmpAgent.customSpriteId = `${fData.prefix}${slot}`;
        document.getElementById('coop-sprite-wrap').innerHTML = buildSpriteContent(slot);
    }
    ...
    panel.innerHTML = `
        ...
        <div style="position:relative;display:flex;align-items:center;justify-content:center;height:110px;margin-bottom:8px;">
            <button id="coop-sprite-prev" style="position:absolute;left:0;top:0;height:100%;width:40%;background:none;border:none;color:${pColor};font-size:28px;cursor:pointer;display:flex;align-items:center;justify-content:flex-start;padding-left:8px;z-index:2;">&#8249;</button>
            <div id="coop-sprite-wrap" style="display:flex;align-items:center;justify-content:center;pointer-events:none;">
                ${buildSpriteContent(currentSlot)}
            </div>
            <button id="coop-sprite-next" style="position:absolute;right:0;top:0;height:100%;width:40%;background:none;border:none;color:${pColor};font-size:28px;cursor:pointer;display:flex;align-items:center;justify-content:flex-end;padding-right:8px;z-index:2;">&#8250;</button>
        </div>
    ...
    document.getElementById('coop-sprite-prev').onclick = (e) => {
        e.stopPropagation();
        applySlot(currentSlot <= 1 ? fData.count : currentSlot - 1);
    };
    document.getElementById('coop-sprite-next').onclick = (e) => {
        e.stopPropagation();
        applySlot(currentSlot >= fData.count ? 1 : currentSlot + 1);
    };
...
function _coopApplyRecruit(faction, agentData, cost, vq, vr) {
    ...
    // FIX SPARIZIONE: Annulla ogni movimento/attacco in corso per evitare sovrascritture
...
function _coopApplyRevive(reviverFaction, deadFaction, vq, vr) {
    ...
    // FIX SPARIZIONE: Annulla ogni movimento/attacco in corso
    if (typeof cancelAction === 'function') cancelAction();
...
function _coopDrawFog() {
    ...
    // HEX_SIZE incluso: uno zoom deve invalidare la cache (cambia la geometria locale)
    let currentHash = `Z${HEX_SIZE}|`;
    for (let p = 1; p <= window._coopHumanPlayers; p++) {
        (players[p].agents || []).forEach(a => {
            if (a.hp > 0) currentHash += `${a.id}:${a.q},${a.r},${a.rng}|`;
        });
    }

    if (coopState._lastFogHash !== currentHash || !coopState.currentVisible) {
        ...
        for (let p = 1; p <= window._coopHumanPlayers; p++) {
            (players[p].agents || []).forEach(agent => {
                ...
                const range = agent.rng;
                for (let dq = -range; dq <= range; dq++) {
                    for (let dr = Math.max(-range, -dq - range); dr <= Math.min(range, -dq + range); dr++) {
                        const cell = grid.get(getKey(agent.q + dq, agent.r + dr));
                        if (cell) {
        ...
        try {
            ...
            console.error("[Fog] Errore ignorato per salvare il rendering:", e);
        }

        const pathDark     = new Path2D();
        const pathExplored = new Path2D();

        grid.forEach(cell => {
            const key = getKey(cell.q, cell.r);
            if (visible.has(key)) return;

            const target = coopState.cellsExplored.has(key) ? pathExplored : pathDark;

            // Coordinate LOCALI: solo q, r, HEX_SIZE — NIENTE offsetX/offsetY/centratura schermo
            const lx = HEX_SIZE * (_SQRT3 * cell.q + _SQRT3_2 * cell.r);
            const ly = HEX_SIZE * (1.5 * cell.r);
            for (let i = 0; i < 6; i++) {
                const angle = (Math.PI / 3) * i + Math.PI / 6;
                const x = lx + (HEX_SIZE + 1) * Math.cos(angle);
                const y = ly + (HEX_SIZE + 1) * Math.sin(angle);
                i === 0 ? target.moveTo(x, y) : target.lineTo(x, y);
            }
            target.closePath();
        });

        coopState._fogPathDark     = pathDark;
        coopState._fogPathExplored = pathExplored;
        coopState.currentVisible   = visible;
        coopState._lastFogHash     = currentHash;
    }

    // Pan e centratura applicati DAL VIVO ogni frame: non serve mai invalidare la cache per il pan
    ctx.save();
    ctx.translate((window.innerWidth / 2) + offsetX, (window.innerHeight / 2) + offsetY);
    ctx.fillStyle = `rgba(0, 0, 0, ${COOP.FOG_ALPHA})`;
    ctx.fill(coopState._fogPathDark);
    ctx.fillStyle = `rgba(0, 0, 0, ${COOP.FOG_ALPHA_EXPLORED})`;
    ctx.fill(coopState._fogPathExplored);
    ctx.restore();
}
...
function _coopCheckExitVictory() {
    ...
    if (allReached) _coopVictory('I giocatori hanno raggiunto la via di fuga!');
}
...
function showCoopMenu() {
    ...
    block.innerHTML = `
    <div style="display: flex; flex-direction: column; align-items: flex-start; margin-bottom: 25px; border-top: 1px solid #333; padding-top: 15px;">
        <div style="display: flex; align-items: center; margin-bottom: 15px;">
            <!-- Ancora da 75px per spostare la scritta a destra -->
            <div style="width: 175px; height: 30px; position: relative; flex-shrink: 0;">
                <!-- Immagine opaca e ingrandita -->
                <img src="img/coop.png" style="height: 60px; position: absolute; top: 50%; left: -5px; transform: translateY(-50%); pointer-events: none; opacity: 1; z-index: 3;">
            </div>
            <!-- Titolo Blu allineato agli altri -->
            <span style="color: #00aaff; font-size: 18px; text-transform: uppercase; font-weight: bold; letter-spacing: 2px; position: relative; z-index: 1;">Cooperativa</span>
        </div>
        <div style="display:flex; gap:4px; flex-wrap:nowrap; width: 100%;">
            ${[1,2,3,4].map(n => `
                <button class="action-btn" onclick="startCoopGame(${n})"
                style="flex: 1; padding:10px 0; border:2px solid #00aaff; color:#00aaff; background:transparent; font-size:12px; min-width: 0; position:relative; z-index:2;">
                ${n}P
                </button>
            `).join('')}
        </div>
...
function _coopSerializeState() {
    return {
        ...
        exitExplored:  coopState.exitExplored,
...
function _coopApplySnapshot(snapshot, numHumanPlayers) {
    ...
    coopState.exitExplored  = snapshot.exitExplored  || false;
    ...
    setTimeout(function() {
        ...
        if (typeof registerClientMessageHandler === 'function') {
            registerClientMessageHandler('COOP_STATE_SYNC', function(data) {
                ...
                // FIX RICONNESSIONE: Se la coop non è ancora attiva sul client (perché ha appena 
                // ricevuto il sync base della mappa da una riconnessione), la inizializziamo ora!
                if (!coopState.active) {
                    _coopApplySnapshot(data.coopSnapshot, data.coopHumanPlayers || window._coopHumanPlayers || 1);
                }
...
setTimeout(function() {
    ...
    // Verifica che la spawnCell indicata dall'host sia ancora libera sul client.
    // Se è stata occupata nel frattempo (es. un agente si è mosso sopra),
    // cerca la prima cella adiacente libera per evitare sovrascrittura silenziosa.
    const targetCell = grid.get(getKey(data.agent.q, data.agent.r));
    if (targetCell && targetCell.entity && targetCell.entity !== data.agent) {
        let fallback = null;
        hexDirections.some(dir => {
            const c = grid.get(getKey(data.agent.q + dir.q, data.agent.r + dir.r));
            if (c && c.type === 'empty' && !c.entity) { fallback = c; return true; }
        });
        if (fallback) {
            data.agent.q = fallback.q;
            data.agent.r = fallback.r;
        } else {
            console.warn('[COOP_RECRUIT_SYNC] Nessuna cella libera per la recluta', data.agent.id);
        }
    }
