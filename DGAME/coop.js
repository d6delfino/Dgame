...
const COOP = {
    ...
    STARTING_CREDITS:    100,    // crediti da spendere nel setup iniziale
...
(function _installCoopClickHook() {
    ...
    setTimeout(() => {
        ...
        window.handleCanvasClick = function (e) {
            ...
            // Villaggio
            // In online il client usa myPlayerNumber, in locale usa currentPlayer
            const actingPlayer = (typeof isOnline !== 'undefined' && isOnline && !isHost)
                ? myPlayerNumber
                : currentPlayer;
            
            // FIX: Cerca direttamente nello stato, ignorando se la griglia ha perso i flag
            const isVillage = coopState.villages.some(v => v.q === hex.q && v.r === hex.r);

            if (cell && isVillage && selectedAgent &&
                hexDistance(selectedAgent, cell) <= 1 &&
                selectedAgent.faction === actingPlayer &&
            ...
            // FIX: Cerca direttamente nello stato (sistema lo stesso bug anche per le tane!)
            const isLair = coopState.lairs.some(l => l.q === hex.q && l.r === hex.r && !l.destroyed);
    ...
    if (activeLairs.length > 0) {
        ...
        if (coopState.virtualLairTimer <= 0) {
            ...
            if (candidates.length > 0) {
                ...
                // --- LOGICA DI INVASIONE SCALATA ---
                const numHumans = window._coopHumanPlayers || 1;
                
                // Sceglie a caso se mandare 2 o 3 mostri per ogni giocatore
                const moltiplicatore = 2 + Math.floor(Math.random() * 2); 
                
                // Calcola il totale (es. 4 giocatori * 3 = 12 mostri)
                const spawnCount = Math.min(candidates.length, numHumans * moltiplicatore);
                ...
                if (typeof showNotificationBanner === 'function') {
                    showNotificationBanner(
                        `⚠️ RILEVATA INVASIONE NEMICA DI MASSA!<br>(${spawnCount} nuovi contatti rilevati)`, 
                        '#ff0000', { duration: 5000, bold: true }
                    );
                }
            }
        }
            ...
            <div style="display:flex; gap:8px; justify-content:center; flex-wrap:wrap;">
                ${[1,2,3,4].map(n => `
                    <button class="action-btn" onclick="startCoopGame(${n})"
                        style="padding:10px 18px; border:2px solid #00aaff; color:#00aaff; background:transparent; cursor:pointer;">
                        ${n}P COOP
                    </button>
                `).join('')}
            </div>
