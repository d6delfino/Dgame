...
const CAMPAIGN_UPGRADE_DEFS = [
    ...
    { key: 'fortressUpgrade',  icon: '🏰', name: 'Fortezza',         cost: () => CAMPAIGN.UPGRADE_FORTRESS_COST,  desc: 'Annulla gli attacchi di artiglieria sul settore. Rendita: -2.' },
    ...
    { key: 'revoltUpgrade',    icon: '✊', name: 'Rivolta',           cost: () => CAMPAIGN.UPGRADE_REVOLT_COST,    desc: `A fine turno, tutti i settori con meno di ${CAMPAIGN.UPGRADE_REVOLT_THRESHOLD} crediti tornano neutrali. Effetto unico.` },
    { key: 'infiltratiUpgrade', icon: '🕵️', name: 'Infiltrati',      cost: () => CAMPAIGN.UPGRADE_INFILTRATI_COST, desc: 'Permette un attacco a qualsiasi settore sulla mappa. Monouso: viene consumato all\'uso.' }
];
...
function _isInfiltratiReachable(targetId, p) {
    // Basta avere almeno un settore con infiltratiUpgrade
...
function showCreditSelector(playerFaction, targetSectorId) {
    ...
    const infiltratiPossible  = _isInfiltratiReachable(targetSectorId, playerFaction);
    const canInfiltrati       = infiltratiPossible && avail >= 4;
    ...
    const modal = _gui_createModalBase(playerFaction, "⚔️ ORDINE AZIONE", `Bersaglio: Settore ${targetSectorId}`, content, [
        ...
        { id: 'btn-artillery', label: `🎯 ARTIGLIERIA (Salve: ${artCharges})`, primary: true,  disabled: !canArtillery },
    ...
    modal.querySelector('#btn-artillery').onclick = () => {
        ...
        if (window.isCampaignOnline && !window.isHost) _net_clientSend('ARTILLERY_STRIKE', { sectorId: targetSectorId });
            ...
            if (!campaignState.artilleryCharges[playerFaction] ||
                campaignState.artilleryCharges[playerFaction] <= 0) return;
                campaignState.artilleryCharges[playerFaction]--;
            if (!campaignState._roundLog) campaignState._roundLog = [];
                campaignState._roundLog.push({ type: 'ARTILLERY', p: playerFaction, sid: targetSectorId });
        renderCampaignMap();
        }
    };

    modal.querySelector('#btn-infiltrati').onclick = () => {
        modal.remove();
        // Consuma il primo settore con infiltratiUpgrade
        if (window.isCampaignOnline && !window.isHost) {
            _net_clientSend('INFILTRATI_ATTACK', { sectorId: targetSectorId });
        } else {
            // Trova e consuma l'upgrade
            const sourceSector = campaignState.sectors.find(s => s.owner === playerFaction && s.infiltratiUpgrade === true);
            if (sourceSector) sourceSector.infiltratiUpgrade = false;
            _applyOrderWithCredits(targetSectorId, parseInt(modal.querySelector('#cr-slider')?.value || 4), playerFaction);
            renderCampaignMap();
        }
    };
