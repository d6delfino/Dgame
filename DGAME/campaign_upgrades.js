...
function showCreditSelector(targetSectorId, netData = null) {
    ...
    const sendOrApply = (type, payload) => {
        ...
        if (isClient) {
            ...
            if (type === 'CONFIRM_CREDIT_ORDER') {
                ...
                _applyOrderWithCredits(targetSectorId, payload.credits, p, {
                    isInfiltrati: true,
                    fromX: source ? source.x : null,
                    fromY: source ? source.y : null,
                });
            }
            renderCampaignMap();
        }
    ...
    modal.querySelector('#btn-nuke').onclick = () => {
        // Usiamo uno span con display:block e un margine sinistro per centrare il blocco ma allineare il testo
    ...
    modal.querySelector('#btn-artillery').onclick = () => {
        showCustomConfirm(
            ...
            "Confermi il bombardamento su questo settore? Ridurrà i crediti in difesa e distruggerà gli upgrade.", 
            "FUOCO!", 
