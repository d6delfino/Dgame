...
const CAMPAIGN_UPGRADE_DEFS = [
    ...
    { key: 'hangarUpgrade',    icon: '🛩️', name: 'Hangar',           cost: () => CAMPAIGN.UPGRADE_HANGAR_COST,    desc: 'Trasporto Aereo: le tue unità si muovono di +1 settore sulla mappa strategica.' },
    ...
    { key: 'nukeUnlockUpgrade',icon: '☢️', name: 'Silo Nucleare',    cost: () => CAMPAIGN.UPGRADE_NUKE_UNLOCK_COST, desc: 'ottieni armi nucleari che annientano un settore adiacente.' },
    ...
    { key: 'infiltratiUpgrade', icon: '🕵️', name: 'Infiltrati',      cost: () => CAMPAIGN.UPGRADE_INFILTRATI_COST, desc: 'Permette a un tuo Agente di muoversi in qualsiasi settore (escluse Fortezze). Effetto consumabile dal menu Unità.' }
];
...
function getPlayerCampaignBonuses(playerFaction) {
    ...
    // Le cariche di artiglieria ora derivano dalle unità mobili possedute, non da un edificio fisso
    b.artilleryCount = campaignState.units.filter(u => u.owner === playerFaction && u.type === 'artillery').length;
    return b;
}
...
function showSectorUpgradePanel(playerFaction, sectorId) {
    ...
    const agentMin = CAMPAIGN.UNIT_AGENT_MIN_VALUE;

    const unitContentHtml = `
        <div style="background:rgba(255,255,255,0.04);border:1px solid #333;border-radius:8px;padding:10px;margin-bottom:10px;">
            <b style="color:#fff;font-size:14px;">CREA AGENTE</b>
            <small style="color:#aaa;"> 👤 investimento minimo 💰${agentMin}</small>
            <div style="display:flex;align-items:center;justify-content:center;gap:8px;margin:8px 0;">
                <button id="unit-val-minus" style="background:rgba(0,0,0,0.9);border:1px solid #666;color:#ccc;width:32px;height:32px;border-radius:4px;cursor:pointer;font-size:20px;font-weight:bold;">−</button>
                <span id="unit-val-display" style="color:#FFD700;font-size:20px;font-weight:bold;min-width:70px;text-align:center;">💰${Math.min(agentMin, avail)}</span>
                <button id="unit-val-plus" style="background:rgba(0,0,0,0.9);border:1px solid #666;color:#ccc;width:32px;height:32px;border-radius:4px;cursor:pointer;font-size:20px;font-weight:bold;">+</button>
            </div>
            <button id="unit-create-agent" class="action-btn" style="width:100%;padding:8px;cursor:pointer;border:2px solid #555;color:#ccc;background:transparent;" ${avail < agentMin ? 'disabled' : ''}>👤 Crea Agente</button>
        </div>`;

    const modal = _gui_createModalBase(playerFaction, "🛠️ COSTRUISCI", `Settore ${sectorId} | Disponibili: 💰${avail}`, unitContentHtml + contentHtml, [{ id: 'btn-close', label: 'CHIUDI', primary: false }]);

    // --- Selettore importo Agente (minimo CAMPAIGN.UNIT_AGENT_MIN_VALUE) ---
    let unitValue = Math.min(agentMin, avail);
    const valDisplay = modal.querySelector('#unit-val-display');
    const refreshUnitVal = () => { valDisplay.textContent = `💰${unitValue}`; };
    modal.querySelector('#unit-val-minus').onclick = () => { if (unitValue > agentMin) { unitValue--; refreshUnitVal(); } };
    modal.querySelector('#unit-val-plus').onclick  = () => { if (unitValue < avail) { unitValue++; refreshUnitVal(); } };

    modal.querySelector('#unit-create-agent').onclick = () => {
        if (unitValue < agentMin || campaignState.credits[playerFaction] < unitValue) return;
        modal.remove();
        if (typeof playSFX === 'function') playSFX('build');
        if (window.isCampaignOnline && !window.isHost) {
            _net_clientSend('CREATE_UNIT', { sectorId, value: unitValue, unitType: 'agent' });
        } else {
            campaignState.credits[playerFaction] -= unitValue;
            createCampaignUnit(sectorId, playerFaction, unitValue, 'agent');
            renderCampaignMap();
...
function _applySectorUpgradeLocale(playerFaction, sectorId, upgradeKey, cost) {
    ...
    // L'Artiglieria è un'unità mobile e non un'infrastruttura fissa permanente:
    // non deve bloccare lo slot di acquisto del settore.
    ...
    if(upgradeKey === 'artilleryUpgrade') {
        ...
        createCampaignUnit(sectorId, playerFaction, cost, 'artillery'); // nasce anche il token mobile, indipendente dalle cariche
    }
...
function showBonificaPanel(playerFaction, sectorId) {
    ...
    const content = `<div style="background:rgba(255,255,255,0.05);padding:15px;border-radius:10px;text-align:left;color:#ccc;font-size:14px;">
        Bonifica questa zona per renderla da subito un settore neutrale conquistabile.<div style="color:#FFD700;font-size:20px;margin-top:10px;font-weight:bold;text-align:center;">Costo: 💰${cost}</div></div>`;
    const modal = _gui_createModalBase(playerFaction, "☣️ BONIFICA", `Settore ${sectorId}`, content, [
        {
            id: 'btn-confirm',
            // Emoji biohazard raddoppiata (36px vs i 18px del testo) — line-height fisso a 18px
            // mantiene invariata l'altezza di riga del pulsante, stessa logica usata per "muovi" e "spara"
            label: `<span style="font-size:36px;line-height:18px;vertical-align:middle;display:inline-block;">🏗️</span> ESEGUI BONIFICA`,
            primary: true,
            disabled: !canBuy
        },
...
function _gui_createModalBase(playerFaction, title, subtitle, contentHtml, buttons = []) {
    ...
    let buttonsHtml = buttons.map(btn => `
        <button id="${btn.id}" class="action-btn" style="width:100%;padding:14px;font-size:18px;font-weight:bold;border-radius:8px;cursor:pointer;
            ${btn.primary ? `border:3px solid ${pColor};color:${pColor};background:${pColor}22;` : `border:2px solid ${pColor};color:${pColor};background:${pColor}22;margin-bottom:8px;`}"
    ...
    modal.innerHTML = `
        <div style="background:rgba(5,10,20,0.98);border:4px solid ${pColor};border-radius:15px;padding:22px;width:100%;max-width:500px;text-align:center;box-shadow:0 0 40px rgba(0,0,0,1);box-sizing:border-box;max-height:95vh;overflow-y:auto;">
            ...
            <div style="color:#aaa;font-size:14px;margin-bottom:12px;text-transform:uppercase;">${subtitle}</div>
            <div id="modal-button-area" style="margin-bottom:16px;">${buttonsHtml}</div>
...
/** Gittata effettiva dell'artiglieria per un giocatore: globale (illimitata) con l'ICBM. */
function _getArtilleryRange(owner) {
    return getPlayerCampaignBonuses(owner).icbmUnlocked ? Infinity : CAMPAIGN.ARTILLERY_RANGE;
}
window._getArtilleryRange = _getArtilleryRange;
...
function showCreditSelector(targetSectorId, netData = null) {
    ...
    // --- 2. DISEGNO INTERFACCIA (Senza Slider e investimenti obsoleti) ---
    const content = `
        <div style="background:rgba(255,255,255,0.05);padding:10px;border-radius:8px;color:#888;margin-bottom:15px;text-align:center;">
            ${data.sectorOwner > 0 ? `Difensore: 🏦 ${data.defCredits} cr` : 'Settore Neutrale'}
        </div>
        <div style="color:#aaa;font-size:13px;text-align:center;margin-bottom:10px;">In Banca: <strong style="color:#FFD700;">💰${data.availCredits}</strong></div>
    `;
    ...
    modal.querySelector('#btn-nuke').onclick = () => {
        const htmlMsg = `<b>EFFETTI DEVASTANTI:</b><br><br>
                         <span style="display: block; text-align: left; width: 80%; margin: 0 auto;">
                            1. Il settore verrà BLOCCATO per 4 turni.<br><br>
                            2. Verranno distrutti tutti i crediti e gli upgrade.<br><br>
                            3. Se i nemici attaccano qui, perderanno tutto!<br><br>
                         </span><br>
                         Vuoi procedere?`;

        showCustomConfirm("☢️ NUCLEARIZZAZIONE", htmlMsg, "NUCLEARIZZA", () => {
...
function _applyOrderWithCredits(sectorId, credits, playerFaction, extra = {}) {
    const avail = campaignState.credits[playerFaction] || 0;
    ...
    campaignState.pendingOrders[playerFaction].push({ sectorId, credits, ...extra });
...
function _applyArtilleryStrikeLocale(sectorId, playerFaction) {
    ...
    // Applica danno al valore degli Agenti nemici presenti nel settore colpito
    // (sostituisce il vecchio danno ai "crediti del settore", ora che il valore
    // combattivo vive nelle unità Agente e non più in campaignState.sectorCredits).
    // L'Artiglieria stessa non subisce danno da questo colpo.
    // FIX TIMING: usa la posizione di FINE TURNO (dopo il movimento in sospeso), non
    // quella di inizio turno — un'unità che fugge dal settore si salva, una che vi
    // entra proprio ora viene colpita.
    campaignState.units
        .filter(u => _getUnitEndOfRoundSector(u) === sectorId && u.type === 'agent' && u.owner !== playerFaction)
        .forEach(u => {
            u.value = Math.max(0, u.value - Math.ceil(u.value * CAMPAIGN.ARTILLERY_CREDIT_DMG_PERCENT));
        });
    campaignState.units = campaignState.units.filter(u =>
        !(_getUnitEndOfRoundSector(u) === sectorId && u.type === 'agent' && u.value <= 0));

    const targetOwner = sector.owner;

    // Compatibilità: se esistesse ancora un valore in sectorCredits (vecchio sistema), lo riduce comunque
...
async function _playNukeAnimation(fromX, fromY, toX, toY) {
    return new Promise(resolve => {
        ...
        if (typeof playSFX === 'function') playSFX('airdrop');
        ...
        function animateMissile(now) {
            ...
            if (t < 1) {
                ...
                // Seconda esplosione dopo 1 secondo (1000ms)
                setTimeout(() => {
                    if (typeof playSFX === 'function') playSFX('crollo'); 
                    // Nota: puoi usare 'boom' di nuovo o un altro suono se lo aggiungi
                }, 1000);
                ...
                const flashDuration = 3800;
                const blastDuration = 3800;
...
async function _playMinefieldAnimation(fromX, fromY, toX, toY) {
    return new Promise(resolve => {
        const svg = document.getElementById('map-svg');
        if (!svg) { resolve(); return; }

        // Truppe che avanzano verso le mine
        const troops = document.createElementNS('http://www.w3.org/2000/svg', 'text');
        troops.setAttribute('text-anchor', 'middle');
        troops.setAttribute('dominant-baseline', 'middle');
        troops.setAttribute('font-size', '28');
        troops.setAttribute('font-family', '"Apple Color Emoji","Segoe UI Emoji",sans-serif');
        troops.setAttribute('pointer-events', 'none');
        troops.textContent = '⚔️';
        svg.appendChild(troops);

        // Icona mine sul settore bersaglio
        const mineIcon = document.createElementNS('http://www.w3.org/2000/svg', 'text');
        mineIcon.setAttribute('text-anchor', 'middle');
        mineIcon.setAttribute('dominant-baseline', 'middle');
        mineIcon.setAttribute('font-size', '28');
        mineIcon.setAttribute('font-family', '"Apple Color Emoji","Segoe UI Emoji",sans-serif');
        mineIcon.setAttribute('pointer-events', 'none');
        mineIcon.setAttribute('x', toX);
        mineIcon.setAttribute('y', toY);
        mineIcon.textContent = '🛸';
        svg.appendChild(mineIcon);

        const duration = 3200; // più lento del proiettile — truppe che marciano
        const startTime = performance.now();

        function animateTroops(now) {
            const t = Math.min((now - startTime) / duration, 1);
            const ease = t * t; // accelera leggermente
            const x = fromX + (toX - fromX) * ease;
            const y = fromY + (toY - fromY) * ease;
            troops.setAttribute('x', x);
            troops.setAttribute('y', y);

            if (t < 1) {
                requestAnimationFrame(animateTroops);
            } else {
                // Truppe e mine spariscono all'impatto
                troops.remove();
                mineIcon.remove();
                playSFX('explosion');

                // Esplosione: cerchio arancione — stesso stile artiglieria
                const blast = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
                blast.setAttribute('cx', toX);
                blast.setAttribute('cy', toY);
                blast.setAttribute('fill', 'rgba(255,140,0,0.4)');
...
window._playMinefieldAnimation = _playMinefieldAnimation;

async function _playRevoltAnimation(sectors) {
    return new Promise(resolve => {
        const svg = document.getElementById('map-svg');
        if (!svg || sectors.length === 0) { resolve(); return; }

        const duration = 2000;
        const startTime = performance.now();
        const amplitude = 22; // pixels su/giù

        // Crea un emoji pugno per ogni settore in rivolta
        const fists = sectors.map(s => {
            const fist = document.createElementNS('http://www.w3.org/2000/svg', 'text');
            fist.setAttribute('text-anchor', 'middle');
            fist.setAttribute('dominant-baseline', 'middle');
            fist.setAttribute('font-size', '32');
            fist.setAttribute('font-family', '"Apple Color Emoji","Segoe UI Emoji",sans-serif');
            fist.setAttribute('pointer-events', 'none');
            fist.setAttribute('x', s.x);
            fist.setAttribute('y', s.y);
            fist.textContent = '✊';
            svg.appendChild(fist);
            return { el: fist, baseY: s.y };
        });
        ...
        function animate(now) {
            const t = Math.min((now - startTime) / duration, 1);
            // Oscillazione su/giù: 3 cicli completi in 2 secondi
            const bounce = Math.sin(t * Math.PI * 6) * amplitude * (1 - t * 0.4);
            fists.forEach(f => {
                f.el.setAttribute('y', f.baseY + bounce);
            });

            if (t < 1) {
                requestAnimationFrame(animate);
            } else {
                fists.forEach(f => f.el.remove());
                resolve();
            }
        }
        requestAnimationFrame(animate);
...
window._playRevoltAnimation = _playRevoltAnimation;

async function _playInfiltratiAnimation(fromX, fromY, toX, toY) {
    return new Promise(resolve => {
        const svg = document.getElementById('map-svg');
        if (!svg) { resolve(); return; }

        const spy = document.createElementNS('http://www.w3.org/2000/svg', 'text');
        spy.setAttribute('text-anchor', 'middle');
        spy.setAttribute('dominant-baseline', 'middle');
        spy.setAttribute('font-size', '28');
        spy.setAttribute('font-family', '"Apple Color Emoji","Segoe UI Emoji",sans-serif');
        spy.setAttribute('pointer-events', 'none');
        spy.textContent = '🕵️';
        svg.appendChild(spy);
        ...
        function animate(now) {
            const t = Math.min((now - startTime) / duration, 1);
            // Movimento furtivo: easeInOut con leggera curva
            const ease = t < 0.5 ? 2*t*t : -1+(4-2*t)*t;
            const x = fromX + (toX - fromX) * ease;
            const y = fromY + (toY - fromY) * ease;
            spy.setAttribute('x', x);
            spy.setAttribute('y', y);
            // Opacità che pulsa leggermente — effetto "furtivo"
            spy.setAttribute('opacity', 0.6 + Math.sin(t * Math.PI * 8) * 0.4);
...
window._playInfiltratiAnimation = _playInfiltratiAnimation;
...
function _applyNuclearize(sectorId, playerFaction, cost = CAMPAIGN.NUCLEARIZE_COST) {
    ...
    // FIX: nel settore nuclearizzato non deve sopravvivere nulla — elimina anche le unità
    // persistenti (agenti e artiglierie, di QUALUNQUE fazione, incluso l'attaccante se si
    // trovava lì) presenti nel settore. Senza questo, un'unità rimasta sul posto lo
    // "riconquista pacificamente" tramite _resolveUnitConflicts() non appena finisce il
    // blocco, vanificando l'effetto del nuke.
    // FIX TIMING: usa la posizione di FINE TURNO (dopo il movimento in sospeso di questo
    // round), non quella di inizio turno — un'unità che scappa dal settore 20 verso il 21
    // nello stesso turno si salva; una che entra nel 20 dal 19 nello stesso turno muore.
    campaignState.units
        .filter(u => _getUnitEndOfRoundSector(u) === sectorId)
        .map(u => u.id)
        .forEach(uid => removeCampaignUnit(uid));
