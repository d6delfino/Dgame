...
window.onload = function () {
    ...
    audioKeys.forEach(key => {
        ...
        if (audio.readyState >= 3) assetItemProcessed();
            ...
            audio.addEventListener('canplaythrough', assetItemProcessed, { once: true });
            audio.addEventListener('error', assetItemProcessed, { once: true });
        }
    });
...
function monitorAsset(asset) {
    if (asset.complete) {
        ...
        asset.addEventListener('load', assetItemProcessed, { once: true });
        asset.addEventListener('error', assetItemProcessed, { once: true }); 
    }
}
...
function initGameEngine() {
    ...
    if (typeof SFX !== 'undefined') {
        ...
        playSFX('intro'); // Suono di conferma click
    ...
    // --- GESTIONE INPUT OTTIMIZZATA (CSS GPU PANNING) ---
    const TAP_MOVE_THRESHOLD = 22;
    let touchStartX = 0, touchStartY = 0;
    let touchHasMoved = false;

    // Variabili per l'ottimizzazione CSS
    let tempCssPanX = 0;
    let tempCssPanY = 0;
    let tempCssScale = 1;

    // Applica le trasformazioni CSS in modo fluido
    function applyCSSTransform() {
        canvas.style.transform = `translate(${tempCssPanX}px, ${tempCssPanY}px) scale(${tempCssScale})`;
    }

    // Applica definitivamente lo spostamento al motore di gioco quando si alza il dito
    function commitCSSTransform() {
        if (tempCssPanX === 0 && tempCssPanY === 0 && tempCssScale === 1) return;

        offsetX = (offsetX * tempCssScale) + tempCssPanX;
        offsetY = (offsetY * tempCssScale) + tempCssPanY;
        HEX_SIZE = Math.max(15, Math.min(100, HEX_SIZE * tempCssScale));
        ...
        // Calcola i limiti e prepara il layer
        clampCamera();
        invalidateStaticLayer();

        // FIX FLICKER: Forza il ridisegno IMMEDATO scavalcando la coda dei 15 FPS.
        // Javascript applicherà il reset del CSS e i nuovi pixel del Canvas 
        // nello stesso identico fotogramma del browser. Zero flicker.
        if (typeof window._executeDraw === 'function') {
            window._executeDraw();
        } else {
            drawGame();
        }
    ...
    // FIX: Se il tocco non è direttamente sul canvas (ma su un pulsante sopra di esso), ignora.
    ...
    canvas.addEventListener('touchmove', (e) => {
        ...
        if (isDragging && e.touches.length === 1) {
            ...
            // Calcola il centro del pinch (dove si trovano le dita) rispetto al centro dello schermo
            ...
            // FIX LIMITI: Impediamo che l'animazione CSS zoomi oltre i limiti del gioco (15 - 100)
            const prospectiveHexSize = HEX_SIZE * tempCssScale * factor;
            let allowedFactor = factor;
            
            if (prospectiveHexSize < 15) allowedFactor = 15 / (HEX_SIZE * tempCssScale);
            else if (prospectiveHexSize > 100) allowedFactor = 100 / (HEX_SIZE * tempCssScale);

            if (allowedFactor !== 1) {
                tempCssScale *= allowedFactor;
                // FIX AGGANCIO: Spostiamo fisicamente la mappa in senso opposto allo zoom 
                // per tenere l'immagine incollata alle dita del giocatore.
                tempCssPanX += midX * (1 - allowedFactor);
                tempCssPanY += midY * (1 - allowedFactor);
                applyCSSTransform();
            }
...
function startActiveGameUI(startingPlayer) {
    ...
    autoFitMap(); // Calcola il livello di zoom base per far stare la mappa

    // --- Centra la telecamera sul quartier generale del giocatore locale ---
    // Imposta il flag prima di chiamare, così _applyFullStateSync non ripete il focus
    if (typeof focusCameraOnLocalPlayer === 'function') {
