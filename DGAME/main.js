...
function initGameEngine() {
    // Forza fullscreen all'avvio
    checkAndApplyFullscreen();

    // Tenta di attivare il fullscreen reale del dispositivo
    if (document.documentElement.requestFullscreen) {
        document.documentElement.requestFullscreen().catch(e => console.log("Fullscreen negato"));
    }
    ...
    if (typeof SFX !== 'undefined') {
        Object.entries(SFX).forEach(([key, audio]) => {
            audio.play().then(() => {
                if (key !== 'heal' && key !== 'bgMusic') {
                    audio.pause();
                    audio.currentTime = 0;
                } else if (key === 'bgMusic') {
                    // 1. Sincronizziamo la variabile globale
                    musicPlaying = true; 
                    
                    // 2. Aggiorniamo i pulsanti (Battaglia e Campagna) per indicare "Acceso"
                    const activeColor = '#00ff88'; // Verde neon
        ...
        // Avvio del suono iniziale scelto
        playSFX('heal');
    }
