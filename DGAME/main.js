...
function initGameEngine() {
    ...
    canvas.addEventListener('touchmove', (e) => {
        ...
        if (isDragging && e.touches.length === 1) {
            ...
            // Punto medio tra le due dita, relativo al centro schermo
            // (stesso sistema di coordinate usato da hexToPixel/offsetX/offsetY)
            ...
            // Mantiene fisso il punto sotto le dita durante lo zoom
            offsetX = midX - (midX - offsetX) * factor;
            offsetY = midY - (midY - offsetY) * factor;

            HEX_SIZE = Math.max(15, Math.min(60, HEX_SIZE * factor));
...
canvas.addEventListener('mousedown', (e) => {
    if (e.button !== 2 && e.button !== 1) return; // tasto destro o tasto centrale
    if (e.button === 1) e.preventDefault(); // evita l'autoscroll nativo del browser sul tasto centrale
